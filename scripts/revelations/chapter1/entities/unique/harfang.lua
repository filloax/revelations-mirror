...
revel:AddCallback(ModCallbacks.MC_NPC_UPDATE, function(_, npc)
    ...
    elseif data.state == "Flying" then
        if sprite:IsEventTriggered("Flap") then
            ...
            for i=1,10 do
                particleEmitter:EmitParticlesNum(SnowParticle, REVEL.ParticleSystems.LowGravity, Vec3(npc.Position.X + math.random(-60,60), npc.Position.Y, -100), -Vec3(npc.Velocity.X, npc.Velocity.Y, -8), 1, 0.5, 60)
            end
            --particleEmitter:EmitParticlesNum(SnowParticle, REVEL.ParticleSystems.LowGravity, Vec3(npc.Position.X + math.random(-60,60), npc.Position.Y, -100), -Vec3(npc.Velocity.X, npc.Velocity.Y, -8), 10, 0.5, 60)
        else
    ...
    elseif data.state == "Attacking" then
        ...
        elseif sprite:IsEventTriggered("Shoot") then
            ...
            for i=1,8 do
                ...
                REVEL.GetData(proj).projType = "Harfang"
                REVEL.GetData(proj).harfangTimer = rng:RandomInt(20)
                proj:Update()
            end
