...
REVEL.TrapTypes.BoulderTrap = {
    ...
    OnTrigger = function(tile, data, player)
        ...
        REVEL.GetData(boulder).NoHitPlayer = data.TrapIsPositiveEffect
    end,
...
revel:AddCallback(ModCallbacks.MC_POST_EFFECT_UPDATE, function(_, eff)
    ...
    if data.IsCrushingBoulder then
        if sprite:IsEventTriggered("ScreenShake") then
            ...
            for _, enemy in ipairs(REVEL.roomEnemies) do
                if enemy:IsVulnerableEnemy() and enemy.Position:DistanceSquared(eff.Position) < (60 + enemy.Size) ^ 2 and (not REVEL.GetData(enemy).HitByBoulder or REVEL.GetData(enemy).HitByBoulder < enemy.FrameCount) then
                    REVEL.GetData(enemy).HitByBoulder = enemy.FrameCount + 30
    ...
    if onGround then
        ...
        for _,slot in ipairs(Isaac.FindByType(EntityType.ENTITY_SLOT, -1, -1, false, false)) do
            local sdata = REVEL.GetData(slot)
    ...
    if doSpawnUrny then
        ...
        urny:GetSprite():Load("gfx/monsters/revel2/urny/urny.anm2", true)
        urny:GetSprite():SetFrame("Idle", 0)
        urny:GetSprite().Offset = Vector(0,-600)
        REVEL.GetData(urny).UrnyFallingEffect = true
    end
...
revel:AddCallback(ModCallbacks.MC_POST_EFFECT_UPDATE, function(_, eff)
    if REVEL.GetData(eff).UrnyFallingEffect then
        if eff.FrameCount <= 20 then
            ...
            urny:ClearEntityFlags(EntityFlag.FLAG_APPEAR)
            REVEL.GetData(urny).AlwaysActive = true
            eff:Remove()
        end
    end
end, 6)
