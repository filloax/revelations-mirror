...
local function ShouldRemoveReward(kind)
    ...
    for _, effect in ipairs(existingPactShops) do
        local edata = REVEL.GetData(effect)
...
local function SetVanityCollectible(effect, itemType)
    ...
    if EID and not hasBlind then
        ...
        effect:GetData().EID_Description = REVEL.GetEidItemDesc(itemType)
        if not effect:GetData().EID_Description.Name then
            local itemConfig = REVEL.config:GetCollectible(itemType)
            effect:GetData().EID_Description.Name = itemConfig.Name
        end
    end
end
...
local function rewardShop_Init(_, effect)
            ...
            for i = 1, reward.Amount or 1 do
                ...
                if rewardEntity.Variant == PickupVariant.PICKUP_TRINKET then
                    ...
                    if EID then
                        ...
                        effect:GetData().EID_Description = REVEL.GetEidTrinketDesc(configTrinket.ID)
                        if not effect:GetData().EID_Description.Name then
                            effect:GetData().EID_Description.Name = configTrinket.Name
                        end
                    end
                end
    ...
    elseif reward.Kind == RewardKinds.DEVIL_ROOM_TELEPORT then
        ...
        if EID then
            ...
            effect:GetData().EID_Description = {
                Name = "Divine Command",
...
local function rewardShop_PostUpdate(_, effect)
    ...
    if not data.Reward then
        if not data.LoggedUpdateError then
            REVEL.LogError("Vanity shop item with nil reward! At", REVEL.room:GetGridIndex(effect.Position))            
            data.LoggedUpdateError = true
        end
        return
    end
...
function REVEL.RenderVanityPrice(entity, renderOffset, price, pos, replaceOffset)
    ...
    for i = 1, #priceStr do
        ...
        elseif REVEL.WasChanged("vanity warning", GetPtrHash(entity), priceStr) then
            REVEL.DebugToString("[WARN] Vanity item price is wrong:", priceStr, entity)
        end
    end
...
local function rewardShop_PostRender(_, effect, renderOffset)
    ...
    if not reward or not rewardData then
        if not data.LoggedRenderError then
            REVEL.LogError("Vanity shop item with nil reward on render! At", REVEL.room:GetGridIndex(effect.Position))            
            data.LoggedRenderError = true
        end
        return
    end
...
REVEL.Rerolls.RegisterAsCollectible({
    IsRerollable = function(entity)
        return REVEL.GetData(entity).Reward.Kind == RewardKinds.COLLECTIBLE
            and REVEL.GetData(entity).Reward.Item
    end,
    GetItemId = function(entity)
        return REVEL.GetData(entity).Reward.Item
    end,
    GetPersistId = function(entity)
        return tostring(entity.SubType)
    end,
    GetPool = function(entity)
        return REVEL.GetData(entity).Reward.Pool
    end,
...
local function prankShop_Init(_, npc)
    ...
    if REVEL.STAGE.Tomb:IsStage() then
        sprite:Load("gfx/monsters/revel2/prank_tomb/prank_stand_tomb.anm2", true)
    end
...
local function prankShop_Update(_, npc)
    ...
    if npc.SubType ~= PRANK_SHOP_EMPTY_SUBTYPE then
        if not REVEL.MultiPlayingCheck(sprite, "Wheel_Win", "Wheel_Lose") then
            if REVEL.music:GetCurrentMusicID() == REVEL.MUSIC.VANITY_SHOP then
        ...
        local prankForceExploded = REVEL.GetData(npc).PrankForceExploded or npc:GetData().PrankForceExploded
        if exploded or prankForceExploded then
            npc.SubType = PRANK_SHOP_EMPTY_SUBTYPE
            REVEL.GetData(npc).PrankForceExploded = nil
...
local function vanityTrapdoor_SpawnCustomGrid(customGrid)
    ...
    REVEL.GetData(renderer).Trapdoor = EntityPtr(trapdoor)
end
...
local function vanityTrapdoor_PostEffectRender(_, eff)
    if eff.SubType == TRAPDOOR_RENDERER_SUBTYPE then
        local trapdoor = REVEL.GetData(eff).Trapdoor.Ref
        if trapdoor then
...
local function vanityRoom_PostRender()
    local isShopMusic = REVEL.music:GetCurrentMusicID() == REVEL.MUSIC.VANITY_SHOP
...
local function HideHUDAndThings()
    ...
    for _, ent in ipairs(makeInvisible) do
        REVEL.GetData(ent).WasVisible__Vanity = ent.Visible
        ent.Visible = false
    end
...
local function ShowHUDAndThings()
    ...
    for _, ent in ipairs(makeInvisible) do
        if REVEL.GetData(ent).WasVisible__Vanity then
            ent.Visible = true
        end
        REVEL.GetData(ent).WasVisible__Vanity = nil
    end  
        ...
        if not TriggeredMorshuThisRoom then
            ...
            for _, player in ipairs(REVEL.players) do
                ...
                then
                    ...
                    if bomb then
                        REVEL.GetData(bomb).MorshuTrigger = true
...
local function morshu_PostEntityRemove(_, entity)
    if REVEL.GetData(entity).MorshuTrigger then
        ShowMorshuAnim()
    end
end
...
local function morshu_player_EntityTakeDmg(_, player)
    ...
    and REVEL.GetData(player).WasVisible__Vanity ~= nil 
    then
        return false
    end
end
...
local function SpawnCasinoDoor()
    ...
    if not extraRoomData then
        extraRoom = StageAPI.LevelRoom {
            ...
            IsExtraRoom = true,
            Music = REVEL.MUSIC.VANITY_CASINO
        }
...
local function casinoNight_RedKey_UseItem(_, itemType, rng, player, useFlags, activeSlot)
    if StageAPI.GetCurrentRoomType() == RevRoomType.VANITY then
        ...
        if player.Position:DistanceSquared(pos) < 160 ^ 2 then
            ...
            REVEL.PlayJingleForRoom(REVEL.MUSIC.SECRET_JINGLE)
            REVEL.music:Queue(REVEL.MUSIC.VANITY_SHOP)
