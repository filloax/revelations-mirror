...
revel:AddCallback(ModCallbacks.MC_FAMILIAR_UPDATE, function(_, e)
    if REVEL.ITEM.BCONTROL:PlayerHasCollectible(e.Player) then
        if not e:GetData().SpriteReplace then
            e:GetData().SpriteReplace = true
...
revel:AddCallback(ModCallbacks.MC_FAMILIAR_UPDATE, function(_, e)
    if REVEL.ITEM.BCONTROL:PlayerHasCollectible(e.Player) then
        if not e:GetData().BControl then
            e:GetData().BControl = true 
        end
    end
...
revel:AddCallback(ModCallbacks.MC_FAMILIAR_UPDATE, function(_, e)
    ...
    if REVEL.ITEM.BCONTROL:PlayerHasCollectible(player) and
        ...
        if not e:GetData().BirthControlled then
            ...
            e:GetData().BirthControlled = true
        end
    elseif e:GetData().BirthControlled then
        e.Visible = true
        e.OrbitDistance = EntityFamiliar.GetOrbitDistance(4)
        e:GetData().BirthControlled = nil
    end
