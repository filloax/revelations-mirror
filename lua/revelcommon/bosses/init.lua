...
do
    ...
    function REVEL.IsChampion(npc)
        ...
        if npc:GetData().IsChampion ~= nil then return npc:GetData().IsChampion end
...
do
    ...
    REVEL.HPScalingBalance = {
        ...
        ItemPowerModifier = { -- Iterated through in order, adding to a variable that DPS is multiplied by at the end
            ...
            {Multi = 1.65, MultiplyMulti = true, Tag = "CharonMerged", AntiTags = {"BlockRevCharon"}, OnlyIf = {REVEL.Dante.IsMerged}, Priority = 100},
            {Multi = 2.5, MultiplyMulti = true, AntiTags = {"CharonMerged", "BlockRevCharon"}, OnlyIf = {REVEL.Dante.IsDante}, Priority = 101},
        },
    ...
    ---@field OnlyIf table<fun(player: EntityPlayer): boolean>
    ---@field OnlyTags any[]
    ---@field AntiWeapons WeaponType[]
    ---@field AntiCharacters PlayerType[]
    ---@field AntiIf table<fun(player: EntityPlayer): boolean>
    ---@field AntiTags any[]
    ---@field Tag any
    ---@field Stacks any
    ...
    function REVEL.SetScaledBossHP(npc, targetLength, vulnerability, hp, hpPerSecond, expectedLength, noPrint)
        ...
        npc:GetData().HPScaled = hpPerSecond
    end
    ...
    revel:AddCallback(ModCallbacks.MC_ENTITY_TAKE_DMG, function(_, ent, amount, flags, source, iframes)
        ...
        if ent and hpPerSecond and source and not callingRescaledDamage then
            ...
            if dealBombDamage then
                local bombScaling = ent:GetData().BombScaling or 1
                ...
                ent:GetData().BombScaling = bombScaling + 1
                return false
            end
        end
    end)
