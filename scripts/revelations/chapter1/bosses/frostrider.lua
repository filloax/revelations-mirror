...
local function spawnStalactite(npc, pos, anim, preTarget)
    ...
    if preTarget then
        REVEL.GetData(stal).spawnedTarget = true
        REVEL.GetData(stal).target = preTarget
        REVEL.GetData(preTarget).noRequireStalactite = nil
        REVEL.GetData(preTarget).Stalactite = stal
    end
    return stal
end
...
local function spawnIcePooter(npc, pos, vel, isEmpty, speed)
    ...
    if isEmpty then
        REVEL.GetData(pooter).noPooter = true
        pooter:GetSprite():ReplaceSpritesheet(0, "gfx/monsters/revel1/ice_pooter/ice_pooter_empty.png")
        pooter:GetSprite():LoadGraphics()
    end
    if speed then
        REVEL.GetData(pooter).speed = speed
    end
...
local function frostRider_NpcUpdate(_, npc)
    ...
    local spr, data, target = npc:GetSprite(), REVEL.GetData(npc), npc:GetPlayerTarget()
    ...
    if not data.Init then
        ...
        if data.IsChampion then
            ...
            REVEL.GetData(head).IsChampion = true
            REVEL.GetData(head).Body = npc
            REVEL.GetData(head).AttackCooldown = data.bal.SpawnHeadCooldown
    ...
    for i,e in ripairs(stals) do --reverse look (for optimization as were removing stuff from the table) the stalactites table for any ded ones and contact with shockwaves
        for j,t in ripairs(FrostRiderVars.shocks) do
            if REVEL.GetData(e).type then
                if t.Position:Distance(e.Position) < 32 and (e:GetSprite():IsPlaying("Idle"..REVEL.GetData(e).type) or e:GetSprite():WasEventTriggered("Collision")) then
                    e:Die()
                    t:Remove()
    ...
    if data.State == "Idle" then
        ...
        if data.IsChampion and (data.Head and data.Head:Exists() and not data.Head:IsDead()) then
            ...
            local hdata = REVEL.GetData(head)
            ...
            if data.AttackCooldown <= 0 then
                ...
                elseif attack == "SpitPooter" then
                    ...
                    if data.NoPooter then
                        spr:ReplaceSpritesheet(2, "gfx/monsters/revel1/ice_pooter/ice_pooter_empty.png")
                        spr:LoadGraphics()
                    else
                        spr:ReplaceSpritesheet(2, "gfx/monsters/revel1/ice_pooter/ice_pooter.png")
                        spr:LoadGraphics()
                    end
                elseif attack == "Dash" then
                    spr.FlipX = target.Position.X < npc.Position.X
    ...
    elseif data.State == "SwingAtHead" then
        if data.Head then
            REVEL.GetData(data.Head).State = "Bouncing"
        end
        ...
        if spr:IsEventTriggered("Spawn") then
            ...
            if data.Head then
                ...
                REVEL.GetData(data.Head).Hit = true
            else
        ...
        elseif spr:IsEventTriggered("Spit") then
            ...
            for i = 1, math.random(8, 14) do
                ...
                REVEL.GetData(pro).FamineSpawned = true
            end
        elseif spr:IsFinished("AttackPooter") then
            data.State = "Idle"
            data.AttackCooldown = math.random(45, 75)
    ...
    elseif data.State == "DashEnd" then
        ...
        if spr:IsEventTriggered("Hit") then
            ...
            if data.bal.Dash.SpawnStalactites then
                ...
                REVEL.GetData(firstTarget).noRequireStalactite = true
...
local function frostRiderPhase2_NpcUpdate(_, npc)
    local data, spr, target = REVEL.GetData(npc), npc:GetSprite(), npc:GetPlayerTarget()
    ...
    if spr:IsEventTriggered("Shoot") then
        ...
        elseif data.State == "Pooter" then
            ...
            REVEL.GetData(pooter).HitCooldown = 5
        end
