...
revel:AddCallback(ModCallbacks.MC_POST_PICKUP_UPDATE, function(_, pickup)
    ...
    if REVEL.IsDanteCharon(player) then
        ...
        elseif pickup.Variant == PickupVariant.PICKUP_COLLECTIBLE then
            ...
            local data = REVEL.GetData(pickup)
            if data.CharonBossChoice and not data.CharonOtherChoiceSpawned then
                ...
                REVEL.GetData(newpickup).CharonBossChoice = true
                REVEL.GetData(newpickup).CharonOtherChoiceSpawned = true
                ...
                REVEL.GetData(repickup).CharonBossChoice = true
                REVEL.GetData(repickup).CharonOtherChoiceSpawned = true
                pickup:Remove()
            end

            if REVEL.GetData(pickup).CharonBossChoice and pickup.SubType == 0 then
                REVEL.GetData(pickup).CharonBossChoice = nil
                ...
                for _, collectible in ipairs(otherCollectibles) do
                    if REVEL.GetData(collectible).CharonBossChoice and collectible.SubType ~= 0 then
                        REVEL.GetData(collectible).CharonBossChoice = nil
                        --[[
...
revel:AddCallback(ModCallbacks.MC_POST_PICKUP_INIT, function(_, pickup)
    ...
    and (REVEL.level:GetStage() ~= LevelStage.STAGE3_1 
        ...
        REVEL.GetData(pickup).CharonBossChoice = true
            ...
            if shouldBeCombined then
                ...
                if wavesCount >= 2 then
                    ...
                    if not revel.data.run.dante.IsDante or revel.data.run.dante.IsCombined then
                        REVEL.GetData(player).NoShotYet = true
                        REVEL.GetData(player).AimBonus = 1000
                    end
...
StageAPI.AddCallback("Revelations", RevCallbacks.ON_TEAR, 1, function(tear, data, sprite, player, split)
    if player and REVEL.Dante.IsDante(player) and not REVEL.IncludesIncubusTear(tear) then
        ...
        REVEL.GetData(tear).DanteInkTear = true

        -- Synergyes

        -- Burning bush
        if REVEL.BurningBush.IsShootingFireTear() then
            REVEL.GetData(tear).CustomColor = Color(0.3, 0.3, 0.5)
            REVEL.GetData(tear).NoInkCreep = true
        end
    end
end)
...
revel:AddCallback(ModCallbacks.MC_POST_ENTITY_REMOVE, function(_, tear)
    if REVEL.GetData(tear).DanteInkTear and not REVEL.GetData(tear).NoInkCreep then
