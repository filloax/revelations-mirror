...
REVEL.Elites = {
    Chuck = {
        Music = REVEL.MUSIC.ELITE1,
        ClearMusic = REVEL.MUSIC.GLACIER_BOSS_OUTRO
    }
}
...
function REVEL.AddSpikesToIceHazard(entity, dontLock)
    if not dontLock then
        REVEL.GetData(entity).LockedInPlace = true
        entity.Mass = 2000
    end
...
local function spawnChuckEntity(index)
    ...
    if inRadius == 0 then
        ...
        if entities and #entities > 0 then
            ...
            REVEL.GetData(stalTarget).noRequireStalactite = true
...
revel:AddCallback(RevCallbacks.POST_ENTITY_ZPOS_LAND, function(_, entity, airMovementData, fromPit)
    ...
    for i = 1, 4 do
        REVEL.SpawnIceRockGib(entity.Position, RandomVector() * math.random(1, 5), entity, REVEL.GetData(entity).DarkIce and REVEL.IceGibType.DARK)
    end
...
revel:AddCallback(ModCallbacks.MC_NPC_UPDATE, function(_, npc)
    ...
    local sprite, data, target, path = npc:GetSprite(), REVEL.GetData(npc), npc:GetPlayerTarget(), npc.Pathfinder
    ...
    elseif data.State == "SpawnIceBlock" then
        ...
        elseif data.Substate == "Start" then
            ...
            if sprite:IsEventTriggered("Throw") then
                ...
                REVEL.GetData(block).State = "Toss"
                REVEL.GetData(block).FaceDir = faceDir
            end
    ...
    elseif data.State == "Idle" then
        ...
        if data.IdleWait <= 0 then
            if data.IdleState == 0 then
                if block then
                    if REVEL.GetData(block).State == "Idle" then
                        data.IdleWait = nil
                        data.IdleState = 1
    ...
    elseif data.State == "BlockChain" then
        ...
        if not data.Substate then
            ...
            for i, hazard in ipairs(data.TempHazards) do
                if REVEL.GetData(hazard).LockedInPlace then
        ...
        elseif data.Substate == "Push" then
            ...
            elseif data.HazardTarget and data.HazardSmash and #data.HazardOrder > 0 then
                ...
                if npc.Position:Distance(block.Position) > data.bal.PushRangeMin and REVEL.GetData(block).HazardTarget then
                    REVEL.GetData(block).HazardTarget = nil
                ...
                elseif npc.Position:Distance(block.Position) > data.bal.PushRangeMin*0.66 and not REVEL.GetData(block).HazardTarget then
                ...
                --Push
                    REVEL.GetData(block).HazardTarget = data.HazardTarget
        ...
        elseif data.Substate == "Adjust" then
            if not data.HazardTarget and #data.HazardOrder > 0 then
                ...
                if npc:CollidesWithGrid() then
                    ...
                    REVEL.GetData(block).LockPosition = block.Position
                end
            end
        ...
        elseif data.Substate == "Punch" then
            ...
            if sprite:IsEventTriggered("Hit") then
                if npc.Position:Distance(block.Position) < data.bal.BlockPunchRange then
                    ...
                    REVEL.GetData(block).PunchVec = (data.PunchPos-block.Position):Normalized()
                end
            end

            if sprite:IsFinished("HitIceBlock") then
                if not REVEL.GetData(block).PunchVec then
                    sprite:Play("Dash Start")
                    data.PunchPos = nil
                    data.Substate = "Push"
                else
    ...
    elseif data.State == "Throw" then
        ...
        if sprite:IsEventTriggered("Throw") and sprite:IsPlaying("Throw") then
            ...
            REVEL.GetData(block).State = "Thrown"
        end
        ...
        if sprite:IsEventTriggered("Throw") and sprite:IsPlaying("Catch") then
            REVEL.GetData(block).State = "ThrownHigh"
            REVEL.PlaySound(npc, data.bal.Sounds.Throw)
        end
    ...
    elseif data.State == "Dashing" then
        ...
        elseif sprite:IsPlaying("Dash") or sprite:WasEventTriggered("Dash") or sprite:IsPlaying("Turn") or isJumping then
            ...
            if block then
                if block.Position:DistanceSquared(npc.Position) < (block.Size + npc.Size + data.bal.ExtraImpactDistance) ^ 2 then
                    ...
                    REVEL.GetData(block).PunchVec = (block.Position-npc.Position):Normalized()
                    npc.Velocity = Vector.Zero
                    REVEL.PlaySound(npc, data.bal.Sounds.Bump)
                end
            end
        ...
        for _, player in ipairs(REVEL.players) do
            if not REVEL.GetData(player).Pitfalling and player.Position:DistanceSquared(npc.Position) < (player.Size + npc.Size + data.bal.ExtraImpactDistance) ^ 2 and player:GetDamageCooldown() > 0 then
    ...
    elseif data.State == "Hit" then
        if sprite:IsEventTriggered("Hit") and data.HittingEnemies then
            for _, enemyData in ipairs(data.HittingEnemies) do
                ...
                if REVEL.GetData(icehazard).LockedInPlace then
                    REVEL.sfx:NpcPlay(npc, REVEL.SFX.MINT_GUM_BREAK, 1, 0, false, 1)
                    REVEL.GetData(icehazard).BreakShotVel = data.bal.HazardShotSpeed
                    REVEL.GetData(icehazard).BreakShotRotated = math.random(0,1)
                end

                REVEL.GetData(icehazard).LockedInPlace = false
            end
        ...
        elseif data.Substate == "BlockSpawn" then
            ...
            if sprite:IsFinished("SpawnIceDeath") then
                ...
                for i = 1, math.random(20, 25) do
                    ...
                    REVEL.GetData(projectile).SpawnIceCreep = true
                    -- REVEL.GetData(projectile).IceCreepTimeout = projtimeout
                    REVEL.GetData(projectile).IceCreepScaleMulti = 1
                end
        ...
        elseif data.Substate == "BlockSmash" then
            ...
            elseif sprite:WasEventTriggered("Dash") or sprite:IsPlaying("Dash3") then
                ...
                elseif npc.Position:Distance(block.Position) <= data.bal.PushRangeMin then
                    ...
                    REVEL.GetData(block).State = "Death"
                    REVEL.GetData(block).ChuckPosition = npc.Position
                    npc:Remove()
                    return
                else
...
revel:AddCallback(ModCallbacks.MC_NPC_UPDATE, function(_, npc)
    ...
    if data.Chuck and not data.bal then
        data.bal = REVEL.GetData(data.Chuck).bal
        if REVEL.GetData(data.Chuck).Gigachuck then
            data.Gigachuck = true
    ...
    if not data.Airborne then
        ...
        if npc.Velocity:Length() > 4 then
            for _,ent in ipairs(REVEL.roomEnemies) do
                ...
                and not REVEL.GetData(ent).ChuckSmacked then
    ...
    elseif data.HazardTarget then
        ...
        if npc.Position:Distance(data.HazardTarget.Position) < 50 then
            REVEL.GetData(data.Chuck).HazardSmash = true

            if data.HazardTarget:Exists() then
                local shotSpeed = data.bal.HazardShotSpeed
                local rotated = 0
                if #REVEL.GetData(data.Chuck).HazardOrder % 2 == 0 then rotated = 45 end
                for i = 1, 4 do
                    Isaac.Spawn(9, 4, 0, data.HazardTarget.Position, Vector.FromAngle((i * 90) + rotated) * shotSpeed, npc)
                end

                REVEL.GetData(data.HazardTarget).ChuckSmackVel = npc.Velocity*data.bal.EnemySmackMulti
...
revel:AddCallback(ModCallbacks.MC_ENTITY_TAKE_DMG, function(_, entity, amount)
    if entity.Variant == REVEL.ENT.CHUCK.variant then
        ...
        if entity.HitPoints - amount - REVEL.GetDamageBuffer(entity) <= 0 then
            ...
            splat.Color = REVEL.GetData(entity).bal.BloodColor
            ...
            explosion.Color = REVEL.GetData(entity).bal.BloodColor
            ...
            guy.Visible = false
            guy.SplatColor = REVEL.GetData(entity).bal.BloodColor
            guy:Kill()

            -- entity.HitPoints = entity.MaxHitPoints
            REVEL.GetData(entity).State = "Respawn"
            REVEL.GetData(entity).Death = true
            REVEL.GetData(entity).Substate = nil
            entity:AddEntityFlags(EntityFlag.FLAG_NO_STATUS_EFFECTS)
            entity.Visible = false
            entity.CollisionDamage = 0
            entity.Position = REVEL.GetData(entity).StartPosition
            ...
            for _, block in ipairs(iceblocks) do
                local sprite, data = block:GetSprite(), REVEL.GetData(block)
