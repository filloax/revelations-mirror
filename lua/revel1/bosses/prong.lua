...
revel:AddCallback(ModCallbacks.MC_POST_EFFECT_UPDATE, function(_, eff)
    if eff:GetData().ProngCrack then
...
local function CrackSegment(seg, longCrack, noRemoveCrack)
    ...
    if not seg.CrackEffect and crack then
        ...
        seg.CrackEffect:GetData().ProngCrack = true
    ...
    if not seg.Broken then
        ...
        for _, ent in ipairs(hazards) do
            if breakingGrids[REVEL.room:GetGridIndex(ent.Position)] then
                ...
                ent:GetData().HazardStart = ent.Position
                ent:GetData().HazardTarget = Isaac.GetFreeNearPosition(npc:GetPlayerTarget().Position, 0)
                ent:GetData().HazardTime = 0
                ent:GetData().HazardFlightTime = npc:GetData().bal.HazardFlightTime
                ent:GetData().NoBreak = true
...
local function JumpPosition(npc, segments, attack)
    ...
    if #jumpPositions > 0 then
        ...
        npc:GetData().CurrentPositionSegment = jumpPositions[currentSegment].Index
    else
...
revel:AddCallback(ModCallbacks.MC_NPC_UPDATE, function(_, npc)
        ...
        if sprite:IsEventTriggered("Spawn") then
            ...
            for i = 1, numToSpawn do
                ...
                if REVEL.ENT.ICE_WORM:isEnt(toSpawn) then
                    ...
                    worm:GetData().State = "Arise"
                    -- REVEL.sfx:Play(REVEL.SFX.ICE_WORM_BOUNCE, 1.3, 0, false, 1.1)
                    if data.InvincibleWormSpawn then
                        worm:GetData().InvulnerableWithEntity = npc
    ...
    elseif data.State == "BalancingBlock" then
        ...
        elseif sprite:IsEventTriggered("Spawn") then
            ...
            data.IceHazard:GetData().HazardStart = data.IceHazard.Position
            data.IceHazard:GetData().HazardTarget = Isaac.GetFreeNearPosition(npc:GetPlayerTarget().Position, 0)
            data.IceHazard:GetData().HazardTime = 0
            data.IceHazard:GetData().HazardFlightTime = data.bal.HazardFlightTime
    ...
    elseif data.State == "HighHail" then
        if sprite:IsEventTriggered("Freeze") then
            if not data.bal.HighHailUseBomb then
                ...
                stalactite:GetData().type = math.random(1, 3)
                stalactite:GetData().DontShootEnd = true
                stalactite:GetSprite():Play("Freeze"..stalactite:GetData().type, true)
                stalactite:GetData().init = true
        ...
        if data.CrackKebab then
            if not data.DeliciousWorm or not data.DeliciousWorm:Exists() then
                ...
                if #iceWorms > 0 then
                    ...
                    data.DeliciousWorm:GetData().InvulnerableWithEntity = npc
                else
                    cutShort = true
                end
            end
            ...
            if not cutShort then
                data.DeliciousWorm:GetData().ForceNextPosition = data.CrackKebab.TargetPos
        ...
        if data.Blowing then
            ...
            if not data.WindMoving then
                ...
                for _, player in ipairs(REVEL.players) do
                    ...
                    and not player:GetData().TotalFrozen 
                    and not player:GetData().FrozenPitFalling 
                ...
                for _, player in ipairs(REVEL.players) do
                    if REVEL.IsPositionInRect(player.Position, affectedAreaTL, affectedAreaBR) then
                        if not player:GetData().TotalFrozen and not player:GetData().FrozenPitFalling then
                            if REVEL.Glacier.CheckIce(player) then
                                REVEL.Glacier.TotalFreezePlayer(player, data.BlowDir, 8, nil, true, false, true)
                            end
                        else
                            player:GetData().TFWaitFrameForResurface = player.FrameCount + 5 --wait resurfacing until prong has passed the area
                        end
                    end
                end
...
revel:AddCallback(ModCallbacks.MC_ENTITY_TAKE_DMG, function(_, ent, amount, flag)
    if REVEL.ENT.PRONG:isEnt(ent) and ent:GetData().State == "Appear" then
...
revel:AddCallback(ModCallbacks.MC_POST_BOMB_UPDATE, function(_, bomb)
    ...
    then
        local sprite, data = bomb:GetSprite(), bomb:GetData()
