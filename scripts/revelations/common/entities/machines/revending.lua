            ...
            local spawnable = REVEL.filter(RevelShopPool, function(item)
                ...
                local cannotAdd = REVEL.some(REVEL.players, function(player)
                    return player:HasCollectible(item.id) 
                        ...
                        or (REVEL.GetData(player).RevendingMachineBlacklist and REVEL.GetData(player).RevendingMachineBlacklist[item.id])
                end)
...
local function revendingMachineUpdate(machine, data)
    ...
    if sprite:IsFinished("Prize") then
        ...
        REVEL.GetData(collectible).Revended = true
        REVEL.GetData(collectible).RevendedPrice = price
        REVEL.GetData(collectible).RevendedDiscount = discount
        REVEL.GetData(collectible).RevendedType = type
        REVEL.GetData(collectible).RevendedRestockAmount = data.RestockAmount or 0
    end
end
...
local function revendingPostPickupCollectible(_, pickup)
    ...
    and pickup.SubType == 0 then --picked up sold item
        ...
        if not doExplode then
            local newPrice = REVEL.GetData(pickup).RevendedPrice + 2
            local type = REVEL.GetData(pickup).RevendedType
            local discounted = REVEL.GetData(pickup).RevendedDiscount
            pickup:Remove()
        ...
        elseif not REVEL.GetData(pickup).Exploded then
            REVEL.GetData(pickup).Exploded = true
