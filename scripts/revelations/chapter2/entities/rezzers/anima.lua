...
local function isUnbuffedRagFamily(ent)
    return REVEL.RAG_FAMILY[ent.Type] and not REVEL.GetData(ent).Buffed
end

local function anima_NpcUpdate(_, npc)
    if npc.Variant ~= REVEL.ENT.ANIMA.variant then return end
    local data, spr = REVEL.GetData(npc), npc:GetSprite()
    ...
    elseif data.State == "Move Default" then
        ...
        elseif not spr:IsPlaying("PickupSkull") then
            ...
            if data.FrameCount > 10 and #ragNpcs ~= 0 then
                ...
                REVEL.GetData(data.targ).Anima = npc
    ...
    elseif data.State == "GoTo Host" then
        ...
        if (targ:IsDead() or not targ:Exists()) or (REVEL.GetData(targ).Anima.InitSeed ~= npc.InitSeed) then --if dead or targeted simultaneously as another anima
            data.State = "Move Default"
        else
    ...
    elseif data.State == "Possess" then
        ...
        if targ:IsDead() or not targ:Exists() then
            ...
            elseif spr:IsFinished("Possess") then
                data.State = "Buffing"
                data.Skull = REVEL.SpawnDecoration(npc.Position, Vector.Zero, "SkullIdle", "gfx/monsters/revel2/anima/anima.anm2", nil, nil, nil, skullFinish, skullUpdate, nil, false)
            end
        end
    ...
    elseif data.State == "Buffing" then
        ...
        if targ:IsDead() or not targ:Exists() or data.Skull:IsDead() or not data.Skull:Exists() then
            ...
            if targ:Exists() then
                REVEL.GetData(targ).Buffed = false
                REVEL.GetData(targ).Anima = nil
            end
    ...
    if (REVEL.room:IsClear() and REVEL.includes(dieableStates, data.State)) then
        ...
        if #ragNpcs == 0 then
            if data.State == "Move Default" then --drop skull, die
                data.Skull = REVEL.SpawnDecoration(npc.Position, Vector.Zero, "DropSkull", "gfx/monsters/revel2/anima/anima.anm2", nil, nil, nil, skullFinish, skullUpdate, nil, false)
                spr:Play("Death")
                data.State = "Death"
            elseif data.State == "GoTo Skull" then
                spr:Play("Death")
...
revel:AddCallback(ModCallbacks.MC_NPC_UPDATE, function(_, npc)
    if not REVEL.GetData(npc).AnimaChecked then
        REVEL.GetData(npc).AnimaChecked = true
            ...
            if currentRoom then
                ...
                if currentRoom.Metadata:Has{Index = index, Name = "Anima"} then
                    ...
                    if isAnimaEntity then
                        ...
                        local adata = REVEL.GetData(anima)
                        adata.targ = npc
                        adata.ForcePossess = true

                        local ndata = REVEL.GetData(npc)
                        ndata.Buffed = true
                        ndata.Anima = anima
                    end
                end
