...
local function iceHazardInit(npc)
    local spr, data = npc:GetSprite(), REVEL.GetData(npc)
...
function REVEL.BreakIceHazard(npc)
    ...
    for i=1, 8 do
        REVEL.SpawnIceRockGib(npc.Position, Vector.FromAngle(1*math.random(0, 360)):Resized(math.random(1, 5)), npc, REVEL.GetData(npc).DarkIce and REVEL.IceGibType.DARK or REVEL.IceGibType.DEFAULT)
    end
    ...
    if npc.Variant == REVEL.ENT.ICE_HAZARD_GAPER.variant then
        ...
        npcvariant = 1
        gaperSkin = tostring(REVEL.GetData(npc).Skin)
    elseif npc.Variant == REVEL.ENT.ICE_HAZARD_DRIFTY.variant then
    ...
    local smackVel
    smackVel = REVEL.GetData(npc).ChuckSmackVel or nil

    local shotVel, shotRotated
    shotVel = REVEL.GetData(npc).BreakShotVel or nil
    shotRotated = REVEL.GetData(npc).BreakShotRotated or nil
    ...
    if npc:ToNPC() then
        ...
        if npctype and npcvariant then
            ...
            if smackVel then
                REVEL.GetData(enemy).ChuckSmacked = smackVel
            ...
            REVEL.GetData(enemy).FromIceHazard = true
            ...
            if ShutsDoorsInClearStartRooms and REVEL.WasRoomClearFromStart() then
                REVEL.GetData(enemy).IceHazardKeepDoorsClosed = true
...
revel:AddCallback(ModCallbacks.MC_NPC_UPDATE, function(_, npc)
    ...
    elseif data.NotMovingLastFrame and not data.SpawnedGibs then
        for i = 1, 3 do
            REVEL.SpawnIceRockGib(npc.Position, RandomVector() * math.random(1, 4), npc, REVEL.GetData(npc).DarkIce and REVEL.IceGibType.DARK or REVEL.IceGibType.DEFAULT)
            end
        ...
        if not doBreak then
            ...
            for _, spike in pairs(stalagSpikes) do
                if spike.EntityCollisionClass ~= EntityCollisionClass.ENTCOLL_NONE and spike.Position:DistanceSquared(npc.Position) < (spike.Size + npc.Size)^2 then
                    doBreak = true
                    spike:GetSprite():Play("Explode" .. REVEL.GetData(spike).randomnum, false)
                end
            end
        end
...
function REVEL.MorphToIceHazard(entity, lockTime, spiked)
    ...
    if hazardDef then
        ...
        if lockTime then
            REVEL.GetData(entity).LockedInPlaceTime = lockTime
        end
        if spiked then
...
local function icehazardHorfPreUpdate(_, npc)
    if REVEL.GetData(npc).FromIceHazard and npc.FrameCount <= 60 then
...
local function icehazardDmg(_, npc, amount, flags)
    ...
    or HasBit(flags, DamageFlag.DAMAGE_CRUSH) then
        if amount > npc.MaxHitPoints*5 then
            REVEL.GetData(npc).Overkill = true
