...
-- TODO: check if repentogon can replace this
...
-- TODO: replace as much of this as possible with repentogon
...
revel:AddCallback(RevCallbacks.POST_BASE_PEFFECT_UPDATE, function(_, p)
    if not REVEL.GetData(p).__chargeMax then
...
function REVEL.GetMaxCharge(p, base, itemId)
    ...
    if not REVEL.GetData(p).__chargeMaxBase[itemId] then
        REVEL.GetData(p).__chargeMaxBase[itemId] = barItemsMaxCharge[itemId]
        REVEL.GetData(p).__chargeMax[itemId] = barItemsMaxCharge[itemId]
    end

    if base then
        return REVEL.GetData(p).__chargeMaxBase[itemId]
    else
        return REVEL.GetData(p).__chargeMax[itemId]
    end
end
...
function REVEL.SetBatteryChargeMult(p, mult)
    REVEL.GetData(p).__batMult = mult
end
