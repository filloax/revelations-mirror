...
function REVEL.SpawnHaemoProjectile(pos, vel, spawner, color)
    ...
    --projectile:Update()
    projectile:GetData().IsHaemo = true
	projectile:GetData().HaemoColor = color
...
function REVEL.SpawnWilliwawClone(pos, vel, spawner)
	...
	clone:GetData().IsWilliwawClone = true
...
revel:AddCallback(ModCallbacks.MC_POST_PROJECTILE_UPDATE, function(_, proj)
    if not proj:GetData().IsHaemo then return end
    ...
    if proj.FrameCount % 5 == 0 then
		...
		if proj:GetData().HaemoColor then
			trail:GetSprite().Color = proj:GetData().HaemoColor
		end
    end
    if proj:IsDead() then
        ...
        for i = 1, projnum do
			...
			eff:GetSprite().Color = proj:GetData().HaemoColor
        end
    end
end)
...
GoRainDash = function(n, s, d)
    ...
    QueueAction(n, d, function(npc, sprite, data)
        ...
        for _, aura in pairs(data.FreezeAuras) do
            REVEL.AuraExpandFade(aura, 5, aura:GetData().Radius * 1.6)
        end
        data.FreezeAuras = {}
	...
	QueueAction(n, d, function(npc, sprite, data)
		...
		if not data.LastSnowflakeWave and data.TimeUntilNextSnowflakePattern == 0 then
			...
			for _,pos in ipairs(pattern.Snowflakes) do
				...
				snowflake:GetData().State = "Projectile"
				snowflake:GetData().MaxMarginOutsideRoom = -10000
				REVEL.DelayFunction(function() snowflake:GetData().MaxMarginOutsideRoom = -100 end, 15 + math.ceil(pattern.Length*7.5), nil, true)
			end
		...
		if sprite:IsEventTriggered("Wall") then
			...
			data.Wisp:GetData().IsProjectile = true
			data.Wisp:GetData().ProjectileVelocity = data.Wisp.Velocity
			data.Wisp:GetData().MaxMarginOutsideRoom = -120
		end
...
RotateWithClonesAroundTarget = function(npc, centerPos, rotation, radius, numClones)
	...
	if numClones then
		for i=1, numClones do
			local clone = npc:GetData().Clones[i]
		...
		for i,clone in ipairs(npc:GetData().Clones) do
			targetPos = centerPos + Vector.FromAngle(rotation + i*(360/(data.bal.SteamOutNumClonesPhase3+1)))*radius
			clone.Velocity = (targetPos-clone.Position) * 0.2
		end
	end
...
GoGatewaySniping = function(n, s, d)
	...
	QueueAction(n, d, function(npc, sprite, data)
		if sprite:IsEventTriggered('Icicle') then
			...
			--[[for _,clone in ipairs(data.Clones) do
				clone:GetData().Snowflakes = {}
				for i = 1, data.bal.GatewaySnipingCloneFlakeCount do
					table.insert(clone:GetData().Snowflakes,
	...
	QueueAction(n, d, function(npc, sprite, data)
		...
		if sprite:IsEventTriggered('Land') then
			...
			--[[for _,clone in ipairs(data.Clones) do
				for _, snowflake in pairs(clone:GetData().Snowflakes) do
					local sdata = snowflake:GetData()
					sdata.MoveState = nil
					sdata.State = 'dummy'
	...
	QueueAction(n, d, function(npc, sprite, data)
		...
		for i=1, data.NumClones do
			...
			if clone then
				...
				elseif not clone:IsDead() and not clone:GetData().IsDying then
		...
		if data.DeathTimer%data.bal.CloneDeathInterval == 1 then
			...
			if data.ShockLevel > 0 then
				...
				for i=1, data.NumClones do
					local clone = data.Clones[i]
					if clone and not clone:GetData().IsDying and not clone:IsDead() and clone:Exists() then
						clone:GetData().DisableIdleAnimation = true
...
- Repeat until death
    ...
    GoIdle = function(n, s, d)
        ...
        QueueAction(n, d, function(npc, sprite, data)
			...
			if data.IsWilliwawClone and data.CurrentPhase ~= "Snow Clone" and npc.Parent:GetData().CurrentPhase == "Snow Clone" then
				data.CurrentPhase = "Snow Clone"
				data.IdleWait = nil
				data.ActionQueue = {}
			end
			...
			if data.CurrentPhase == "Fast & Flurrious" and data.ActionQueue[1] == nil and not data.IsWilliwawClone then
				for i=1, math.random(1,2) do
					...
					QueueAction(data.Clones[1], data.Clones[1]:GetData(), 'GoRainDash')
					data.bal.AddIdleWait(data.Clones[1], data.Clones[1]:GetData(), REVEL.GetFromMinMax(data.bal.IdleWaitWithClone))
					QueueAction(data.Clones[1], data.Clones[1]:GetData(), 'GoIdle')
				end
				
				QueueAction(npc, data, 'GoSnowflakeSniping')
				data.bal.AddIdleWait(npc, data, REVEL.GetFromMinMax(data.bal.IdleWaitWithClone))

				data.Clones[1]:GetData().CurrentAttacks = {['RainDash'] = 1, ['SlowDrizzle'] = 1}
				data.bal.AddIdleWait(data.Clones[1], data.Clones[1]:GetData(), REVEL.GetFromMinMax(data.bal.IdleWaitWithClone))
				
				table.insert(data.ActionQueue, function()
					data.Clones[1]:GetData().CurrentAttacks = {}
					data.CurrentAttacks = {['SnowflakeSniping'] = 1, ['RainDash'] = 1}
					data.bal.AddIdleWait(npc, data, REVEL.GetFromMinMax(data.bal.IdleWaitWithClone))
					QueueAction(data.Clones[1], data.Clones[1]:GetData(), 'GoBeastWinds')
					table.insert(data.Clones[1]:GetData().ActionQueue, function()
						data.CurrentAttacks = {}
					end)
				end)
			end
			...
			if data.CurrentPhase == "Snow Clone" and data.ActionQueue[1] == nil and not data.IsWilliwawClone then
				...
				for i, clone in ipairs(data.Clones) do
					if clone:GetData().ActionQueue[3] then
...
StageAPI.AddCallback("Revelations", RevCallbacks.POST_INGAME_RELOAD, 1, function()
	...
	for _, npc in ipairs(wills) do
        npc:GetData().bal = REVEL.GetBossBalance(REVEL.WilliwawBalance, "Default") --change when adding champion
	end
end)
