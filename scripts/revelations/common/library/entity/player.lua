    ...
    if srcEnt and srcEnt:Exists() then
        ...
        elseif srcEnt.SpawnerType == 1 then
            if srcEnt.SpawnerEntity then
                ...
                player = REVEL.GetData(srcEnt).__player
            end
        end
...
function REVEL.WasPlayerDamagedThisRoom(player)
    return not not REVEL.GetData(player).damagedThisRoom
end

local function damagedThisRoomPlayerTakeDmg(e, dmg, flag, src, invuln)
    REVEL.GetData(e).damagedThisRoom = true
end

local function damagedThisRoomPostNewRoom()
    for _, player in ipairs(REVEL.players) do
        REVEL.GetData(player).damagedThisRoom = nil
    end
end
...
function REVEL.PlayerCameraMode(player)
    ...
    if not data.CameraMode then
        ...
        if not data.Camera.FakePlayer then
            ...
            REVEL.GetData(data.Camera.StandinEffect).Player = player
        end
        ...
        if data.Camera.WasForgottenSoul then
            ...
            if REVEL.GetData(mainPlayer).Bone then
                REVEL.GetData(mainPlayer).Bone.Visible = false
            end
        end
...
local function cameraModePlayerTakeDmg(_, player)
    if REVEL.GetData(player).CameraMode then
        return false
    end
end
...
local function cameraModeFamiliarUpdate(_, familiar)
    ...
    if REVEL.GetData(player).CameraMode 
    and REVEL.GetData(player).Camera.StandinEffect 
    and REVEL.GetData(player).Camera.StandinEffect:Exists() 
    then
...
local function cameraModeStandinPostRender(_, effect, renderOffset)
    local player = REVEL.GetData(effect).Player
...
function REVEL.NewPlayer(playerType, controllerIndex, strawmanParent)
    ...
    REVEL.GetData(player).RevFakePlayer = true
...
local function disablePlayerInput_InputAction(_, entity, inputHook, buttonAction)
    if entity and ActionsToDisable[buttonAction] and not REVEL.HasQueuedForcedInput(entity, buttonAction) then
...
function REVEL.CancelPlayerCutscene(player)
    REVEL.GetData(player).RevCutsceneData = nil
end
