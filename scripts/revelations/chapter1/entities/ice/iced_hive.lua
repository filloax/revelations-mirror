...
local function icedHive_NpcUpdate(_, npc)
    if npc.Variant == REVEL.ENT.ICED_HIVE.variant then
        ...
        local data, sprite, player, room = REVEL.GetData(npc), npc:GetSprite(), npc:GetPlayerTarget(), REVEL.room
...
revel:AddCallback(ModCallbacks.MC_POST_ENTITY_KILL, function(_, npc)
    ...
    for i = 1, 2 do
        if i == 2 and (math.random(1,2) <= 1 or REVEL.GetData(npc).PooterInMouth) then
