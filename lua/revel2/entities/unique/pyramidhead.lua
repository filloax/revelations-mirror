...
local function pyramidHead_NpcUpdate(_, npc)
    if npc.Variant == REVEL.ENT.PYRAMID_HEAD.variant then
        ...
        if data.Init == nil then
            ...
            data.bubble = Sprite()
            data.bubble:Load("gfx/monsters/revel2/pyramid_head_shield.anm2", true)
        ...
        if data.Shielded == true then
            for k, v in pairs(REVEL.roomEnemies) do
                ...
                and v:GetData().Mark == nil then
                v:GetData().Mark = Isaac.Spawn(1000, REVEL.ENT.PYRAMID_HEAD_TRIANGLE.variant, 0, v.Position - Vector(0, ( v.Size * 5 )), Vector (0, 0), nil)
                v:GetData().Mark:ToEffect():FollowParent(v)
                end
            end
        end
...
local function pyramidHeadDamage_EntityTakeDmg(_, ent, dmg, flag, source)
    if ent.Variant == REVEL.ENT.PYRAMID_HEAD.variant then
        if ent:GetData().Shielded == true or not ent:GetData().Appeared then
            return false
        end
    end
end
