...
revel:AddCallback(ModCallbacks.MC_ENTITY_TAKE_DMG, function(_, ent)
    if REVEL.GetData(ent).OopsBlockDamage then
        return false
    end
end)
        ...
        for i = 0, REVEL.room:GetGridSize() do
            ...
            if grid then
                ...
                elseif grid:ToPressurePlate() then
                    ...
                    REVEL.GetData(REVEL.player).OopsBlockDamage = true
                    grid:Update()
                    REVEL.GetData(REVEL.player).OopsBlockDamage = false
