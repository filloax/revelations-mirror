...
local function processDraugrTargetsForMap(draugr, targetSets, grillosAndFires, chillos, ...)
    local data = REVEL.GetData(draugr)
    ...
    GetTargetSets = function()
        ...
        for i, draugr, _ in REVEL.MultiTableIterate(draugrs, haugrs, juniaugrs) do
            if not draugrAvoidsFire(draugr) then
                ...
                chillos = rchillos
                REVEL.GetData(draugr).UsingAvoidFireMap = false
            end
        end
    ...
    GetTargetSets = function()
        ...
        for i, draugr, _ in REVEL.MultiTableIterate(draugrs, haugrs, jaugrs, juniaugrs) do
            if draugrAvoidsFire(draugr) then
                ...
                grillosAndFires = rgrillosAndFires
                chillos = rchillos
                REVEL.GetData(draugr).UsingAvoidFireMap = true
            end
        end
...
local function eat(npc, entity)
    ...
    if entity.Type == 33 then
        ...
        if REVEL.GetData(entity).aura then
            REVEL.GetData(entity).aura:Remove()
        end

        REVEL.GetData(entity).SpawnedGrillo = true --prevent grillo spawn
    ...
    elseif REVEL.ENT.CHILL_O_WISP:isEnt(entity) then
        if useAura then
            ...
            if eatenAuraData then
                ...
                local radius = REVEL.GetData(entity).radius or REVEL.GetChillFreezeRadius()
    ...
    if data.Aura then
        REVEL.GetData(data.Aura).Spawner = npc
...
local function draugrHaugrNpcUpdate(_, npc)
    ...
    if data.Holding and #data.Holding > 0 then
        ...
        if data.Aura then 
            REVEL.GetData(data.Aura).RenderOnTop = data.RenderOnTopParticles
        end
    end
    ...
    if not data.Eating and not data.HeadSprite:IsPlaying("Eat") and not ((REVEL.ENT.DRAUGR:isEnt(npc) or REVEL.ENT.JUNIAUGR:isEnt(npc)) and data.Holding and #data.Holding > 0) then
        ...
        for _, entity in ipairs(eatable) do
            if not REVEL.GetData(entity).BeingEaten and npc.Position:DistanceSquared(entity.Position) < (entity.Size + data.bal.EatDistance) ^ 2 then
                -- sprite:Play("Eat", true)
                data.HeadSprite:Play("Eat", true)
                data.Eating = entity
                REVEL.sfx:NpcPlay(npc, REVEL.SFX.ICE_MUNCH, 1, 0, false, 0.9+(math.random()*0.2))
                REVEL.GetData(data.Eating).StayStill = true
                REVEL.GetData(data.Eating).BeingEaten = true
                break
            end
        end
    end
...
local function isHazardHittableJaugr(entity, jaugr)
    ...
    --     REVEL.DebugToConsole(REVEL.GetData(entity).LockedInPlaceTime, REVEL.GetData(jaugr).LastFrozenEntitySeed ~= entity.InitSeed, REVEL.room:CheckLine(entity.Position, jaugr:GetPlayerTarget().Position, 0, 1000, false, false))
    -- end
    return entity.Type == REVEL.ENT.ICE_HAZARD_GAPER.id and not REVEL.GetData(entity).LockedInPlaceTime
    -- Only throw last frozen entity if it can be thrown directly at the player
    and (REVEL.GetData(jaugr).LastFrozenEntitySeed ~= entity.InitSeed or REVEL.room:CheckLine(entity.Position, jaugr:GetPlayerTarget().Position, 0, 1000, false, false))
end

local function getIsHittableJaugr(jaugr)
    return function(entity)
        return isHazardHittableJaugr(entity, jaugr)
        or REVEL.GetIceHazardForEnt(entity) and not (REVEL.ENT.BROTHER_BLOODY:isEnt(entity) and REVEL.ENT.JAUGR:isEnt(REVEL.GetData(entity).Host))
    end
end
...
local function jaugrNpcUpdate(_, npc)
    ...
    if data.State == "Walk" and npc.FrameCount > 5 then
        ...
        if punchTarget then
            ...
            REVEL.GetData(data.Punching).JaugrStunned = npc
        ...
        elseif dashDir then
            ...
            if not data.Eating and not isChilled then
                ...
                for _, entity in ipairs(eatable) do
                    if entity:Exists() and not REVEL.GetData(entity).BeingEaten and npc.Position:DistanceSquared(entity.Position) < (entity.Size + data.bal.EatDistance) ^ 2 then
                        sprite:Play("Eat", true)
                        REVEL.GetData(entity).StayStill = true
                        REVEL.GetData(entity).BeingEaten = true
                        data.Eating = entity
                        data.DontRenderHeadSprite = true
                        REVEL.sfx:NpcPlay(npc, REVEL.SFX.ICE_MUNCH, 1, 0, false, 0.9+(math.random()*0.2))
                        data.State = "Attack"
    ...
    elseif data.State == "Attack" then
        if REVEL.MultiPlayingCheckSuffix(sprite, data.bal.JaugrBatAnims, animSuffix) then
            ...
            if sprite:IsEventTriggered("Punch") and data.Punching and data.Punching:Exists() then --and data.Punching.Position:DistanceSquared(npc.Position) < (80) ^ 2 then
                ...
                REVEL.GetData(data.Punching).JaugrStunned = nil
                REVEL.GetData(data.Punching).LockedInPlaceTime = nil --in case immediately punched
                data.Punching = nil
            end
...
local function draugrPostEntityRemove(_, entity)
    ...
    elseif data.Eating then
        REVEL.GetData(data.Eating).BeingEaten = nil
        REVEL.GetData(data.Eating).StayStill = nil
    end
end
