...
-- Obsolete with Repentogon
-- (can't avoid in mod versions compatible with no-repentogon
-- as it needs replacing files which happens regardless)
...
StageAPI.AddCallback("Revelations", RevCallbacks.TEAR_UPDATE_INIT, 1,  function(tear) --not using tears_fire so this works on first update and so after all TEARS_FIRE callbacks
    local shouldPlay, data = true, REVEL.GetData(tear)
