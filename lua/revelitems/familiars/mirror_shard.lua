...
revel:AddCallback(ModCallbacks.MC_ENTITY_TAKE_DMG , function(_,  e, dmg, flag, src, invuln)
    if src.Entity and src.Entity.Type == 3 and src.Entity.Variant == REVEL.ENT.MIRRORSHARD.variant and not e:IsBoss() then
        ...
        revel.mirrorshard.bloodiedShards[src.Entity.InitSeed] = 0
    end
end)
...
--  fam.OrbitSpeed = -fam.OrbitSpeed
    ...
    if revel.mirrorshard.bloodiedShards[fam.InitSeed] == 0 then
        local spr = fam:GetSprite()
        spr:ReplaceSpritesheet(0, "gfx/familiar/revelcommon/mirror_shard_bloody.png")
        spr:LoadGraphics()
        revel.mirrorshard.bloodiedShards[fam.InitSeed] = 1
    end

    for _,e in ipairs(REVEL.roomProjectiles) do
        if not e:GetData().MirrorReflected and e.Position:DistanceSquared(fam.Position) < (e.Size+fam.Size+2) ^ 2 then
            e = e:ToProjectile()
            e:GetData().MirrorReflected = true
            ...
            if revel.mirrorshard.bloodiedShards[fam.InitSeed] then
                ...
                spr:LoadGraphics()
                revel.mirrorshard.bloodiedShards[fam.InitSeed] = nil
            end
        end
    end
end, REVEL.ENT.MIRRORSHARD.variant)
