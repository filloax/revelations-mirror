...
Remember to update both this file and lua/revelcommon/enums/callbacks.lua 
if you add new ones
    ...
    revel:AddCallback(ModCallbacks.MC_POST_ENTITY_REMOVE, function(_, tear)
        if BitAnd(REVEL.GetSplitFlags(tear), TearFlags.TEAR_QUADSPLIT) > 0 then
            RecentlyDiedQuadshotTears[#RecentlyDiedQuadshotTears + 1] = {
                ...
                Player = tear:GetData().__player,
                -- Flags = tear:GetData().TearFlags,
                -- Color = tear.Color,
            }
        ...
        elseif not data.RanFireTear then --tears that are shot from split shots don't trigger FIRE_TEAR
            ...
            if not isCricket then
                ...
                for _, ct in pairs(closeTears) do
                    ...
                    and ct.TearIndex ~= tear.TearIndex and ct:GetData().__player
                    and (
                ...
                player = player or (parentTear and parentTear:GetData().__player)
            end
    ...
    revel:AddCallback(ModCallbacks.MC_POST_ENTITY_REMOVE, function(_, ent)
        ...
        if not data.__manuallyRemoved then
            ...
            if closest then
                ...
                closest:GetData().spawnerParent = ent.SpawnerEntity or ent.Parent
        
                StageAPI.CallCallbacks(RevCallbacks.POST_TEAR_POOF_INIT, false, closest, closest:GetData(), cSpr, ent, ent.SpawnerEntity or ent.Parent)
            elseif REVEL.DEBUG and not ent:GetData().BurningBush then
...
revel:AddCallback(ModCallbacks.MC_POST_ENTITY_REMOVE, function(_, ent)
    ...
    if not data.__manuallyRemoved then
        ...
        if closest then
            ...
            StageAPI.CallCallbacks(RevCallbacks.POST_PROJ_POOF_INIT, false, closest, closest:GetData(), spr, ent, ent.SpawnerEntity or ent.Parent)
...
do
    ...
    local function ppccallbackPostPickupUpdate(_, pickup)
        local data, sprite = pickup:GetData(), pickup:GetSprite()
