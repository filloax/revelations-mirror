...
revel:AddCallback(ModCallbacks.MC_PRE_NPC_COLLISION, function(_, npc, collider)
    if collider.Type == 1 and npc.Type ~= REVEL.ENT.ICE_HAZARD_GAPER.id and REVEL.GetData(collider).TotalFrozen then
        return true
    end
end)

revel:AddCallback(ModCallbacks.MC_PRE_PLAYER_COLLISION, function(_, player, collider)
    if not (collider.Type == 5 or collider.Type == REVEL.ENT.ICE_HAZARD_GAPER.id) and REVEL.GetData(player).TotalFrozen then
        return true
    end
end)
        ...
        if not data.Pitfalling then
            ...
            for _, spike in pairs(stalagSpikes) do
                if spike.EntityCollisionClass ~= EntityCollisionClass.ENTCOLL_NONE and spike.Position:DistanceSquared(player.Position) < (spike.Size + player.Size)^2 then
                    ...
                    REVEL.GetData(spike).DontShootThisTime = true
                end
            end
