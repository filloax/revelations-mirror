...
revel:AddCallback(ModCallbacks.MC_POST_PICKUP_UPDATE, function(_, item)
    ...
    if data.LotteryTicketRerolls and (not data.RerollDelay or data.RerollDelay < REVEL.game:GetFrameCount()) and item.SubType ~= 0 then
        ...
        item:GetData().LotteryTicketRerolls = lotteryRerolls

        if not lotteryTicketPicksRandom then
            item:GetData().LotteryTicketRerollIndex = index
        end
        ...
        if slowed then
            item:GetData().RerollDelay = REVEL.game:GetFrameCount() + lotterySwitchTimerSlow
        elseif sped then
            item:GetData().RerollDelay = REVEL.game:GetFrameCount() + lotterySwitchTimerFast
        else
            item:GetData().RerollDelay = REVEL.game:GetFrameCount() + lotterySwitchTimer
        end
