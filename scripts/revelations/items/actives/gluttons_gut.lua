...
revel:AddCallback(ModCallbacks.MC_USE_ITEM,
    ...
    and not HasBit(useFlags, UseFlag.USE_CARBATTERY) then
        ...
        elseif data.gutState == "CHEW" then
            ...
            player:AddHearts(2)
            REVEL.sfx:Play(SoundEffect.SOUND_VAMP_GULP, 1.0, 0, false, 1.0)
...
local function fireGutTear(player, pos, vel, size, projs)
    ...
    ---@type EntityTear
    local tear = Isaac.Spawn(2, TearVariant.BLOOD, 0, pos, vel, player):ToTear()
    local spr, data = tear:GetSprite(), REVEL.GetData(tear)
    spr:Load("gfx/itemeffects/revelcommon/gluttons_gut_projectile.anm2", true)
    spr:Play("Idle", true)
    ...
    if data.gSize then
        if data.gSize > 0.5 then
            ...
            if data.gSize == 1 then
                ...
                REVEL.sfx:Play(SoundEffect.SOUND_MEATY_DEATHS, 0.7, 0, false, 1.0)
                projAmnt = 8
                dmgMult = 2.5
            end
            for i = 1, projAmnt do
                ...
                t.CollisionDamage = (data.__player and data.__player.Damage or 3.50) * dmgMult
            end
        end
        if data.projs and #data.projs ~= 0 then
            ...
            for i, projData in ipairs(data.projs) do
                local vel = RandomVector() * math.random(8, 11)
                local pos = e.Position + vel:Resized(35)
                ---@type EntityProjectile
                local p = Isaac.Spawn(
                    EntityType.ENTITY_PROJECTILE, projData[1],
                    projData[2], pos, vel, data.__player
                ):ToProjectile()

                p.EntityCollisionClass = EntityCollisionClass.ENTCOLL_ENEMIES
                p:AddProjectileFlags(BitOr(
    ...
    if not edata.gutAbsorbing then
        ...
        if dist < 16000 then
            ...
            if (not e.Height) and e:IsBoss() then -- if is boss, spawn projectile to be absorbed instead
                ...
                local p = e:ToNPC():FireBossProjectiles(1, player.Position, 1, param)
                --        p.SpriteScale = Vector.One * 2
                --        p.SpriteOffset = REVEL.GetYVector(- 32 + e.SpriteOffset.Y)
                local pdata = REVEL.GetData(p)
...
revel:AddCallback(ModCallbacks.MC_POST_UPDATE, function()
    for j, player in ipairs(REVEL.players) do -- why not use PEFFECT_UPDATE? scale is wonky outside of POST_UPDATE
        ...
        if data.gutState then -- and REVEL.ITEM.GUT:PlayerHasCollectible(player)
            ...
            local finish = data.gutFrameCount >= Gut.animDur[data.gutState] -- for brevity, since it's going to be used a lot
            ...
            for i, e in ripairs(data.gutEnms) do
                if e:IsDead() or not e:Exists() then
                    ...
                    local diff = (player.Position + Gut.mouthOffset - e.SpriteOffset) - edata.pos
                    local dist = diff:LengthSquared()

                    if dist > 36 then
                        edata.gutAngleSpeed = REVEL.Clamp(
                            edata.gutAngleSpeed + 3 * edata.gutAngleDir,
                            -25, 25
                        )
                        edata.gutAngle = edata.gutAngle + edata.gutAngleSpeed
                        e:GetSprite().Rotation = edata.gutAngle

                        -- 13 = gaper size, so by default its 0.15 for gaper-sized ents
                        e.SpriteScale = REVEL.Lerp(
                            Vector.One * 0.15, edata.startScale,
                            math.max(dist, edata.prevDist) / edata.startDist
                        )
                        ...
                        if e.Height then
                            ...
                            if edata.gutBossTear then
                                e.Color = Color.Lerp(Color.Default, fullBossTearColor, math.sin(e.FrameCount * 4) / 2 + 1)
                            end
                        end
                    else
                        if edata.gutBossTear or
                            ...
                            e.Position = player.Position + Gut.mouthOffset
                            e:BloodExplode()
                            e:Remove()

                            table.remove(data.gutEnms, i)
                        else
                            data.gutHp = data.gutHp + math.max(e.CollisionDamage, 1.5)
                            table.remove(data.gutEnms, i)
                        ...
                        REVEL.sfx:Play(SoundEffect.SOUND_MEAT_JUMPS, 0.6, 0, false, 1)
                    end
                end
            end
            ...
            elseif data.gutState == "SUCK" then
                for i, e in ipairs(REVEL.roomEnemies) do
                    if not (e:HasEntityFlags(EntityFlag.FLAG_NO_TARGET) or
                        ...
                        e:IsDead()) and not REVEL.GetData(e).absorbed and
                ...
                for i, e in ipairs(REVEL.roomProjectiles) do
                    if e.SpawnerType ~= 1 and not REVEL.GetData(e).absorbed and
                        not HasBit(e.ProjectileFlags, ProjectileFlags.HIT_ENEMIES) then
                        absorbEntity(e, player, data)
                    end
                end

                if #data.gutEnms == 0 
                and (finish or data.gutHp > getMaxGutHp() or data.forceGutEndSuck) 
                then
            ...
            elseif data.gutState == "SUCK_E" and finish then
                if data.gutHp ~= 0 or #data.gutProjs ~= 0 then
                    ...
                    player:SetActiveCharge(player:GetActiveCharge() + REVEL.config:GetCollectible(REVEL.ITEM.GUT.id).MaxCharges)
                else
            ...
            elseif data.gutState == "CHEW" and finish then
                if REVEL.IsShooting(player, false, true) then
                    ...
                    player:SetActiveCharge(player:GetActiveCharge() - REVEL.config:GetCollectible(REVEL.ITEM.GUT.id).MaxCharges)
                    data.gutAction = REVEL.dirToShootAction[player:GetFireDirection()]
                    changeCostume(player, data, "SPIT")
                    local maxHp = getMaxGutHp()

                    REVEL.sfx:Play(SoundEffect.SOUND_BOSS_LITE_GURGLE, 1.05, 0, false, 1.00)
