...
function REVEL.ShootChillSnowball(source, pos, vel, chillTime)
    ...
    REVEL.GetData(proj).FreezeOnHit = chillTime
    REVEL.GetData(proj).IsSnow = true
    REVEL.GetData(proj).Flomp = true
...
local function chill_PreProjectileCollision(_, projectile, collider, low)
    if REVEL.GetData(projectile).FreezeOnHit then
        if collider.Type == EntityType.ENTITY_PLAYER then
            REVEL.ChillFreezePlayer(collider:ToPlayer(), REVEL.GetData(projectile).FreezeOnHit, REVEL.GlacierBalance.DarkIceInChill)
    
            if REVEL.ENT.PRANK_GLACIER:isEnt(projectile.SpawnerEntity) then
                REVEL.GetData(projectile.SpawnerEntity).FrozePlayerFrame = projectile.SpawnerEntity.FrameCount
            end
...
local function chill_wisps_PreFamiliarCollision(_, familiar, collider, low)
    ...
    and REVEL.GetData(collider).FreezeOnHit then
        -- Ignore collision
        return true
    end
end
...
function REVEL.RemoveWarmAura(entity)
    ...
    if auraData then
        ...
        REVEL.GetData(entity).WarmAuraData = nil
    end
end
...
function REVEL.GetWarmAuraData(entity)
    return REVEL.GetData(entity).WarmAuraData
end
...
function REVEL.RemoveChillAura(entity)
    ...
    if auraData then
        ...
        REVEL.GetData(entity).ChillAuraData = nil
    end
end
...
function REVEL.GetChillAuraData(entity)
    return REVEL.GetData(entity).ChillAuraData
end
    ...
    if RoomHasChillAura then
        ...
        for _, entity in ipairs(roomNPCs) do
            ...
            if auraData and auraData.Enabled then
                for _, player in ipairs(REVEL.players) do
                    local ed = REVEL.GetData(entity)
                    local d = REVEL.GetData(player)
