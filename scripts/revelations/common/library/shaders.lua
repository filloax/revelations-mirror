...
revel:AddCallback(ModCallbacks.MC_POST_RENDER, function()
    if Input.IsButtonTriggered(Keyboard.KEY_END, 0) and not (REVEL.GetData(REVEL.player).seqProg and REVEL.GetData(REVEL.player).seqProg > 1) and REVEL.DEBUG and not REVEL.game:IsPaused() then
    ...
    if drawLineStage then
        ...
        elseif drawLineStage == 2 then
            REVEL.LinePreviewShader.Expansion = REVEL.LineDistance(normPos, P1, P2)
        elseif drawLineStage == 3 then
            REVEL.LinePreviewShader.Feather = REVEL.LineDistance(normPos, P1, P2) - REVEL.LinePreviewShader.Expansion
        end
