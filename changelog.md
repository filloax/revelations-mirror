...
Tweaks
- [Repentogon] Sarah: directly change devil chance (affects Found HUD)
    and blacklist goat head, eucharist and Penance
- Changed most SFX to mono as in the base game, which considerably 
    reduced the mod's filesize

Internal
- Yet Another Refactor, move a lot of script files and reorganize them
    (might lead to unforeseen issues, make sure to report!); will make it
    easier to work on the mod
- Rev now has autogenerated library docs! Not included in steam version
    for size
- change almost all usages of :GetData() to rev's version
...
- Fixed forced input (mainly cardboard robot)
- Other divide by 0 errors (slambip, line check)
- Fix sarah damage
- Other minor errors
