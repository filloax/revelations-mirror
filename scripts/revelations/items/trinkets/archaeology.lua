...
StageAPI.AddCallback("Revelations", RevCallbacks.POST_ROCK_BREAK, 1, function(grid)
    ...
    if hasArch and grid.Desc.Type == GridEntityType.GRID_ROCK_ALT and math.random() < 0.3 then
        local drop
        if Archaeology.drop[stage] then
            drop = StageAPI.WeightedRNG(Archaeology.drop[stage], nil, "Weight")
        elseif Archaeology.drop[stage - 1] then
            drop = StageAPI.WeightedRNG(Archaeology.drop[stage - 1], nil, "Weight")
        else
