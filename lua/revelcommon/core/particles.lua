...
do
    ...
    ---@field RemoveOutOfRoom boolean | number
    ...
    function REVEL.ParticleType:Spawn(system, pos, vel, parent, entityRenderer)
        ...
        local spr, data = part:GetSprite(), part:GetData()
        ...
        if entityRenderer then
            ...
            if not entityRenderer:GetData().ParticlesToRender then
                entityRenderer:GetData().ParticlesToRender = {}
            end
            entityRenderer:GetData().ParticlesToRender[GetPtrHash(part)] = EntityPtr(part)
        end
...
local function spawnFloorParticleRenderer()
    ...
    eff:GetData().FloorParticleRenderer = true
...
do
        ...
        if #self.Particles ~= 0 then
            for i = #self.Particles, 1, -1 do --faster than ipairs when it needs to be done a lot of times per frame
                ...
                if not part:Exists() then
                    ...
                    if not alive then
                        local renderer = part:GetData().PartData.RenderedByEnt and part:GetData().PartData.RenderedByEnt.Ref
                        if renderer then
                            renderer:GetData().ParticlesToRender[GetPtrHash(part)] = nil
                        end
                        particlesToRenderOnFloor[GetPtrHash(part)] = nil

                        local shouldRemove = part:GetData().PartData.Type.RemoveOnDeath
    ...
    function REVEL.PartSystem:UpdatePartColor(part, color)
        local pdata = part:GetData().PartData
        pdata.Color = color
    ...
    function REVEL.PartSystem:UpdateParticle(part, i) --return false if the particle is dead
        local pdata, spr = part:GetData().PartData, part:GetSprite()
        ...
        if pdata.Velocity.Z >= 0 and pdata.Position.Z >= self.GroundHeight then --if on ground and not floating away
            if pdata.Type.Bounce == 0 then
                ...
                if not pdata.WasGrounded then
                    part.Visible = false
                    if renderer then
                        pdata.RenderedByEnt:GetData().ParticlesToRender[GetPtrHash(part)] = nil
                    end
        ...
        elseif pdata.WasGrounded then
            particlesToRenderOnFloor[GetPtrHash(part)] = nil
            if renderer then
                renderer:GetData().ParticlesToRender[GetPtrHash(part)] = EntityPtr(part)
            end
        ...
        if normalVelocityUpdate then
            ...
            if pdata.WasGrounded then
                ...
                if pdata.RenderedByEnt then
                    pdata.RenderedByEnt:GetData().ParticlesToRender[GetPtrHash(part)] = EntityPtr(part)
                end
...
do
    REVEL.ENABLE_PARTICLE_TEST = false
...
function REVEL.RemoveParticlesOfType(particleType)
    ...
    for _, effect in ipairs(particles) do
        local pdata = effect:GetData().PartData
...
StageAPI.AddCallback("Revelations", RevCallbacks.POST_INGAME_RELOAD, 1, function()
    for _, entity in pairs(REVEL.roomEntities) do
        if entity:GetData().Emitters then
            for k, emitter in pairs(entity:GetData().Emitters) do
...
revel:AddCallback(ModCallbacks.MC_POST_EFFECT_RENDER, function(_, effect, renderOffset)
    if effect:GetData().FloorParticleRenderer and REVEL.IsRenderPassFloor() then
        local cnt = 0
