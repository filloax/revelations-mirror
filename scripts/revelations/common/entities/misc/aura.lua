...
function REVEL.SpawnAura(radius, position, color, spawner, follow, isFireAura, fire, instantRemove, chillFade, time)
    ...
    local sprite, data = aura:GetSprite(), REVEL.GetData(aura)
...
function REVEL.UpdateAuraRadius(aura, newRadius)
    ...
    REVEL.GetData(aura).Radius = newRadius
...
revel:AddCallback(ModCallbacks.MC_POST_EFFECT_RENDER, function(_, eff, renderOffset)
    local data = REVEL.GetData(eff)
    if data.IsAura then
        if (data.Spawner and not REVEL.GetData(data.Spawner).NoAuraRerender and data.Spawner.Visible and not REVEL.GetData(data.Spawner).NoExtraRenders)
        or (data.ForceSourceSprite and data.SourceSpritePosition) then
            local sourceSprite = data.ForceSourceSprite or REVEL.GetData(data.Spawner).AuraSourceSprite or data.Spawner:GetSprite()
...
function REVEL.HoldAuraProjectile(proj, aura)
    ...
    data.Aura = aura
    data.AuraRadius = REVEL.GetData(aura).Radius
...
revel:AddCallback(ModCallbacks.MC_POST_PROJECTILE_UPDATE, function(_, proj)
    ...
    if data.HeldByAura then
        ...
        if not REVEL.GetData(data.HeldByAura).StartFade then
