...
local function snareProcessing(npc, newRoom)
    ...
    for _, ent in ipairs(Isaac.FindInRadius(npc.Position, 1000, EntityPartition.ENEMY)) do
        ...
        if not check then
            if REVEL.GetData(ent).ragmaParent then
                if REVEL.GetData(ent).ragmaIndex == data.npcIndex then
...
revel:AddCallback(ModCallbacks.MC_NPC_UPDATE, function(_, npc)
    ...
    if data.Buffed and not data.BuffedInit then
        for i=0, 5 do
            sprite:ReplaceSpritesheet(i, "gfx/monsters/revel2/ragma/ragma_buffed.png")
        end
        sprite:LoadGraphics()
    ...
    if data.ragsOut and #data.ragSnared > 0 then
        for _, entPtr in ipairs(data.ragSnared) do
            ...
            if ent then
                local entData = REVEL.GetData(ent)
                if ent:Exists() and ent:IsVisible() and not REVEL.GetData(ent).ragmaIgnore then 
                    ...
                    if not isFriendly and not ent:HasEntityFlags(EntityFlag.FLAG_FRIENDLY) then
                        ...
                        if not entData.ragmaCord[npc.Index] then
                            ...
                            entData.ragmaCord[npc.Index]:GetSprite():Load("gfx/monsters/revel2/ragma/ragma_rag.anm2", true)
                            ...
                            if entData.ragmaParent then
                                ...
                                if data.Buffed then
                                    if not entData.ragmaCord[npc.Index]:GetSprite():IsPlaying("Gut_Buffed") then
                                        ...
                                        for i=0, 1 do
                                            entData.ragmaCord[npc.Index]:GetSprite():ReplaceSpritesheet(i, "gfx/monsters/revel2/ragma/ragma_rag_buffed.png")
                                        end
...
revel:AddCallback(ModCallbacks.MC_POST_NPC_RENDER, function(_, npc)
    if REVEL.GetData(npc).ragmaCord then
        local data = REVEL.GetData(npc)
        if data.ragmaParent and data.ragmaParent.Ref and not data.ragmaWrapIgnore then
            if data.ragmaParent.Ref:Exists() then
                if not data.ragmaWrap then
                    ...
                    data.ragmaWrap:Load("gfx/monsters/revel2/ragma/ragma_wrapping.anm2", true)
                else
                    local size = "_Small"
                    ...
                    if data.ragmaBuffed then
                        if not data.ragmaLightning then
                            ...
                            data.ragmaLightning:Load("gfx/monsters/revel2/ragma/ragma_rag.anm2", true)
                            data.ragmaWrap:ReplaceSpritesheet(0, "gfx/monsters/revel2/ragma/ragma_wrapping_buffed.png")
...
revel:AddCallback(ModCallbacks.MC_POST_EFFECT_RENDER, function(_, eff)
    ...
    if npc then
        ...
        if not data.bottomSprite then
            ...
            data.bottomSprite:Load("gfx/monsters/revel2/ragma/ragma.anm2", true)
...
;
            ...
            if data.Buffed and not REVEL.GetData(eff).BuffedInit and REVEL.GetData(npc).BuffedInit then
                for i=0, 5 do
                    data.bottomSprite:ReplaceSpritesheet(i, "gfx/monsters/revel2/ragma/ragma_buffed.png")
                end
                data.bottomSprite:LoadGraphics()
                REVEL.GetData(eff).BuffedInit = true
            end
        ...
        if not data.ragTransition then
            ...
            data.ragTransition:Load("gfx/monsters/revel2/ragma/ragma_rag.anm2", true)
