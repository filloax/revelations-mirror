...
StageAPI.AddCallback("Revelations", RevCallbacks.POST_ITEM_PICKUP, 2, function(player, _, __, ___, firstTimeObtained)
    if not REVEL.GetData(player).PrescriptionPills then
        REVEL.GetData(player).PrescriptionPills = {}
    end
...
revel:AddCallback(ModCallbacks.MC_USE_PILL, function(_, pillEffect, player, useFlags)
    if REVEL.ITEM.PRESCRIPTION:PlayerHasCollectible(player) then
        ...
        if REVEL.PrescriptionPills.Positive[pillEffect] or REVEL.PrescriptionPills.Negative[pillEffect] then
            REVEL.GetData(player).PrescriptionPills[pillEffect] = pillColor
            if REVEL.PrescriptionPillsOppositeVersion[pillEffect] then
                REVEL.GetData(player).PrescriptionPills[REVEL.PrescriptionPillsOppositeVersion[pillEffect]] = pillColor
            end
        end
    end
end)
