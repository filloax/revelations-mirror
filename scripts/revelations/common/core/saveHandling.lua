...
if not LUA_API_DOCS_PARSING then
    hub2 = require("scripts.hubroom2.init") --hub2 savedata table is seperately stored
end
...
revel:AddPriorityCallback(ModCallbacks.MC_POST_PLAYER_INIT, CallbackPriority.IMPORTANT, function()
...
revel:AddPriorityCallback(ModCallbacks.MC_EVALUATE_CACHE, CallbackPriority.IMPORTANT, function()
    REVEL.LoadModData(false)
end)
StageAPI.AddCallback("Revelations", StageAPICallbacks.PRE_STAGEAPI_LOAD_SAVE, CallbackPriority.IMPORTANT, function()
    REVEL.LoadModData(false)
end)
...
revel:AddPriorityCallback(ModCallbacks.MC_POST_RENDER, CallbackPriority.LATE, function()
    if REVEL.game:IsPaused() and not wasPaused and LoadedData then
        REVEL.SaveModData()
    end
    wasPaused = REVEL.game:IsPaused()
end)
revel:AddPriorityCallback(ModCallbacks.MC_POST_NEW_LEVEL, CallbackPriority.LATE, function()
    REVEL.SaveModData()
end)
revel:AddPriorityCallback(ModCallbacks.MC_PRE_GAME_EXIT, CallbackPriority.LATE, function(_, menuExit)
    if LoadedData then
...
revel:AddPriorityCallback(ModCallbacks.MC_PRE_MOD_UNLOAD, CallbackPriority.LATE, function(_, mod)
...
StageAPI.AddCallback("Revelations", RevCallbacks.EARLY_POST_NEW_ROOM, CallbackPriority.IMPORTANT, function()
