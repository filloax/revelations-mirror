...
REVEL.LoadFunctions[#REVEL.LoadFunctions + 1] = function()
    ...
    function REVEL.AddReflections(alpha, renderMirrorFloor, renderMirrorOverlays)
        if not roomHasReflections then
            ...
            refMan:GetData().alphaMult = alpha
            ...
            if doRender then
                if layerZeroEntities[ent.Type] and layerZeroEntities[ent.Type][ent.Variant] then
                    ...
                    if not data.ReflectRenderLayer then
                        ...
                        sprite:RenderLayer(data.ReflectRenderLayer, Isaac.WorldToRenderPosition(ent.Position) + data.reflectOffset + REVEL.room:GetRenderScrollOffset())
                    end
                end
            end
    ...
    local function playerPreReflection(entity, sprite, offset)
        local player, data = entity:ToPlayer(), entity:GetData()
