...
StageAPI.AddCallback("Revelations", RevCallbacks.POST_STAGEAPI_NEW_ROOM_WRAPPER, 1, function()
    ...
    for _, phil in pairs(phils) do
        REVEL.GetData(phil).Light = REVEL.SpawnLightAtEnt(phil, Color(0, 0.5, 1, 1,conv255ToFloat( 0, 80, 255)), 1, Vector(0, -15))
        REVEL.GetData(phil).LightSize = 1
        REVEL.GetData(phil).LightSizeStart = 1
    end
end)
...
revel:AddCallback(ModCallbacks.MC_POST_FAMILIAR_RENDER, function(_, fam, renderOffset)
    ...
    if REVEL.GetData(player).LastAimDirection and REVEL.GetData(player).LastAimDirection > -1 
    and not REVEL.GetData(player).NoShotYet 
    and (not revel.data.run.dante.IsDante or revel.data.run.dante.IsCombined) then
        local bonus = REVEL.GetData(player).AimBonus or 0
        if bonus > 0 then
            local alpha
            if bonus < 225 then
            ...
            phylacteryGlow.Rotation = REVEL.GetData(player).LastAimDirection * 90 - 270
    ...
    if not phylacteryIncubusSwitching then
        ...
        if not isGivingItem then
            ...
            if changeDir ~= Direction.NO_DIRECTION then
                if changeDir ~= REVEL.GetData(player).IncubusDirection then
...
function REVEL.Dante.GetPartnerSwitchingDirection()
    ...
    if phylacteryIncubusSwitching then
        if type(phylacteryIncubusSwitching) ~= "boolean" then
            ...
            local player = REVEL.player
            local changeDir = REVEL.Dante.GetMovingCharacterDirection(player)
            if changeDir ~= Direction.NO_DIRECTION and changeDir ~= REVEL.GetData(player).IncubusDirection then
