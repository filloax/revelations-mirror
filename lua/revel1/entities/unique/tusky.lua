...
REVEL.AddBrokenCallback(ModCallbacks.MC_PRE_NPC_COLLISION, function(_, npc, ent)
    ...
    and not (ent:GetData().TuskySpawner and GetPtrHash(npc) == GetPtrHash(ent:GetData().TuskySpawner))
...
local function SpawnRider(npc, data, velocity)
    ...
    rider:GetData().TuskySpawner = npc
    ...
    if REVEL.ENT.SNOWBOB:isEnt(data.Rider) then
        rider:GetData().bobskin = data.RiderVariant
    else
        rider:GetData().ForceSpriteVariant = data.RiderVariant
    end
...
revel:AddCallback(ModCallbacks.MC_PRE_NPC_UPDATE, function(_, npc)
    ...
    if not data.Init then
        ...
        if rng:RandomFloat() < data.bal.PinkChance then
            sprite:ReplaceSpritesheet(0, "gfx/monsters/revel1/tusky_pink.png")
            sprite:ReplaceSpritesheet(1, "gfx/monsters/revel1/tusky_pink.png")
            sprite:LoadGraphics()
        end
        ...
        if data.Rider then
            ...
            data.RiderSprite:Load("gfx/monsters/revel1/" .. data.bal.RiderSprites[data.Rider.Type] .. ".anm2", true)
