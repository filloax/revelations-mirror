...
revel.monolith = {}
revel.monolithTimer = 0
revel.monolith.lasers = {} --getData doesn't seem reliable with lasers, so this is a workaround (list with indexes)
revel.monolith.lastId = 1
revel:AddCallback(ModCallbacks.MC_POST_EFFECT_UPDATE, function(_, e1)
    --[[
    if revel.monolithTimer<REVEL.player.MaxFireDelay then
        revel.monolithTimer=revel.monolithTimer+1
        return
    end]]

    local monolithPlayer = e1.Parent:ToPlayer()

    local data = e1:GetData()
            ...
            elseif e2.SpawnerType == EntityType.ENTITY_PLAYER then
                if not data.BurningBush then
                    ...
                    for i=-1,1,2 do
                        ...
                        t:GetData().monolith = t:GetData().monolith or {}
                        t:GetData().monolith[GetPtrHash(e1)] = true
                        t:GetData().monolithCount = data.monolithCount or 0
                        t:GetData().monolithCount = data.monolithCount+1
    ...
    for i,e2 in ipairs(REVEL.roomLasers) do
        --        REVEL.DebugToString({e2.Index, e2:GetData().monolith, e2.SpawnerType})
        local alreadyDuped --check using the ids list, as getData doesn't seem reliable with lasers
        for j,id in ipairs(revel.monolith.lasers) do
            if id == e2.InitSeed then alreadyDuped = true end
        end

        if not e2:GetData().monolith and not alreadyDuped and e2.SpawnerType == 1 then
            local circle = e2:IsCircleLaser()
            if REVEL.CollidesWithLaser(e1.Position, e2, e1.Size / 2) then
                --add id to duped list
                revel.monolith.lasers[revel.monolith.lastId] = e2.InitSeed
                if revel.monolith.lastId < 64 then
                    revel.monolith.lastId = revel.monolith.lastId + 1
                else
                    revel.monolith.lastId = 1
                end
                ...
                if not circle then
                    ...
                    if e2.Variant == 1 or e2.Variant == 3 or e2.Variant == 9 then
                        for i=-1,1,2 do
                            ...
                            t.Color = Color(1,1,1,1,conv255ToFloat(150,0,100))
                            t:GetData().monolith = true
                            t.Parent = e1

                            revel.monolith.lasers[revel.monolith.lastId] = t.InitSeed --this is only really necessary with normal lasers, not circle ones
                            if revel.monolith.lastId < 64 then
                                revel.monolith.lastId = revel.monolith.lastId + 1
                            else
                                revel.monolith.lastId = 1
    ...
    for i,e2 in ipairs(REVEL.roomKnives) do
        if e2.SpawnerType == EntityType.ENTITY_PLAYER then
            if e2:IsFlying() then
                if not e2:GetData().monolith and e1.Position:DistanceSquared(e2.Position) < (15 + e2.Size) ^ 2 then
                    revel.monolithTimer = 0
                    e2.Color = Color(1,1,1,1,conv255ToFloat(150,0,100))
                    e2:GetData().monolith = true
                    for i=-1,1,2 do
