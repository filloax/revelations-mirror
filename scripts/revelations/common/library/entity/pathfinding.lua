...
-- (checkPos: Vector, index: integer, radius: number, ground: boolean?, general: boolean?, ignorePits: boolean?, ignoreFires: boolean?, entity: Entity?): boolean?
-- For internal use
Pathfinding.CHECK_LINE_CHECK_INDEX_CALLBACK = "_REV_CHECK_LINE_CHECK_INDEX"

function Pathfinding.LoadFunction()
...
---@param name string
---@param parent PathMap
---@param tbl PathMapArgs
---@return PathMap
function REVEL.PathMapFromPathMap(name, parent, tbl)
    local useTbl = REVEL.CopyTable(parent)
    REVEL.mixin(useTbl, tbl)
    return REVEL.NewPathMapFromTable(name, useTbl)
end
...
function REVEL.UsePathMap(pathMap, entity, dontUpdateNow)
    ...
    if not data.UsingPathMaps[name] then
        ...
        REVEL.DebugStringMinor(("Entity %d.%d[%d] is using path map %s"):format(entity.Type, entity.Variant, entity.Index, name))
...
function REVEL.StopUsingPathMap(pathMap, entity)
    ...
    if data.UsingPathMaps and data.UsingPathMaps[name] then
        ...
        REVEL.DebugStringMinor(("Entity %d.%d[%d] is no longer using path map %s"):format(entity.Type, entity.Variant, entity.Index, name))
...
function REVEL.GetTargetSetMatchingEntity(entity, sets, data)
    data = data or REVEL.GetData(entity)
...
function REVEL.GetMinimumTargetSetsFromData(dataKey, entityTable)
    return GetMinimumTargetSetsFromMatch(dataKey and function(ent) return REVEL.GetData(ent)[dataKey] end, entityTable)
end
...
function REVEL.SetPathOnMatchingEntsWithData(dataKey, entityTable, map, sets)
    return SetPathOnMatchingEnts(
        dataKey and function(ent) return REVEL.GetData(ent)[dataKey] end, 
...
---Arg info:
---
---* indicesConnectedCheck: optional, a function called for adjacent 
---    indices that checks if they should considered reachable from one 
---    to the other
---@param startIndex integer
---@param targetIndex integer
---@param validCollisions? GridCollisionClass[]
---@param indicesConnectedCheck? fun(index1: integer, index2: integer, validCollisions: GridCollisionClass[]): boolean
---@return table? path
---@return integer? pathLength
function REVEL.GeneratePathAStar(startIndex, targetIndex, validCollisions, indicesConnectedCheck)
    ...
    while #indicesToCheck > 0 do
        ...
        for _, index in ipairs(adjacentIndices) do
            if not processedIndices[index] and gridCollisions[index] == 0
            and (not indicesConnectedCheck or indicesConnectedCheck(index, checkIndex, gridCollisions))
            then
...
---Checks if the line between two positions is free of obstacles, 
---returning position to the first obstacle if not.
---
---Allows to use the `Pathfinding.CHECK_LINE_CHECK_INDEX_CALLBACK` so
---other modules can add check conditions for collision without 
---overcomplicating this function.
---
---Arg info:
---
---* ground: check if the line would be walkable by a non flying entity.
---* general: check if the line would be walkable by a normal entity (fire and wall collisions)
---* ignorePits: treat pits as walkable for ground flag
---* ignoreFires: treat fires as walkable for general flag
---* entity: not required for normal collision, but might be used by checks with the callback
---@param posA Vector
---@param posB Vector
---@param radius number
...
---@return Vector? hitPos
function REVEL.CheckLine(posA, posB, radius, ground, general, ignorePits, ignoreFires, entity)
    ...
    for i = 1, numChecks do
        ...
        for _, index in ipairs(indices) do
            ...
            local callbacksRet = Isaac.RunCallback(
                Pathfinding.CHECK_LINE_CHECK_INDEX_CALLBACK, 
                check, index, radius, ground, general, ignorePits, ignoreFires, entity
            )
            if callbacksRet then
                collides = true
            end
...
function REVEL.FollowPath(entity, speed, path, useDirect, friction, ground, general, ignorePits, ignoreFires, reset)
    ...
    if useDirect then
        ...
        for i = #path, data.PathIndex, -1 do
            local index = path[i]
            if REVEL.CheckLine(entity.Position, REVEL.room:GetGridPosition(index), 20, ground, general, ignorePits, ignoreFires, entity) then
                pathIndex = i
                break
            end
        end
...
function REVEL.GetDirectPath(path, ground, general, ignorePits, ignoreFires, entity)
    if ground == nil then ground = true end
    if general == nil then general = true end
    ...
    for i = 2, #path do
        ...
        local freeToLast = REVEL.CheckLine(lastPos, thisPos, 5, ground, general, ignorePits, ignoreFires, entity)
        if freeToLast then
            lastFree = path[i]
        else
            if lastFree then
                ...
                out[#out+1] = lastFree
                if REVEL.CheckLine(lastFreePos, thisPos, 10, ground, general, ignorePits, ignoreFires, entity) then
                    lastFree = path[i]
                else
                    out[#out+1] = path[i]
                    lastFree = nil
