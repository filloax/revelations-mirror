...
local LineCheckMode     = require("scripts.revelations.common.enums.LineCheckMode")
...
local EntityBlockSprites = {
    ...
    [REVEL.ENT.YELLOW_BLOCK_GAPER] =  "../blockhead_gaper/blockhead_gaper_yellow_2.png",
    [REVEL.ENT.YELLOW_CARDINAL_BLOCK_GAPER] = "../blockhead_gaper/blockhead_gaper_yellow.png",
    [REVEL.ENT.BLOCK_GAPER] = "../blockhead_gaper/blockhead_gaper_2.png",
    [REVEL.ENT.CARDINAL_BLOCK_GAPER] = "../blockhead_gaper/blockhead_gaper.png",
}
    ...
    OnPathUpdate = function(map)
        ...
        for _, sasquatch in ipairs(squatches) do
            for _, set in ipairs(map.TargetMapSets) do
                if REVEL.GetData(sasquatch).PathID == set.ID then
                    REVEL.GetData(sasquatch).Path = REVEL.GetPathToZero(REVEL.room:GetGridIndex(sasquatch.Position), set.Map, nil, map)
                    REVEL.GetData(sasquatch).PathIndex = nil
                end
            end
        end
        ...
        elseif sprite:IsEventTriggered("Grab") then
            ...
            if type(data.Grabbing) == "number" then --grabbing grid, .Grabbing is grid index
                ...
                if grid and REVEL.CanGridBeDestroyed(grid) then
                    ...
                    sprite:ReplaceSpritesheet(2, "gfx/monsters/revel1/sasquatch/" .. sheet)
                    sprite:LoadGraphics()
                end
            elseif data.Grabbing and data.Grabbing:Exists() then
                ...
                if REVEL.ENT.FLURRY_ICE_BLOCK:isEnt(data.Grabbing) or REVEL.ENT.FLURRY_ICE_BLOCK_YELLOW:isEnt(data.Grabbing) then
                    ...
                    if GaperToHead[data.HoldingEntity] then
                        ...
                        REVEL.GetData(data.Grabbing).BeheadNoSpawn = true
            ...
            if data.HeldIceGridFrame or data.HoldingEntIceBlock then
                ...
                elseif data.HoldingEntIceBlock then
                    ...
                    if data.HoldingPissBlock then
                        REVEL.GetData(thrownEnt).IsPiss = true
    ...
    if IsAnimOn(sprite, "Death") then
        if data.HoldingSomething then
            ...
            if data.HeldIceGridFrame or data.HoldingEntIceBlock then
                ...
                elseif data.HoldingEntIceBlock then
                    ...
                    if data.HoldingPissBlock then
                        REVEL.GetData(heldEnt).IsPiss = true
                        data.HoldingPissBlock = nil
                    end
                    data.HoldingEntIceBlock = nil
                end
            ...
            if heldEnt then
                REVEL.GetData(heldEnt).SpawnedOnSasquatchDeath = true
        ...
        if sprite:IsEventTriggered("Slam") and not data.TriggeredSlam then
            ...
            for _, stalactrite in pairs(stalactrites) do
                ...
                REVEL.GetData(stalactrite).State = "JumpDown"
            end
...
local function IceBlockDie(npc)
    ...
    if REVEL.ZPos.GetPosition(npc) < 5 then
        if data.IsPiss then
            ...
            REVEL.GetData(creep).YellowIceblockCreep = true
        else
...
revel:AddCallback(RevCallbacks.POST_ENTITY_ZPOS_LAND, function(_, entity, airMovementData, fromGrid)
    if entity.Type == REVEL.ENT.BLOCKHEAD.id and
            ...
            and REVEL.GetData(entity).SpawnedOnSasquatchDeath then
