...
local KnifeVariant      = require "scripts.revelations.common.enums.KnifeVariant"
...
return function()
    ...
    REVEL.Elites.Ragtime = {
        Music = REVEL.MUSIC.ELITE_RAGTIME.Track,
        ClearMusic = REVEL.MUSIC.TOMB_BOSS_OUTRO
    }
                ...
                if nextAction == "Move" and self.NextPoint then
                    ...
                    for _, dancer in ipairs(self.Dancers) do
                        if not dancer:IsDead() then
                            local data = REVEL.GetData(dancer)
            ...
            HandleIdle = function(self, cuesetsTriggeredThisUpdate, nextAction)
                ...
                if cuesetsTriggeredThisUpdate[self.bossbal.MusicMapSetDefault] then
                    if self.bal.IdleMovesAround then
                        for i, dancer in ipairs(self.Dancers) do
                            if not dancer:IsDead() then
                                local index = self.CurrentIndices[REVEL.GetData(dancer).IndexNum]
                        ...
                        for i, dancer in ipairs(self.Dancers) do
                            if not dancer:IsDead() then
                                ...
                                if math.random() < 0.45 then
                                    DancerHopIndex(dancer, self.CurrentIndices[REVEL.GetData(dancer).IndexNum])
                                end
                            end
                        end
                    end
        ...
        CongaDance = MakeClass({
            ...
            HandleIdle = function(self, cuesetsTriggeredThisUpdate, nextAction)
                ...
                for i, dancer in ipairs(self.Dancers) do
                    if not dancer:IsDead() then
                        -- Hop on place
                        if math.random() < 0.45 then
                            DancerHopIndex(dancer, self.Path[REVEL.GetData(dancer).IndexNum])
                        end
                    end
                end
            end,
            ...
            MoveConga = function(self, shoot)
                local reachedEnd = not self.PathLoops and (
                    (REVEL.GetData(self.Dancers[1]).IndexNum == #self.Path and self.CurrentDirection > 0)
                    or (REVEL.GetData(self.Dancers[1]).IndexNum == 1 and self.CurrentDirection < 0)
                )
                ...
                for i, dancer in ipairs(self.Dancers) do
                    if not dancer:IsDead() then
                        local index = self.Path[REVEL.GetData(dancer).IndexNum]
                        REVEL.UnlockGridIndex(index)
                    end
                end
    ...
    local function LightsOutSpawn(npc, sprite, data, postDeath)
        ...
        if not postDeath then
            ...
            if data.Phase >= 2 then
                ...
                for _, entity in ipairs(rag_dancers) do
                    if REVEL.GetData(entity).IsLeader then
                        extraSpotlightCheck[#extraSpotlightCheck+1] = entity
                    end
                end

                for _, entity in ipairs(extraSpotlightCheck) do
                    if not (REVEL.GetData(entity).RagtimeExtraSpotlight
                    and REVEL.GetData(entity).RagtimeExtraSpotlight.Ref) then
                        ...
                        if npc == entity then
                            REVEL.GetData(spotExtra).ForceWhenEntInvisible = true
                        elseif REVEL.ENT.RAG_DANCER:isEnt(entity) then
                            REVEL.SetSpotlightSizeMult(spotExtra, 0.75)
                        end
                        REVEL.GetData(entity).RagtimeExtraSpotlight = EntityPtr(spotExtra)
                    end
                end
            end
        end
        ...
        local firstPlayerExtraSpotlight = REVEL.GetData(players[1]).RagtimeExtraSpotlight and REVEL.GetData(players[1]).RagtimeExtraSpotlight.Ref

        if firstPlayerExtraSpotlight then
            REVEL.GetData(firstPlayerExtraSpotlight).ForcePosition = players[1].Position
            REVEL.GetData(firstPlayerExtraSpotlight).ForceVelocity = Vector.Zero
            REVEL.GetData(firstPlayerExtraSpotlight).ForceWhenEntInvisible = players[1].Position
        end
    ...
    local function HandleLightsOut(npc, sprite, data, postDeath)
        ...
        elseif data.LightsOutTimer == spawnTime then
            ...
            if data.LightsOutTimer >= spotlightEnlargeTime then
                ...
                if spotlight then
                    REVEL.GetData(spotlight).DarkAroundScale = sizeMult
            ...
            if data.FirstPlayerPos then
                ...
                if data.LightsOutTimer == phaseStartTime then
                    ...
                    local firstPlayerExtraSpotlight = REVEL.GetData(REVEL.player).RagtimeExtraSpotlight and REVEL.GetData(REVEL.player).RagtimeExtraSpotlight.Ref

                    if firstPlayerExtraSpotlight then
                        REVEL.GetData(firstPlayerExtraSpotlight).ForcePosition = nil
                        REVEL.GetData(firstPlayerExtraSpotlight).ForceVelocity = nil
                        REVEL.GetData(firstPlayerExtraSpotlight).ForceWhenEntInvisible = nil
                    end
    ...
    revel:AddCallback(ModCallbacks.MC_NPC_UPDATE, function(_, npc)
        ...
        elseif data.State == States.SHOWS_OVER and not IsAnimOn(sprite, "Death") then
            local spotlight = data.Spotlight and data.Spotlight.Ref
            if spotlight then
                REVEL.GetData(spotlight).ForceWhenEntDead = true
            end
    ...
    revel:AddCallback(ModCallbacks.MC_POST_ENTITY_REMOVE, function(_, entity)
        ...
        if data.LightsOutTimer then
            ...
            local hdata = REVEL.GetData(lightsoutHandler)
    ...
    function SpawnDefaultDancer(dance, index, shooter, leader)
        ...
        local sprite, data = dancer:GetSprite(), REVEL.GetData(dancer)
    ...
    do
        ...
        function IsBeatTriggered()
            if UseCuesets() then
                return REVEL.IsMusicCueTriggeredThisUpdate(nil, REVEL.MUSIC.ELITE_RAGTIME)
            elseif REVEL.music:GetCurrentMusicID() == REVEL.MUSIC.ELITE_RAGTIME.Track then
        ...
        function beatTriggeredPostUpdate()
            if REVEL.music:GetCurrentMusicID() == REVEL.MUSIC.ELITE_RAGTIME.Track then
        ...
        function GetTriggeredBeatSets()
            if UseCuesets() then
                local _, cuesetsTriggeredThisUpdate = REVEL.GetMusicCuesTriggeredThisUpdate(nil, REVEL.MUSIC.ELITE_RAGTIME)
    ...
    StageAPI.AddCallback("Revelations", StageAPICallbacks.POST_SELECT_STAGE_MUSIC, 1, function(stage, musicID, roomType, rng)
        if StageAPI.InTestMode and REVEL.ENT.RAGTIME:countInRoom() > 0 and not REVEL.room:IsClear() then
            return REVEL.MUSIC.ELITE_RAGTIME.Track
        end
    end)   
