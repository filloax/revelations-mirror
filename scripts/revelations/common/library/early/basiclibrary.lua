...
local UserDataToString = {
    Vector = function(v)
        if REVEL.ShortHandVector then
            local x, y = shortenFloatsToString(v.X, v.Y)
...
function REVEL.ToString(v, recursions)
    ...
    if type(v) == "function" and debug then
        local info = debug.getinfo(v)
        v = "fun[" .. tostring(info.name) .. "#" .. tostring(info.short_src) .. "@" .. tostring(info.linedefined) .. "]"
    end
    -- approximate floats
...
---Add a prefix to each string in the table
---@param tbl string[]
---@param prefix string
---@return string[]
---@overload fun(prefix: string, tbl: string[]): string[]
function REVEL.PrefixAll(tbl, prefix)
