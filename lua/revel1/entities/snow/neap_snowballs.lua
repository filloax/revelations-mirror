    ...
    local function YellowSnowPathUpdate(map)
        ...
        for _, yellowSnow in ipairs(yellowSnows) do
            for _, set in ipairs(map.TargetMapSets) do
                if yellowSnow:GetData().PathID == set.ID then
                    yellowSnow:GetData().Path = REVEL.GetPathToZero(REVEL.room:GetGridIndex(yellowSnow.Position), set.Map, nil, map)
                    yellowSnow:GetData().PathIndex = nil
                end
            end
        end
    end
