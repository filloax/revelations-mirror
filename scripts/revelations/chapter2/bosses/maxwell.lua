    ...
    CycleStartCheck = function(data, bal, curCycleSegment, _, _, doors, trapdoors)
        ...
        if curCycleSegment.WaitIdleDoors then
            for _, door in ipairs(doors) do
                if REVEL.GetData(door).State ~= "Idle" then
                    isOkay = false
                end
            end
        end
        ...
        if curCycleSegment.WaitEnemies then
            ...
            if enemyCount ~= 0 then
                ...
                for _, trap in ipairs(trapdoors) do
                    if REVEL.GetData(trap).State ~= "Trap" and REVEL.GetData(trap).State ~= "Trap Switch" and REVEL.GetData(trap).TargetState ~= "Trap" then
                        REVEL.GetData(trap).TargetState = "Trap"
                        REVEL.GetData(trap).NewTrap = bal.EnemyKillingTraps[math.random(1, #bal.EnemyKillingTraps)]
                    end
                end
            end
        end
        ...
        if shouldBeTraps then
            if not data.DoorsTrackPlayer then
                for _, door in ipairs(doors) do
                    if not REVEL.GetData(door).TopDoor and REVEL.GetData(door).State == "Idle" then
            ...
            for _, door in ipairs(doors) do
                if not REVEL.GetData(door).TopDoor then
                    REVEL.GetData(door).TargetState = "IdleNoMove"
                end
            end
        end
        ...
        if curCycleSegment.WaitIdleTraps then
            for _, trap in ipairs(trapdoors) do
                if REVEL.GetData(trap).State ~= "Spring" and REVEL.GetData(trap).State ~= "Spring Triggered" then
                    if not REVEL.GetData(trap).TargetState and not shouldBeTraps then
                        REVEL.GetData(trap).TargetState = "Spring"
                    end
                    isOkay = false
                end
            end
...
local function MaxwellTriggerSpringsFast(npc, data, trapdoors, setSpringCooldown)
    for _, trapdoor in ipairs(trapdoors) do
        if (REVEL.GetData(trapdoor).State == "Spring" or REVEL.GetData(trapdoor).State == "Spring Shaking") and trapdoor.Position:DistanceSquared(npc.Position) < 8 ^ 2 then
            data.Sprung = true
            REVEL.GetData(trapdoor).State = "Spring Triggered"
            REVEL.GetData(trapdoor).SpringCollidingPlayers = true
            if setSpringCooldown then
                REVEL.GetData(trapdoor).SpringCooldown = setSpringCooldown
            end
...
StageAPI.AddCallback("Revelations", "POST_SELECT_BOSS_MUSIC", 1, function(stage, musicID, isCleared, rng)
    if musicID == REVEL.MUSIC.TOMB_BOSS and StageAPI.GetCurrentRoomType() == RevRoomType.TRAP_BOSS_MAXWELL and not isCleared and not triggeredBossMusic then
        return REVEL.MUSIC.WIND
    elseif musicID == REVEL.MUSIC.TOMB_BOSS and StageAPI.GetCurrentRoomType() == RevRoomType.TRAP_BOSS_MAXWELL and REVEL.MaxwellReplacementMusic then
...
StageAPI.AddCallback("Revelations", RevCallbacks.POST_STAGEAPI_NEW_ROOM_WRAPPER, 1, function()
    ...
    then
        ...
        REVEL.GetData(t2).State = "Spring"
        t2:GetSprite():SetFrame("Spring", 0)

        REVEL.GetData(t1).bal = REVEL.GetBossBalance(MaxwellBalance, "Default")
        REVEL.GetData(t2).bal = REVEL.GetBossBalance(MaxwellBalance, "Default")
        REVEL.GetData(t1).LeftTrapdoor = true
        t2:GetSprite().FlipX = true
        REVEL.GetData(t1).Opposite = t2
        REVEL.GetData(t2).Opposite = t1
    end
end)
...
local function prepareRoom(max, maxData)
    ...
    if #trapdoors == 0 and #doors == 0 then
        ...
        REVEL.GetData(trapdoors[1]).State = "Spring"

        trapdoors[2] = REVEL.ENT.MAXWELL_TRAP:spawn( Vector( c.X + maxData.bal.TrapdoorCenterOffset , c.Y), Vector.Zero, nil) --center-right
        REVEL.GetData(trapdoors[2]).State = "Spring Raised"
        trapdoors[2]:GetSprite().FlipX = true

        REVEL.GetData(trapdoors[1]).Opposite = trapdoors[2]
        REVEL.GetData(trapdoors[2]).Opposite = trapdoors[1]
        ...
        REVEL.GetData(doors[1]).Opposite = doors[2]
        REVEL.GetData(doors[2]).Opposite = doors[1]

        REVEL.GetData(trapdoors[1]).CloseDoor = doors[1]
        REVEL.GetData(trapdoors[2]).CloseDoor = doors[2]

        doors[3] = REVEL.ENT.MAXWELL_DOOR:spawn( Vector( c.X, tl.Y), Vector.Zero, nil) --top

        for i, door in ipairs(doors) do
            local data = REVEL.GetData(door)
        ...
        for _, trapdoor in ipairs(trapdoors) do
            local data = REVEL.GetData(trapdoor)
            data.bal = maxData.bal
...
local function shootBounce(npc, data, target, attackType, height, notPlaySound, positionOffset, dir)
  ...
  for i=0, amnt - 1 do
    ...
    proj.FallingSpeed = 0
    REVEL.GetData(proj).maxwBounce = data.bal.FloorBounceCount[attackType] or data.bal.FloorBounceCount.DefaultShot
    if data.bal.WallBounce[attackType] ~= nil then
        if not data.bal.WallBounce[attackType] then
            REVEL.GetData(proj).wallBounced = true
        end
    else
        REVEL.GetData(proj).wallBounced = not data.bal.WallBounce.DefaultShot
    end

    if data.bal.InfiniteWallBounce[attackType] ~= nil then
        REVEL.GetData(proj).infiniteWallBounce = data.bal.InfiniteWallBounce[attackType]
    else
        REVEL.GetData(proj).infiniteWallBounce = data.bal.InfiniteWallBounce.DefaultShot
    end
...
local function PopBubble(npc, data, target, bubbleBlasted)
    ...
    if bubbleBlasted then
        ...
        for i = 1, numProjectiles do
            ...
            proj.FallingSpeed = 0
            REVEL.GetData(proj).maxwBounce = bounceCounts[i]
            REVEL.GetData(proj).infiniteWallBounce = true
        end
    end
...
local function shootCurveFan(npc, spr, data, pos, amnt, height, notClampAngle)
    ...
    for i=0, amnt-1 do
        ...
        REVEL.GetData(proj).maxwBounce = data.bal.FloorBounceCount.CurveFan or data.bal.FloorBounceCount.DoubleHop
        REVEL.GetData(proj).infiniteWallBounce = data.bal.InfiniteWallBounce.CurveFan or data.bal.InfiniteWallBounce.DoubleHop
        REVEL.GetData(proj).wallBounced = true
        REVEL.GetData(proj).noCollTimeout = 5
...
local function shootBigBall(npc, spr, data, target, jumprope, height, notClampAngle)
    ...
    if height then proj.Height = height end
    REVEL.GetData(proj).noCollTimeout = 5
    REVEL.GetData(proj).maxBigBall = 3 --mult for bounce
    REVEL.GetData(proj).maxwBounce = 1
    REVEL.GetData(proj).fallingAccel = 0.45
    REVEL.GetData(proj).wallBounced = true
    REVEL.GetData(proj).maxBigBallJumpRope = jumprope
    proj.GridCollisionClass = EntityGridCollisionClass.GRIDCOLL_WALLS_X
...
local function throwBomb(npc, spr, data, atTrapdoor, offsetDist, spd, y)
    ...
    if atTrapdoor then
        if data.MaxTrapdoor then
            targ = REVEL.GetData(data.MaxTrapdoor).Opposite.Position
        else
            local trapdoors = REVEL.ENT.MAXWELL_TRAP:getInRoom()
...
local function GetTrapdoorOrder(startDoor, trapdoors)
    local isLeftDoor = REVEL.GetData(startDoor).LeftDoor

    local first, second
    for _, trapdoor in ipairs(trapdoors) do
        local isLeftTrapdoor = REVEL.GetData(trapdoor).LeftTrapdoor
...
local function maxwell_NpcUpdate(_, npc)
    ...
    if not data.Init then
        ...
        for _, door in ipairs(REVEL.ENT.MAXWELL_DOOR:getInRoom()) do
            if data.bal.DoorSpritesheet and (not data.bal.SkipTopDoorSheet or not REVEL.GetData(door).TopDoor) then
                for i = 0, 1 do
            ...
            if REVEL.GetData(door).TopDoor then
                topDoor = door
            elseif REVEL.GetData(door).LeftDoor then
                leftDoor = door
            end
        end
        ...
        for _, trapdoor in ipairs(REVEL.ENT.MAXWELL_TRAP:getInRoom()) do
            ...
            if REVEL.GetData(trapdoor).LeftTrapdoor then
                if data.bal.IsCraxwell then
                    REVEL.GetData(trapdoor).BrokenSpring = true
                end
    ...
    for _, enemy in ipairs(REVEL.roomEnemies) do
        REVEL.GetData(enemy).NoRags = true
    ...
    if data.State == "WaitForPlayer" then
        ...
        for _, trapdoor in ipairs(trapdoors) do
            REVEL.GetData(trapdoor).IsTutorial = true
            if REVEL.GetData(trapdoor).State == "Spring Raised" then
                isRaisedSpring = true
            elseif REVEL.GetData(trapdoor).State == "Spring Shaking" then
        ...
        if fightProgress then
            data.WaitTime = nil
            for _, trapdoor in ipairs(trapdoors) do
                REVEL.GetData(trapdoor).IsTutorial = nil
            end
            ...
            if data.WaitTime == data.bal.SpitMessageTime then
                REVEL.GetData(data.LeftDoor).State = "SpitMessage"
    ...
    elseif data.State == "ChampionIntro" then
        if data.IdleTime then
            ...
            if data.IdleTime <= 0 then
                npc.Position = REVEL.GetData(data.LeftTrap).Opposite.Position
        ...
        if sprite:IsEventTriggered("Spawn") then
            ...
            local raisedSpring = REVEL.GetData(data.LeftTrap).Opposite
            raisedSpring:GetSprite():Play("Trap Spring Raised Trigger", true)
            REVEL.GetData(raisedSpring).State = "Spring Triggered"

            data.LeftTrap:GetSprite():Play("Trap Spring Raise", true)
            REVEL.GetData(data.LeftTrap).State = "Spring Raised"
            REVEL.GetData(data.LeftTrap).SpringCollidingPlayers = true
            REVEL.GetData(data.LeftTrap).LowerCooldown = data.bal.CraxwellSeesawLowerCooldown
    ...
    elseif data.State == "IdleChampion" then
        ...
        if REVEL.GetData(REVEL.GetData(data.CurrentTrap).Opposite).JustHitFromRaised then
            ...
            REVEL.GetData(data.CurrentTrap).State = "Spring Triggered"
            REVEL.GetData(data.CurrentTrap).SpringCollidingPlayers = true
            REVEL.GetData(data.CurrentTrap).TargetState = "Trap"
            REVEL.GetData(data.CurrentTrap).NewTrap = data.bal.SeesawTraps[math.random(1, #data.bal.SeesawTraps)]
            REVEL.GetData(REVEL.GetData(data.CurrentTrap).Opposite).TargetState = "Trap"
            REVEL.GetData(REVEL.GetData(data.CurrentTrap).Opposite).NewTrap = data.bal.SeesawTraps[math.random(1, #data.bal.SeesawTraps)]
            TotalCancelHop(npc, data)
                ...
                if IsBelowFinaleThreshold(npc, data) then
                    ...
                    if isAttacking then
                        ...
                        if curCycle.IsFall then
                            for _, trapdoor in ipairs(trapdoors) do
                                REVEL.GetData(trapdoor).TargetState = "Pit"
                            end
    ...
    elseif data.State == "Seesaw" then
        ...
        if data.SeesawSpecial then
            ...
            if data.SeesawSpecial == data.bal.SeesawSpecialBombFrame then
                ...
                local b = Isaac.Spawn(EntityType.ENTITY_BOMBDROP, BombVariant.BOMB_TROLL, 0, REVEL.GetData(data.CurrentTrap).Opposite.Position, Vector.Zero, npc):ToBomb()
                b.SpawnerEntity = npc
                b.SpawnerType = npc.Type
                REVEL.GetData(b).maxw = true
                b:GetSprite():Play("Pulse", true)
            ...
            local oppositeTrap = REVEL.GetData(data.CurrentTrap).Opposite
            if not data.Hopping then
                if sprite:IsPlaying("Champion Seesaw Land") then
                    if sprite:IsEventTriggered("Land") then
                        ...
                        REVEL.GetData(data.CurrentTrap).TriggerTrap = true
                    end
                else
                    ...
                    if data.NumRepeats <= 0 or IsBelowFinaleThreshold(npc, data) then
                        ...
                        for _, trapdoor in ipairs(trapdoors) do
                            REVEL.GetData(trapdoor).TargetState = "Spring"
                        end
                    else
                        data.Dunked = nil
                        data.Hopping = true
                        if data.SwappingTraps then
                            ...
                            data.CurrentTrap = REVEL.GetData(data.CurrentTrap).Opposite
                            data.CurrentTrap:GetSprite():Play("Trap Door Shake", true)
                            REVEL.GetData(data.CurrentTrap).State = "Spring Shaking"

                            oppositeTrap:GetSprite():Play("Trap Spring Raise", true)
                            REVEL.GetData(oppositeTrap).State = "Spring Raised"
                            REVEL.GetData(oppositeTrap).SpringCollidingPlayers = true
                            REVEL.GetData(oppositeTrap).LowerCooldown = data.bal.CraxwellSeesawLowerCooldown
                            REVEL.GetData(oppositeTrap).TargetState = "Trap"
                            REVEL.GetData(oppositeTrap).NewTrap = data.bal.SeesawTraps[math.random(1, #data.bal.SeesawTraps)]
                        else
                            REVEL.GetData(data.CurrentTrap).TargetState = "Spring"
                            REVEL.GetData(oppositeTrap).TargetState = "Trap"
                            REVEL.GetData(oppositeTrap).NewTrap = data.bal.SeesawTraps[math.random(1, #data.bal.SeesawTraps)]
                        end
            ...
            elseif REVEL.GetData(oppositeTrap).JustHitFromRaised then
                data.SeesawSpecial = 0
                sprite:Play("Champion Seesaw Fling", true)
                REVEL.PlaySound(data.bal.Sounds.Flung)
                data.CurrentTrap:GetSprite():Play("Trap Spring Trigger", true)
            ...
            elseif sprite:IsFinished("Champion Hop Bomb") then
                REVEL.GetData(oppositeTrap).TargetState = "Spring"
    ...
    elseif data.State == "TrapSmash" then
        if not data.AttackInit then
            ...
            for _, trapdoor in ipairs(trapdoors) do
                REVEL.GetData(trapdoor).TargetState = "Trap"
                REVEL.GetData(trapdoor).NewTrap = data.bal.TrapSmashTraps[math.random(1, #data.bal.TrapSmashTraps)]
            end
        ...
        if not data.Hopping then
            if data.AttackDuration <= 0 or IsBelowFinaleThreshold(npc, data) then
                ...
                if data.SwappingTraps then
                    REVEL.GetData(data.CurrentTrap).TargetState = "Trap"
                    REVEL.GetData(data.CurrentTrap).NewTrap = data.bal.TrapSmashTraps[math.random(1, #data.bal.TrapSmashTraps)]
                    data.CurrentTrap = REVEL.GetData(data.CurrentTrap).Opposite
                end

                data.SwappingTraps = true
                data.Arcs = {
                    {HopType = REVEL.WeightedRandom(data.bal.CraxwellIdleHops), StartPos = data.CurrentTrap.Position, EndPos = REVEL.GetData(data.CurrentTrap).Opposite.Position}
                }
            end
        end
    ...
    elseif data.State == "BrimstoneBarrier" then
        local doorSprite, doorData = data.TopDoor:GetSprite(), REVEL.GetData(data.TopDoor)
        if not data.AttackInit then
        ...
        local ongoingAttack
        for _, door in ipairs(doors) do
            if REVEL.GetData(door).SpecialTrap then
                ongoingAttack = true
            end
        end
        ...
        if not data.Hopping then
            ...
            elseif (playersOnLeft > (#REVEL.players - playersOnLeft) and REVEL.GetData(data.CurrentTrap).LeftTrapdoor) or (playersOnLeft < (#REVEL.players - playersOnLeft) and not REVEL.GetData(data.CurrentTrap).LeftTrapdoor) then -- switch sides if the player is on the same side
                data.Arcs = {
                    {HopType = "Shoot", StartPos = data.CurrentTrap.Position, EndPos = REVEL.GetData(data.CurrentTrap).Opposite.Position}
                }
                REVEL.GetData(data.CurrentTrap).TargetState = "Spring"
                data.SwappingTraps = true
            else
                if ongoingAttack then
                    ...
                    REVEL.GetData(data.CurrentTrap).TargetState = "Trap"
                    REVEL.GetData(data.CurrentTrap).NewTrap = attack
                    REVEL.GetData(data.CurrentTrap).SpecialTrap = 0
    ...
    elseif data.State == "HopPrep" then
        ...
        if sprite:IsPlaying("Idle") then
            data.IdleTime = data.IdleTime - 1
            if data.IdleTime <= 0 then
                sprite.FlipX = REVEL.GetData(data.StartDoor).LeftDoor
    ...
    elseif data.State == "DoubleHop" then
        if not data.AttackInit then
            if data.bal.IsCraxwell then
                data.Arcs[#data.Arcs + 1] = {HopType = "Shoot", StartPos = data.CurrentTrap.Position, EndPos = REVEL.GetData(data.CurrentTrap).Opposite.Position}
                data.Arcs[#data.Arcs + 1] = {HopType = REVEL.WeightedRandom(data.bal.CraxwellIdleHops), StartPos = REVEL.GetData(data.CurrentTrap).Opposite.Position, EndPos = REVEL.GetData(data.CurrentTrap).Opposite.Position}
            else
                data.Hopping = true
                ...
                data.Arcs = {
                    ...
                    {HopType = "End", StartPos = second.Position, EndPos = REVEL.GetData(data.StartDoor).Opposite.Position}
                }
            end
        ...
        if data.Arcs and #data.Arcs == 1 then -- last arc
            if data.bal.IsCraxwell then
                ...
                local endDoor = REVEL.GetData(data.StartDoor).Opposite
        ...
        if not sprite:IsPlaying("Enter") and not data.Hopping then
            npc.Position = REVEL.GetData(data.StartDoor).Opposite.Position
    ...
    elseif data.State == "BubblePop" then
        if not data.AttackInit then
            ...
            if data.bal.IsCraxwell then
                ...
                data.Arcs[#data.Arcs + 1] = {HopType = "BubblePopAfter", StartPos = bubblePos, EndPos = REVEL.GetData(data.CurrentTrap).Opposite.Position}
            else
                data.Hopping = true
    ...
    elseif data.State == "ThinkingWithPortals" then
        if not data.AttackInit then
            if not data.bal.IsCraxwell then
                ...
                data.EmergeTrapdoor = REVEL.GetData(data.CurrentTrap).Opposite
            end

            data.PortalHops = math.random(data.bal.PortalHops.Min, data.bal.PortalHops.Max)

            REVEL.GetData(REVEL.GetData(data.EmergeTrapdoor).Opposite).BluePortal = true
            REVEL.GetData(data.EmergeTrapdoor).BluePortal = nil
        ...
        if sprite:IsFinished("Hop Start Fall") or sprite:IsFinished("Hop Middle Fall") or sprite:IsFinished("Champion Hop Alt 2 Fall") then
            ...
            for _, trapdoor in ipairs(trapdoors) do
                if REVEL.GetData(trapdoor).BluePortal then
        ...
        if sprite:IsFinished("Hop Portal 1") or sprite:IsFinished("Hop Portal 2") or startPortaling then
            ...
            if data.PortalHops < 0 or IsBelowFinaleThreshold(npc, data) then
                for _, trapdoor in ipairs(trapdoors) do
                    ...
                    REVEL.GetData(trapdoor).TargetState = "Spring"
                    REVEL.GetData(trapdoor).WaitForAnim = anim
                end
                ...
                npc.Position = data.EmergeTrapdoor.Position
                data.EmergeTrapdoor = REVEL.GetData(data.EmergeTrapdoor).Opposite
            end
        end
    elseif data.State == "Elevator" then
        ...
        if sprite:IsFinished("Hop Start Fall") or sprite:IsFinished("Hop Middle Fall") or sprite:IsFinished("Champion Hop Alt 2 Fall") then
            ...
            if not data.NoElevator then
                ...
                if data.bal.IsCraxwell then
                    ...
                    local maxwellTrapdoor = REVEL.GetData(data.CurrentTrap).Opposite
                    maxwellTrapdoor:GetSprite():Play("Trap Door Rise Elevator Maxwell", true)
                    REVEL.GetData(maxwellTrapdoor).State = "Pit Rising"
                    REVEL.GetData(maxwellTrapdoor).Enemy = "Maxwell"
                end

                for _, trapdoor in ipairs(randomEnemyTrapdoors) do
                    local enemy = REVEL.WeightedRandom(data.bal.ElevatorEnemies)
                    REVEL.GetData(trapdoor).Enemy = enemy
                    REVEL.GetData(trapdoor).State = "Pit Rising"
        ...
        if sprite:IsFinished("Invisible") then
            ...
            for _, trapdoor in ipairs(trapdoors) do
                local enemy = REVEL.GetData(trapdoor).Enemy
                if enemy then
                    ...
                    if enemy == "Maxwell" then
                        if trapdoor:GetSprite():IsFinished("Trap Door Rise Elevator Maxwell") then
                            REVEL.GetData(trapdoor).Enemy = nil
                            REVEL.GetData(trapdoor).State = "Spring"
                            data.State = "IdleChampion"
    ...
    elseif data.State == "BulletYell" then
        if not data.AttackInit then
            for _, trapdoor in ipairs(trapdoors) do
                REVEL.GetData(trapdoor).TargetState = "Trap"
                REVEL.GetData(trapdoor).NewTrap = data.bal.BulletYellTraps[math.random(1, #data.bal.BulletYellTraps)]
                REVEL.GetData(trapdoor).SwitchTime = math.random(data.bal.BulletYellTrapSwitchTime.Min, data.bal.BulletYellTrapSwitchTime.Max)
            end
        ...
        for _, trapdoor in ipairs(trapdoors) do
            REVEL.GetData(trapdoor).SwitchTime = REVEL.GetData(trapdoor).SwitchTime - 1
            if REVEL.GetData(trapdoor).SwitchTime <= 0 and trapdoor:GetSprite():GetFrame() == 0 then
                REVEL.GetData(trapdoor).TargetState = "Trap"
                REVEL.GetData(trapdoor).NewTrap = data.bal.BulletYellTraps[math.random(1, #data.bal.BulletYellTraps)]
                REVEL.GetData(trapdoor).SwitchTime = math.random(data.bal.BulletYellTrapSwitchTime.Min, data.bal.BulletYellTrapSwitchTime.Max)
            end
        ...
        if data.AttackDuration <= 0 and (not data.SideAttack or data.SideAttack == "Idle") then
            for _, trapdoor in ipairs(trapdoors) do
                REVEL.GetData(trapdoor).SwitchTime = nil
                REVEL.GetData(trapdoor).TargetState = "Spring"
            end
    ...
    elseif data.State == "BombTheSpot" then
        if not data.AttackInit then
            ...
            for _, trapdoor in ipairs(trapdoors) do
                REVEL.GetData(trapdoor).TriggerBoth = true
            end
        ...
        if data.MaxTrapdoor then
            ...
            if REVEL.GetData(data.MaxTrapdoor).LeftTrapdoor then
                off = data.bal.BombTheSpotOffset.Left
            end

            npc.Position = data.MaxTrapdoor.Position + off
            local tsprite, tdata = data.MaxTrapdoor:GetSprite(), REVEL.GetData(data.MaxTrapdoor)
            ...
            if data.BombCooldown then
                ...
                if data.BombCooldown <= 0 then
                    if data.NumRepeats <= 0 or IsBelowFinaleThreshold(npc, data) then
                        for _, trapdoor in ipairs(trapdoors) do
                            REVEL.GetData(trapdoor).TargetState = "Spring"
                            REVEL.GetData(trapdoor).TriggerBoth = nil
                        end
            ...
            for _, trapdoor in ipairs(trapdoors) do
                if (REVEL.GetData(trapdoor).State ~= "Spring" and REVEL.GetData(trapdoor).State ~= "Trap") or trapdoor:GetSprite():GetFrame() ~= 0 then
                    isBusy = true
                end
            end
            ...
            if not isBusy then
                ...
                if data.PreviousMaxTrapdoor then
                    if math.random(1, data.bal.BombTheSpotJukeChance) == 1 then
                        ...
                        maxTrapdoor = REVEL.GetData(data.PreviousMaxTrapdoor).Opposite
                    end
                ...
                REVEL.GetData(maxTrapdoor).State = "Occupied"
                maxTrapdoor:GetSprite():Play("Trap Switch Bomb", true)

                data.MaxTrapdoor = maxTrapdoor
                data.PreviousMaxTrapdoor = maxTrapdoor

                local otherTrapdoor = REVEL.GetData(maxTrapdoor).Opposite
                REVEL.GetData(otherTrapdoor).TargetState = "Trap"
                REVEL.GetData(otherTrapdoor).NewTrap = data.bal.BombTheSpotTraps[math.random(1, #data.bal.BombTheSpotTraps)]
            end
        end
    elseif data.State == "JumpRope" then
        if not data.AttackInit then
            ...
            for _, door in ipairs(doors) do
                if not REVEL.GetData(door).TopDoor then
                    REVEL.GetData(door).BrimstoneDoor = firstDoorBrimstone
            ...
            local otherDoor = REVEL.find(doors, function(door)
                return not REVEL.GetData(door).BrimstoneDoor
                    and not REVEL.GetData(door).TopDoor
            end)
            ...
            if data.CycleTime then
                ...
                if data.CycleTime >= cycleLength then
                    ...
                    if not data.RequiredJumps then
                        ...
                        for _, door in ipairs(doors) do
                            local doorSprite, doorData = door:GetSprite(), REVEL.GetData(door)
    ...
    elseif data.State == "GrandFinale" then
        ...
        if data.Death and sprite:IsFinished("Hop Start Fall") or sprite:IsFinished("Hop Middle Fall") then
            ...
            REVEL.sfx:Stop(REVEL.SFX.BLOOD_LASER_LOOP)
            REVEL.sfx:Play(REVEL.SFX.BLOOD_LASER_STOP_SHORT, 0.6, 0, false, 1)
            REVEL.GetData(data.DeathTrapdoor).Blender = true
            REVEL.GetData(data.DeathTrapdoor).KillingMaxwell = npc
            REVEL.GetData(data.DeathTrapdoor).Collision = "All"
            data.State = "Death"
            return
        end
            ...
            if data.CycleTime then
                ...
                if data.bal.IsCraxwell then
                    if not data.Hopping and (data.CycleTime >= data.bal.GrandFinaleHopTrigger * cycleLength) then
                        ...
                        if (data.TargetX < data.StartX and REVEL.GetData(data.CurrentTrap).LeftTrapdoor) or (data.TargetX > data.StartX and not REVEL.GetData(data.CurrentTrap).LeftTrapdoor) then
                            data.Dunked = nil
                            data.Hopping = true

                            if data.Death then
                                data.DeathTrapdoor = REVEL.GetData(data.CurrentTrap).Opposite
                                REVEL.GetData(data.DeathTrapdoor).TargetState = "Pit"
                                data.DeathHop = true
                                data.Arcs = {
                                    {HopType = "Middle", StartPos = data.CurrentTrap.Position, EndPos = REVEL.GetData(data.CurrentTrap).Opposite.Position},
                                    "Fall"
                                }
                            else
                                data.Arcs = {
                    ...
                    if not data.HopFinale and (data.CycleTime >= data.bal.GrandFinaleHopTrigger * cycleLength) then
                        local leftDoor
                        for _, door in ipairs(doors) do
                            if REVEL.GetData(door).LeftDoor then
                                leftDoor = door
                            end
                        end
                        ...
                        if doorX > center.X then
                            jumpDoor = REVEL.GetData(leftDoor).Opposite
                        else
                            jumpDoor = leftDoor
                        end
        ...
        if data.HopFinale then
            if data.IdleTime then
                ...
                if data.IdleTime <= 0 then
                    ...
                    if data.Death then
                        REVEL.GetData(second).TargetState = "Pit"
                        data.DeathHop = true
                ...
                for i = 1, #data.bal.GrandFinaleProjectileBounces do
                    ...
                    if data.bal.WallBounce["GrandFinale"] ~= nil then
                        if not data.bal.WallBounce["GrandFinale"] then
                            REVEL.GetData(pro).wallBounced = true
                        end
                    end
                    REVEL.GetData(pro).maxwBounce = bounceCounts[i]
                end
            ...
            if sprite:IsFinished("ShootWhip Start") or sprite:IsPlaying("ShootWhip Right") or sprite:IsPlaying("ShootWhip Left") or sprite:IsPlaying("BreatheIn") then
                ...
                if not sprite:IsPlaying("BreatheIn") or (data.WhipDuration > data.BreatheStart + data.bal.WhipBreatheDuration) then
                    ...
                    if npc.FrameCount % data.bal.WhipProjectileFrequency == 0 then
                        ...
                        p.Scale = p.Scale * 1.1
                        REVEL.GetData(p).maxwBounce = 4
                        REVEL.GetData(p).wallBounced = true
                        REVEL.GetData(p).noCollTimeout = 5
                        REVEL.GetData(p).velMult = 0.99
                        REVEL.GetData(p).velMultTime = 7
    ...
    if data.Dunking then
        ...
        if sprite:IsEventTriggered("Dunk") then
            ...
            for _, dunking in ipairs(data.Dunking) do
                ...
                REVEL.GetData(dunking).gotDunkedOn = true
                REVEL.GetData(dunking).dunker = EntityPtr(npc)
                REVEL.GetData(dunking).maxwellDunking = nil
            end
        end
        ...
        if not finishedArc and hopData.SlamDunk and not data.Dunked then
            ...
            if #dunking > 0 then
                for _, player in ipairs(dunking) do
                    REVEL.GetData(player).maxwellDunking = true
                end
...
revel:AddCallback(ModCallbacks.MC_ENTITY_TAKE_DMG, function(_, npc, amount, flags, source, invuln)
    ...
    if source.Entity then
        if source.Type == EntityType.ENTITY_BOMBDROP then
            for i,b in ipairs(REVEL.roomBombdrops) do
                if GetPtrHash(b) == GetPtrHash(source.Entity) and REVEL.GetData(b).maxw then
                    return false
                end
            end
    ...
    if npc.HitPoints - amount - REVEL.GetDamageBuffer(npc) <= 0 and REVEL.GetData(npc).bal and REVEL.GetData(npc).bal.BlenderDeath then
        if not REVEL.IsAchievementUnlocked("MAX_HORN") and REVEL.GetData(npc).bal.IsCraxwell then
            REVEL.UnlockAchievement("MAX_HORN")
        end

        local deathframe = REVEL.GetData(npc).Death
        if not deathframe then
            deathframe = REVEL.game:GetFrameCount()
            REVEL.GetData(npc).Death = deathframe
        end
...
revel:AddCallback(ModCallbacks.MC_POST_PROJECTILE_UPDATE, function(_, p)
  local data,spr = REVEL.GetData(p),p:GetSprite()
    ...
    if p:IsDead() then
      if not data.maxBigBallJumpRope then
        for i=0, 5 do --0,1,2 left, 3,4,5 right
          ...
          proj2.Scale = 1.5
          REVEL.GetData(proj2).maxwBounce = 4
          REVEL.GetData(proj2).noCollTimeout = 5
          REVEL.GetData(proj2).wallBounced = true
...
local function maxwell_Trapdoor_PostEffectUpdate(_, effect)
    local data, sprite = REVEL.GetData(effect), effect:GetSprite()
        ...
        for _, player in ipairs(REVEL.players) do
            ...
            then --if player is completely inside pit
                ...
                for _, maxwell in ipairs(maxwells) do
                    REVEL.GetData(maxwell).PlayerFellInto = effect
                end
    ...
    elseif data.State == "Trap" then
        ...
        local isPressable = REVEL.GetData(data.CloseDoor).State == "Idle"
        if data.TriggerBoth then
            isPressable = isPressable and REVEL.GetData(REVEL.GetData(data.CloseDoor).Opposite).State == "Idle"
        end
        ...
        if isPressable then
            ...
            if steppedOn or data.TriggerTrap then
                REVEL.PlaySound(data.bal.Sounds.ActivateTrap)
                REVEL.GetData(data.CloseDoor).TargetState = data.Trap
                REVEL.GetData(data.CloseDoor).SpecialTrap = not not data.SpecialTrap
                if data.TriggerBoth then
                    REVEL.GetData(REVEL.GetData(data.CloseDoor).Opposite).TargetState = data.Trap
                end
...
local function tutorialMessageUpdate(e, data, spr)
    if data.Arcs then
        ...
        if not data.Collected then
            local forceCollect
            if REVEL.GetData(data.Trapdoor).State ~= data.TrapdoorState then
                forceCollect = true
            end
...
local function maxwell_Door_NpcUpdate(_, npc)
    ...
    elseif data.State == "SpitMessage" then
        if sprite:IsFinished("Door Hop Start") then
            ...
            REVEL.GetData(msg).Message = data.bal.SpitMessage
            if data.bal.IsCraxwell then
                REVEL.GetData(msg).Trapdoor = first
                REVEL.GetData(msg).Arcs = {
                    {HopType = "MessageStartChampion", StartPos = npc.Position, EndPos = first.Position}
                }
            else
                REVEL.GetData(msg).Trapdoor = second
                REVEL.GetData(msg).Arcs = {
            ...
            REVEL.GetData(msg).TrapdoorState = REVEL.GetData(REVEL.GetData(msg).Trapdoor).State
            REVEL.PlaySound(data.bal.Sounds.DoorShoot)
...
revel:AddCallback(ModCallbacks.MC_POST_NPC_DEATH, function(_, npc)
    if npc.Variant ~= REVEL.ENT.MAXWELL.variant then return end
    REVEL.sfx:Stop(REVEL.SFX.BLOOD_LASER_LOOP)
    if REVEL.GetData(npc).Laser then
        REVEL.sfx:Play(REVEL.SFX.BLOOD_LASER_STOP_SHORT, 0.6, 0, false, 1)
    end
end, REVEL.ENT.MAXWELL.id)
...
StageAPI.AddCallback("Revelations", RevCallbacks.POST_BOULDER_IMPACT, 2, function(boulder, npc, isGrid)
    ...
    if not REVEL.GetData(npc).Hopping then
