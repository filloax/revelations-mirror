...
do
    ...
    function REVEL.SpawnRevivalRag(npc, id, variant, subtype, position)
        ...
        local data, spr = REVEL.GetData(rag), rag:GetSprite()
    ...
    function REVEL.BuffEntity(npc)
        ...
        REVEL.GetData(s).Buffed = true
        REVEL.SpawnPurpleThunder(s)
        return s
    end

    function REVEL.IsEntityBuffable(npc)
        return REVEL.IsTypeVariantInList(npc, REVEL.RAG_FAMILY) and not REVEL.GetData(npc).Buffed and not REVEL.GetData(npc).Anima
    end
