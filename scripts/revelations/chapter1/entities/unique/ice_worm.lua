...
local function iceWorm_NpcUpdate(_, npc)
    ...
    elseif sprite:IsEventTriggered("Vulnerable") then
        ...
        if data.State == "Arise" then
            local playSound = false
            for _, player in ipairs(REVEL.players) do
                if not REVEL.GetData(player).TotalFrozen and player.Position:DistanceSquared(npc.Position) < (20 + player.Size) ^ 2 then
                    REVEL.SpringPlayer(player)
                    playSound = true
                end
            end
...
local function iceWorm_EntityTakeDmg(_, ent, dmg, flag, source, frames)
    if ent.Variant == REVEL.ENT.ICE_WORM.variant and (REVEL.GetData(ent).Invulnerable == true or (REVEL.GetData(ent).InvulnerableWithEntity and REVEL.GetData(ent).InvulnerableWithEntity:Exists())) then
        return false
    end
end
