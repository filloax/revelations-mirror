...
REVEL.SinMusic = {
    [EntityType.ENTITY_ENVY] = REVEL.MUSIC.SIN.ENVY,
    [EntityType.ENTITY_GREED] = REVEL.MUSIC.SIN.GREED,
    [EntityType.ENTITY_GLUTTONY] = REVEL.MUSIC.SIN.GLUTTONY,
    [EntityType.ENTITY_LUST] = REVEL.MUSIC.SIN.LUST,
    [EntityType.ENTITY_PRIDE] = REVEL.MUSIC.SIN.PRIDE,
    [EntityType.ENTITY_SLOTH] = REVEL.MUSIC.SIN.SLOTH,
    [EntityType.ENTITY_WRATH] = REVEL.MUSIC.SIN.WRATH
}
...
revel:AddCallback(ModCallbacks.MC_POST_UPDATE, function()
    ...
    if room and room.PersistentData.IsSinami then
		...
		for _, rag in ipairs(rags) do
			if REVEL.GetData(rag).SpawnID == EntityType.ENTITY_LUST then
				sinLeft = true
				break
			end
		end
        ...
        if not sinLeft then
			...
			if not StoppedMus then
				...
				REVEL.music:Queue(REVEL.MUSIC.WIND)
