...
return function()
    ...
    local function areThereSuperFires()
        ...
        for _, fire in pairs(fires) do
            if REVEL.GetData(fire).SuperFireTime then
                superFiresLeft = true 
                break
            end
        end
        ...
        if not superFiresLeft then
            ...
            for _, ent in pairs(bulletFires) do
                if REVEL.GetData(ent).FreezerburnFire and REVEL.GetData(ent).Super then
                    superFiresLeft = true
                    break
                end
            end
            ...
            if data.JustHitFire then
                ...
                for _, fire in pairs(fires) do
                    if fire.Position:DistanceSquared(npc.Position) <= (fire.Size + npc.Size) ^ 2 then
                        ...
                        npc:TakeDamage(npc.MaxHitPoints * data.bal.DefaultFireDamageHpPercent, 0, EntityRef(REVEL.GetData(fire).Fire), 15)
    ...
    local function shootBulletFire(npc, pos, vel, targetFire, noTarget, friction, shrinking, endVel, endVelTime, super, sizeMult, shutdownAfterSuper, glow)
        ...
        local data, sprite = REVEL.GetData(fireBullet), fireBullet:GetSprite()
        if glow then
        ...
        if super then
            ...
            data.SuperFireDuration = REVEL.GetData(npc).bal.Fireworks.SuperFireDuration
        end
    ...
    local function fireFireAtFire(npc, targFire, time, super, shutdownAfterSuper, superOnFires)
        ...
        REVEL.GetData(fire).SyncSuperFires = superOnFires and REVEL.ENT.LIGHTABLE_FIRE:getInRoom() or getOffFires()
        return fire
    end
    

    local function isHeadless(freezerburn)
        local data = REVEL.GetData(freezerburn)
    ...
    local function spawnFreezerBurnHead(npc, vel, bdata)
        ...
        local data, sprite = REVEL.GetData(head), head:GetSprite()
    ...
    local function FreezerBurnUpdate(npc, data, sprite, target)
        ...
        if not data.State then
            ...
            if data.bal.HeadAlwaysOff then
                ...
                for _, fire in pairs(fireEntities) do
                    if REVEL.GetData(fire).Init then --if fire was initialized before champion was decided
        ...
        if npc.HitPoints < npc.MaxHitPoints * data.bal.FinaleHpPct and not data.Finale and data.bal.FinaleHpPct > 0 then
            ...
            for _, fire in pairs(REVEL.ENT.LIGHTABLE_FIRE:getInRoom()) do
                REVEL.GetData(fire).SuperFireShootTime = data.bal.Finale.SuperFireShootTime --do it now since it will get applied later anyways
            end
        ...
        if data.State == "Chase" then
            ...
            if isHeadless(npc) then
                ...
                doNextAttack = REVEL.GetData(data.Head).State == "Escape" and REVEL.GetData(data.Head).StateFrame > data.AttackCooldown
            else
            ...
            if doNextAttack then
                ...
                if isAttacking then
                    ...
                    if curCycle.HeadAttack then
                        REVEL.GetData(data.Head).State = attack
                    end
        ...
        if data.Finale then
            ...
            for _, fire in pairs(getOnFires()) do
                firstTime = firstTime or REVEL.GetData(fire).SuperFireTime
                if not firstAlign then
                    firstAlign = REVEL.GetData(fire).LastWasAxisAligned
                else
                    REVEL.GetData(fire).LastWasAxisAligned = firstAlign
                end

                if REVEL.GetData(fire).SuperFireTime == 1 then
                    REVEL.GetData(fire).SuperFireTime = data.bal.Finale.SuperFireShootTime * 2 + 1
                elseif REVEL.GetData(fire).SuperFireTime ~= firstTime then
                    REVEL.GetData(fire).SuperFireTime = firstTime
                end
            end
        end
    end
        ...
        elseif data.State == "HeadShoot" then
            ...
            if sprite:IsEventTriggered("Fire") then
                ...
                for i = 1, num do
                    ...
                    REVEL.DelayFunction(data.bal.HeadShoot.TimeBeforeShotShrink, function() REVEL.GetData(fire).Shrinking = true end)
                end
                REVEL.sfx:NpcPlay(npc, SoundEffect.SOUND_FIRE_RUSH, 1, 0, false, 1.05)
            elseif sprite:IsFinished("Fire") then
            ...
            elseif sprite:IsPlaying("Idle") then
                if data.bal.HeadAlwaysOff then
                    ...
                    if npc.Position:DistanceSquared(data.Body.Position) <= (data.Body.Size + npc.Size) ^ 2 then
                        REVEL.GetData(data.Body).Head = nil
                        npc:Remove()
                    end
                end
            end
	...
	revel:AddCallback(ModCallbacks.MC_POST_NPC_RENDER, function(_, npc)
		...
		if not data.Dying and npc:HasMortalDamage() then
            ...
            if isHeadless(npc) then
                REVEL.GetData(data.Head).State = "Death"
    ...
    revel:AddCallback(ModCallbacks.MC_ENTITY_TAKE_DMG, function(_, ent, dmg, flag, source, cool)
        ...
        if source and source.Type == 33 then
            ...
            if srcEnt and REVEL.GetData(srcEnt).LightableFireFire then
                if not (data.bal.ChaseImmuneToFires and data.State == "Chase") then
                    data.JustHitFire = REVEL.GetData(srcEnt).LightableFireFire
    ...
    revel:AddCallback(ModCallbacks.MC_POST_UPDATE, function()
        if #pendingFires == numFiresToActivate then
            for _, fireData in pairs(pendingFires) do
                ...
                if REVEL.Glacier.IsLightableFireOn(fire) then
                    local toPlay = "SuperFlickeringStart" .. ((REVEL.GetData(fire).Type == 1) and "" or REVEL.GetData(fire).Type)
                    REVEL.GetData(fire).FloorEffect:GetSprite():Play(toPlay, true)
                    fire:GetSprite():Play(toPlay, true)
                    REVEL.GetData(fire).ShutdownAfterSuper = fireData.ShutdownAfter
                    REVEL.GetData(fire).SuperFireCooldown = fireData.Duration
                else
                    REVEL.Glacier.ActivateLightableFire(fire, nil, nil, fireData.Duration, fireData.ShutdownAfter)
                end
                REVEL.GetData(fire).SuperFireTime = fireData.ShootTime
            end
