...
function REVEL.SandCastleParticleCheck(_, eff)
    if not REVEL.GetData(eff).SandCastleRegistered and StageAPI.IsCustomGrid(REVEL.room:GetGridIndex(eff.Position), REVEL.GRIDENT.SAND_CASTLE.Name) then
	...
	REVEL.GetData(eff).SandCastleRegistered = true
end
