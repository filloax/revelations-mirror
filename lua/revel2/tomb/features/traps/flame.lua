...
REVEL.TrapTypes.FlameTrap = {
    ...
    OnTrigger = function(tile, data)
        local fdata = data.FlameTrap:GetData()
        ...
        if data.FlameTrapCooldown > stopHurtingFrame then
            ...
            if not data.NoHitEnemies then
                for _, enemy in ipairs(REVEL.roomNPCs) do
                    if enemy.EntityCollisionClass ~= EntityCollisionClass.ENTCOLL_NONE and enemy.Position:DistanceSquared(pro.Position) < (pro.Size + enemy.Size) ^ 2 and enemy:IsVulnerableEnemy() and enemy.HitPoints >= 0 and not enemy:IsDead() and not enemy:HasEntityFlags(EntityFlag.FLAG_NO_TARGET) and not enemy:GetData().FlameTrapPassThrough then
