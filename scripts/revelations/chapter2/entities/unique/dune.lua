...
revel:AddCallback(ModCallbacks.MC_NPC_UPDATE, function(_, npc)
    ...
    if REVEL.GetData(target).OnDuneTile then
        if #REVEL.players > 1 then
            for _, player in ipairs(REVEL.players) do
                if not REVEL.GetData(player).OnDuneTile then
                    target = player
                end
            end
        end
    ...
    elseif d.state == "Idle" then
        ...
        if not sprite:IsPlaying("Respawn") and not REVEL.GetData(target).OnDuneTile then
    ...
    elseif d.state == "Chase" then
        ...
        if not d.OnDuneTile and not REVEL.GetData(target).OnDuneTile then
            d.subTime = 40
        end
