...
local function isSpinFree(fam, frags)
    ...
    for i, v in ipairs(frags) do
        ...
        and (REVEL.GetData(v).State == "Spin" or REVEL.GetData(v).State == "FloatToSpin") then
            count = count + 1
...
local function StartSpin(fam, spr, data)
    ...
    if next then
        ...
        data.spinCount = REVEL.GetData(next).spinCount
    end
end
        ...
        if REVEL.IsShooting(fam.Player) then
            ...
            if data.spinCount > 0 and data.spinId == 1 then
                ...
                for i = 2, #spinFrags do
                    REVEL.GetData(spinFrags[i]).spinCount = 0
...
revel:AddCallback(ModCallbacks.MC_POST_UPDATE, function()
    ...
    for i, fam in ipairs(stuckFrags) do
        ...
        for j, e in ipairs(REVEL.roomEnemies) do
            ...
            and REVEL.CollidesWithLine(e, prev.Position, fam.Position, enmDetectWidth) then
                enmCrossing = true
                break
            end
        end
