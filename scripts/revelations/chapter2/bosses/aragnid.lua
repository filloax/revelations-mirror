...
local aragnidBalance = {
        ...
        if bal.Champion == "Ruthless" then
            ...
            if curCycleSegment.CheckSlam then
                ...
                for _, rag in ipairs(rags) do
                    if REVEL.GetData(rag).SpawnID == REVEL.ENT.RAG_FATTY.id and REVEL.GetData(rag).SpawnVariant == REVEL.ENT.RAG_FATTY.variant then
    ...
    GetInnardRags = function()
        ...
        for _, rag in ipairs(rags) do
            if REVEL.GetData(rag).SpawnID == REVEL.ENT.ARAGNID_INNARD.id and REVEL.GetData(rag).SpawnVariant == REVEL.ENT.ARAGNID_INNARD.variant then
...
local function GetJumpableInnards()
    ...
    for _, innard in ipairs(aragnidInnards) do
        if not REVEL.GetData(innard).AgitatedFrame then
...
local function KillNearbyInnards(npc)
    ...
    for _, innard in ipairs(innards) do
        if innard.Position:DistanceSquared(npc.Position) < (npc.Size + innard.Size) ^  2 then
            local bal = REVEL.GetData(innard).bal
...
local function SpawnInnard(npc, data, pos, vel, height, fallSpeed, fallAccel)
    ...
    local idata = REVEL.GetData(innard)
    idata.Aragnid = npc
...
local function GetRagEnemiesCount(getRagCount, rags)
    ...
    if getRagCount then
        ...
        for _, rag in ipairs(rags) do
            local isInnard = REVEL.GetData(rag).SpawnID == REVEL.ENT.ARAGNID_INNARD.id and REVEL.GetData(rag).SpawnVariant == REVEL.ENT.ARAGNID_INNARD.variant
            if not isInnard and REVEL.GetData(rag).MagicBallTargeted then
                ragCount = ragCount + 1

                local spawnID, spawnVariant = REVEL.GetData(rag).SpawnID, REVEL.GetData(rag).SpawnVariant
...
local function GetMagicBallTargetRag(targetPlayer, avoidPosition, bal)
    ...
    for _, rag in ipairs(rags) do
        ...
        if (isInnard or not minIsInnard) and (not avoidPosition or avoidPosition:DistanceSquared(rag.Position) > 20 ^ 2) and not REVEL.GetData(rag).MagicBallTargeted and (isInnard or ragEnemyCount < bal.MaxActiveRagEnemies) then
...
local function ShootMagicBall(npc, data, addHeight, position, bloodRain, offset)
    ...
    if bloodRain then
        ...
        REVEL.GetData(proj).NoSplash = true
        REVEL.GetData(proj).NoHome = true
        REVEL.GetData(proj).BloodRain = true
    ...
    REVEL.GetData(proj).MagicSplash = true
    REVEL.GetData(proj).FallingSpeedMax = data.bal.AragnidFunkyFallingSpeedMax
    REVEL.GetData(proj).Player = targetPlayer

    if targetRag then
        REVEL.GetData(targetRag).MagicBallTargeted = proj
    end

    REVEL.GetData(proj).Rag = targetRag
    REVEL.GetData(proj).bal = data.bal
    proj:Update()
    proj.ProjectileFlags = 0
    REVEL.GetData(proj).PurpleColor = proj.Color
    --proj.Color = visible
end
...
revel:AddCallback(ModCallbacks.MC_NPC_UPDATE, function(_, npc)
    ...
    if not data.Init then
        ...
        REVEL.GetData(data.Rags).Aragnid = npc
        REVEL.GetData(data.Rags).bal = data.bal
        REVEL.GetData(data.Rags).Anchor = data.RoomCenter + data.bal.RagAnchorOffset
    ...
    if shouldHitProjectiles then
        ...
        for _, projectile in ipairs(projectiles) do
            ...
            local pdata = REVEL.GetData(projectile)
            ...
            and (
                ...
                if pdata.Innard then
                    local idata = REVEL.GetData(pdata.Innard)
    ...
    for _, innard in ipairs(innards) do
        ...
        if isprite:IsPlaying("SpewLoop") or isprite:IsPlaying("SpewStart") or isprite:IsFinished("SpewStart") or isprite:IsFinished("SpewLoop") or REVEL.GetData(innard).GoingToStart then
            activeInnards[#activeInnards + 1] = innard
        elseif isprite:IsPlaying("Hide") then
            local data = REVEL.GetData(innard)
    ...
    if #activeInnards < 2 and #inactiveInnards + #veryInactiveInnards > 0 then
        for i = 1, 2 - #activeInnards do
            ...
            if asprite:IsPlaying("SpewEnd") then
                REVEL.GetData(activate).GoingToStart = true
            else
    ...
    elseif data.State == "Swing Frenzy Idle" then
        ...
        if data.InnardBarrage then
            ...
            if #innards > 1 and data.InnardRageCooldown <= 0 then
                ...
                for _, innard in ipairs(innards) do
                    local isprite = innard:GetSprite()
                    if (isprite:IsPlaying("SpewStart") or isprite:IsPlaying("SpewLoop") or isprite:IsPlaying("Hide")) and not REVEL.GetData(innard).AgitatedFrame then
                ...
                if #validInnards > 0 then
                    ...
                    if activate then
                        ...
                        REVEL.GetData(activate).AgitatedFrame = REVEL.game:GetFrameCount() + data.InnardFuse
                        REVEL.GetData(activate).StartMoveFrame = REVEL.game:GetFrameCount()
                    end
                end
    ...
    elseif data.State == "Rupturing Scream" then
        if sprite:IsEventTriggered("Shoot") then
            ...
            for _, innard in ipairs(innards) do
                ...
                if (isprite:IsPlaying("SpewStart") or isprite:IsPlaying("SpewLoop") or isprite:IsPlaying("Hide")) and not REVEL.GetData(innard).AgitatedFrame then
                    validInnards[#validInnards + 1] = innard
                    if REVEL.GetData(innard).Buffed then
    ...
    elseif data.State == "Swinging Slam" then
        ...
        if sprite:IsEventTriggered("Land") then
            ...
            REVEL.GetData(rag).SpecialAppear = {NoCreep = true, NoJump = true}
            data.GroundPoundFrames = 0
        end
    ...
    if npc:IsDead() then
        for i, innard in ipairs(innards) do
            REVEL.GetData(innard).SelfAgitate = i * 30
        end
    end
    ...
    for _, aragnid in ipairs(aragnids) do
        ...
        if sprite:IsPlaying("Death") then
            ...
            if sprite:IsEventTriggered("Yell") then
                REVEL.sfx:NpcPlay(npc, REVEL.GetData(aragnid).bal.Sounds.DeathYell, 0.7, 0, false, 1)
            end
...
revel:AddCallback(ModCallbacks.MC_POST_NPC_RENDER, function(_, npc)
    ...
    or REVEL.game:IsPaused() or not REVEL.GetData(npc).Init
...
revel:AddCallback(ModCallbacks.MC_POST_EFFECT_RENDER, function(_, eff)
    ...
    if data.Aragnid and data.Aragnid:Exists() then
        local anim = "Idle"
        if REVEL.GetData(npc).RagsRevived then
            anim = "IdleChampion"
        end
        if REVEL.GetData(data.Aragnid).Falling or REVEL.GetData(data.Aragnid).Swinging or REVEL.GetData(data.Aragnid).GroundPound then
...
revel:AddCallback(ModCallbacks.MC_NPC_UPDATE, function(_, npc)
    ...
    if not data.Init then
        ...
        data.bal = REVEL.GetData(data.Aragnid).bal
        ...
        if not data.AgitatedFrame then
            ...
            if sprite:IsFinished("Explode") then
                if not data.bal.InnardMagicBallReplaceSpider then
                    ...
                    elseif not data.Aragnid or not REVEL.GetData(data.Aragnid).BloodRainTime then
            ...
            elseif not data.Buffed or not data.Aragnid or not REVEL.GetData(data.Aragnid).BloodRainTime then
...
revel:AddCallback(ModCallbacks.MC_ENTITY_TAKE_DMG, function(_, ent, amount, flags, source, cooldown)
    if ent.Variant == REVEL.ENT.ARAGNID_INNARD.variant then
        if not REVEL.GetData(ent).AgitatedFrame then
            if REVEL.GetData(ent).Aragnid then
                local aragnid = REVEL.GetData(ent).Aragnid
                local adata = REVEL.GetData(aragnid)

                if adata.NumInnardsRelayedDamageInFrame and adata.NumInnardsRelayedDamageInFrame < 1 then
                    innardRelayedDamage = true
                    REVEL.GetData(ent).Aragnid:TakeDamage(amount, flags, source, cooldown)
            ...
            if ent.HitPoints - amount - REVEL.GetDamageBuffer(ent) <= 0 then
                REVEL.GetData(ent).AgitatedFrame = -1000
                return false
            end
        else
            REVEL.GetData(ent).AgitatedFrame = -1000
