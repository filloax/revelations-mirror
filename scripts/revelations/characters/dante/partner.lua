...
local function MergeRoomCharacterUpdate(effect, data, sprite, player, pdata)
    ...
    if effect.FrameCount > 4 and not data.HighFiving then
        ...
        REVEL.GetData(effect).TargetIndex = REVEL.room:GetGridIndex(player.Position)
    ...
    if data.Charon and REVEL.IsDanteCharon(data.Charon) then
        ...
        if not data.HighFiving then
            ...
            if data.CharonIncubus then
                ...
                if isStandardShooting and REVEL.GetData(data.CharonIncubus).ShouldShoot then
                ...
                elseif isChargeShooting and REVEL.GetData(data.CharonIncubus).ShouldShoot then
...
revel:AddCallback(ModCallbacks.MC_POST_EFFECT_RENDER, function(_, dante, renderOffset)
    local data, danteSprite = REVEL.GetData(dante), dante:GetSprite()
    if data.CharonIncubus and data.Charon and REVEL.GetData(data.Charon).FrozenSprite then
        local player = data.Charon
        local pdata = REVEL.GetData(player)
        ...
        if pdata.FrozenSprite and not pdata.FrozenSprite:IsFinished("Melt") and player:IsExtraAnimationFinished() then
            local headDir = REVEL.GetData(player).IncubusDirection
            local headDirName
            if headDir == Direction.DOWN then
                headDirName = "Down"
            elseif headDir == Direction.UP then
...
function REVEL.Dante.Callbacks.Partner_PostUpdate(player, data, setIncubusDirection)
    if revel.data.run.dante.IsCombined then
        ...
        if not incubus or not incubus:Exists() then
            ...
            REVEL.GetData(incubus).Charon = player
            ...
            REVEL.GetData(incubus).ShootingCooldown = math.min(player.FireDelay * 2.25, player.MaxFireDelay)
        end
        ...
        if incubus then
            ...
            REVEL.GetData(data.Dante).CharonIncubus = incubus
            REVEL.GetData(data.Dante).Charon = player

            local fdata = REVEL.GetData(incubus)
...
revel:AddCallback(ModCallbacks.MC_POST_LASER_RENDER, function(_, laser)
    ...
    and not REVEL.GetData(laser).CharonIncubusModify 
    and REVEL.GetData(laser.Parent).CharonIncubusAngle 
    and laser:IsCircleLaser() then
        if not REVEL.GetData(laser.Parent).ShouldShoot then
            laser:Remove()
        else
            REVEL.GetData(laser).CharonIncubusModify = true
            local player = REVEL.GetData(laser.Parent).Charon
            ...
            laser.Position = laser.Parent.Position + relativePosition:Rotated(REVEL.GetData(laser.Parent).CharonIncubusChangeAngle)
            laser.Velocity = laser.Velocity:Length() * Vector.FromAngle(REVEL.GetData(laser.Parent).CharonIncubusChangeAngle + 90)
        end
    end
end)

revel:AddCallback(ModCallbacks.MC_POST_LASER_UPDATE, function(_, laser)
    if laser.Parent and laser.Parent.Type == EntityType.ENTITY_FAMILIAR and laser.Parent.Variant == FamiliarVariant.INCUBUS and not REVEL.GetData(laser).CharonIncubusModify and REVEL.GetData(laser.Parent).CharonIncubusAngle and not laser:IsCircleLaser() then
        if not REVEL.GetData(laser.Parent).ShouldShoot then
            laser:Remove()
        else
            REVEL.GetData(laser).CharonIncubusModify = true
            laser.AngleDegrees = REVEL.GetData(laser.Parent).CharonIncubusChangeAngle + laser.AngleDegrees
            laser.FirstUpdate = true
            local posDifference = laser.Position - laser.Parent.Position
            laser.Position = laser.Parent.Position + posDifference:Rotated(REVEL.GetData(laser.Parent).CharonIncubusChangeAngle)
...
revel:AddCallback(ModCallbacks.MC_POST_TEAR_RENDER, function(_, tear)
    ...
    and REVEL.IncludesIncubusTear(tear) and not REVEL.GetData(tear).CharonIncubusModify 
    and not REVEL.GetData(tear).oarTear 
    and REVEL.GetData(tear.Parent).CharonIncubusAngle then
        if not REVEL.GetData(tear.Parent).ShouldShoot then
            tear:Remove()
        else
            REVEL.GetData(tear).CharonIncubusModify = true
            local player = REVEL.GetData(tear.Parent).Charon
            local fireDir = player:GetFireDirection()
            ...
            tear.Position = tear.Parent.Position + relativePosition:Rotated(REVEL.GetData(tear.Parent).CharonIncubusChangeAngle)
            tear.Velocity = tear.Velocity:Rotated(REVEL.GetData(tear.Parent).CharonIncubusChangeAngle)
...
revel:AddCallback(ModCallbacks.MC_POST_BOMB_RENDER, function(_, bomb)
    ...
    and not REVEL.GetData(bomb).CharonIncubusModify 
    and REVEL.GetData(bomb.Parent).CharonIncubusAngle then
        if not REVEL.GetData(bomb.Parent).ShouldShoot then
            bomb:Remove()
        else
            REVEL.GetData(bomb).CharonIncubusModify = true
            local player = REVEL.GetData(bomb.Parent).Charon
            local fireDir = player:GetFireDirection()
            ...
            bomb.Position = bomb.Parent.Position + relativePosition:Rotated(REVEL.GetData(bomb.Parent).CharonIncubusChangeAngle)
            bomb.Velocity = bomb.Velocity:Rotated(REVEL.GetData(bomb.Parent).CharonIncubusChangeAngle)
        end
    end
end)

revel:AddCallback(ModCallbacks.MC_POST_KNIFE_UPDATE, function(_, knife)
    if knife.Parent and knife.Parent.Type == EntityType.ENTITY_FAMILIAR and knife.Parent.Variant == FamiliarVariant.INCUBUS and REVEL.GetData(knife.Parent).CharonIncubusAngle and knife.IsFlying then
        if not REVEL.GetData(knife.Parent).ShouldShoot then
            knife:Reset()
        else
            knife.Rotation = REVEL.GetData(knife.Parent).CharonIncubusAngle
        end
    end
end)
