    ...
    if data.Spinning then
        ...
        for _, bip in pairs(Isaac.FindInRadius(npc.Position, 300, EntityPartition.ENEMY)) do
            if REVEL.SandstormEnts[bip.Type.." "..bip.Variant] or REVEL.SandstormEnts[bip.Type.." "..bip.Variant.." "..bip.SubType] then
                ...
                REVEL.GetData(bip).SandstormPulled = true
            end
        end
        ...
        if npc.FrameCount % 12 == 0 then
            ...
            REVEL.GetData(proj).SandProjectile = true
            REVEL.GetData(proj).IsSandTear = true
            REVEL.GetData(proj).projType = "Sandstorm"
            REVEL.GetData(proj).SandstormSpeed = 0.5
            REVEL.GetData(proj).SandstormSpeedTarget = math.random(2,6) * 0.5
            if sprite.FlipX then
                REVEL.GetData(proj).SandstormRot = -1
            else
                REVEL.GetData(proj).SandstormRot = 1
            end
            REVEL.GetData(proj).SandstormRotTarget = math.random(4,8) * 0.5
...
revel:AddCallback(ModCallbacks.MC_POST_PROJECTILE_UPDATE, function(_, proj)
    ...
    if data.projType == "Sandstorm" then
        if proj.FrameCount > 150 or not (proj.Parent and proj.Parent:Exists() and REVEL.GetData(proj.Parent).Spinning) then
