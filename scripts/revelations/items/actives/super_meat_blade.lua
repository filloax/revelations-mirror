...
revel:AddCallback(RevCallbacks.POST_BASE_PEFFECT_UPDATE, function(_, player)
    ...
    if REVEL.ITEM.SMBLADE:PlayerHasCollectible(player) or REVEL.ITEM.SMBLADE_UNUSED:PlayerHasCollectible(player) then
        ...
        and REVEL.IsShooting(player, true, true) then
            ...
            local bdata = REVEL.GetData(blade)
...
function REVEL.FlingMeatBlades(player) -- no player specified: all of them
    local blades = REVEL.filter(REVEL.ENT.DECORATION:getInRoom(false, false, false), 
        function(deco) 
            return REVEL.GetData(deco).SuperMeatBlade and isOnWall(deco) and (not player or GetPtrHash(REVEL.GetData(deco).Player) == GetPtrHash(player))
        end)

    local flungOne = false

    for _, blade in ipairs(blades) do
        local data = REVEL.GetData(blade)
        data.OnWall = false
...
local function bladeUpdate(entity, data, sprite)
    ...
    for i = 1, #hittable do
        ...
        and target.Position:DistanceSquared(entity.Position) < (target.Size + SMBladeBalance.Radius) ^ 2 then
            ...
            REVEL.GetData(target).HitByBladeAt = target.FrameCount

            if not REVEL.GetData(target).SawBloodFrame or REVEL.GetData(target).SawBloodFrame + 5 < target.FrameCount then
                target:BloodExplode()
                REVEL.GetData(target).SawBloodFrame = target.FrameCount
            end
            --play hit sound
        end
    end
...
revel:AddCallback(ModCallbacks.MC_POST_ENTITY_REMOVE, function(_, entity)
    if REVEL.GetData(entity).HitByBladeAt and REVEL.dist(REVEL.GetData(entity).HitByBladeAt, entity.FrameCount) < 2 then
...
revel:AddCallback(ModCallbacks.MC_POST_EFFECT_RENDER, function(_, entity, renderOffset)
    local sprite, data = entity:GetSprite(), REVEL.GetData(entity)
