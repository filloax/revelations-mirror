...
local function CheckAddMirrorIcon(room, level)
...
-- In case minimapapi acts after stageapi for whatever change in code
local function mirrorRoomMinimapPostMapPosUpdate(room, pos)
    local room = REVEL.room
    local level = REVEL.level
    CheckAddMirrorIcon(room, level)
end
...
if MinimapAPI then
    MinimapAPI:AddPlayerPositionCallback("Revelations", mirrorRoomMinimapPostMapPosUpdate)
end
    ...
    if sprite:IsPlaying("Default Closed") or sprite:IsFinished("Default Closed") then
        for _, e in ipairs(Isaac.FindByType(EntityType.ENTITY_EFFECT, EffectVariant.BOMB_EXPLOSION, -1, false, false)) do
            if e.Position:DistanceSquared(door.Position) < 144 * 144 then
                ...
                REVEL.PlayJingleForRoom(REVEL.MUSIC.MIRROR_DOOR_OPENS)
