...
function REVEL.Chillo.OnFreeze(entity)
    if REVEL.ENT.ICE_WRAITH:isEnt(entity) then
        REVEL.GetData(entity).ChillCooldown = WRAITH_COOLDOWN
    end
end
...
local function chilloWisp_NpcUpdate(_, npc)
    ...
    then
        ...
        if npc.Variant == REVEL.ENT.GRILL_O_WISP.variant then
            ...
            if not data.Init then
                if not data.SpriteChanged and npc.SubType == 1 then
                    local spritesheet = "gfx/monsters/revel1/willowisps/chill_o_wisp_hot_small.png"
        ...
        --CHILL INIT
            ...
            if not data.Init then
                if not data.SpriteChanged and npc.SubType == 1 then
                    local spritesheet = "gfx/monsters/revel1/willowisps/chill_o_wisp_small.png"
                    npc:GetSprite():ReplaceSpritesheet(0, spritesheet)
                    npc:GetSprite():ReplaceSpritesheet(1, spritesheet)
                    npc:GetSprite():LoadGraphics()
                end
        ...
        if npc.State == NpcState.STATE_MOVE then
            ...
            if npc.Variant == REVEL.ENT.GRILL_O_WISP.variant then
                ...
                particleEmitter:EmitParticlesPerSec(REVEL.EmberParticle, REVEL.ParticleSystems.LowGravity, Vec3(npc.Position.X + (math.random() * 2 - 1) * 20, npc.Position.Y, -40), -Vec3(0, 0, -5), 5, 0.5, 60)
                ...
                if not stayStill and not data.ChillCooldown then
                    ...
                    local otherChillos = REVEL.filter(REVEL.ENT.CHILL_O_WISP:getInRoom(false, true, false), function(chillo2)
                        return chillo2.Index ~= npc.Index and REVEL.GetData(chillo2).radius
                    end)

                    for _, chillo2 in ipairs(otherChillos) do
                        if chillo2.Position:DistanceSquared(npc.Position) <= ((data.radius + REVEL.GetData(chillo2).radius) * 0.65) ^ 2 then
                    ...
                    if data.ChillPlayer then
                        for _, player in ipairs(REVEL.players) do
                            if player.Index == data.ChillPlayer then
                                REVEL.GetData(player).ChillAuraTimer = nil
                            end
                        end
                ...
                if not data.ChillCooldown then
                    ...
                    particleEmitter:EmitParticlesPerSec(REVEL.EmberParticleBlue, REVEL.ParticleSystems.LowGravity, Vec3(npc.Position.X + (math.random() * 2 - 1) * 20, npc.Position.Y, -40), -Vec3(0, 0, -5), 5, 0.5, 60)
                end
            end
...
local function chilloWisp_Fireplace_NpcUpdate(_, npc)
    if REVEL.IsChilly() then
        if npc.HitPoints <= 1 
        and not REVEL.GetData(npc).SpawnedGrillo 
        ...
        then
            ...
            REVEL.GetData(npc).SpawnedGrillo = true
        end
    end
end
...
local function iceWraith_PostNewRoom()
    ...
    and REVEL.Glacier.RoomGoodForIceWraith() then
        ...
        if wraithNum > 0 then
            ...
            local body, tail = REVEL.GetData(wraith).WraithBody and REVEL.GetData(wraith).WraithBody.Ref, REVEL.GetData(wraith).WraithTail and REVEL.GetData(wraith).WraithTail.Ref
            if body then
            ...
            if REVEL.GetData(wraith).ChillCooldown then
                REVEL.GetData(wraith).ChillCooldown = 0
            end
        else
