...
local function necragmancer_NpcUpdate(_, npc)
    ...
    if #rags > 0 then
        if sprite:IsPlaying("Wrap") or sprite:IsPlaying("Unwrap") or sprite:IsPlaying("Idle") or sprite:IsPlaying("Appear") or sprite:IsPlaying("Revive") then
            ...
            for _, otherNPC in ipairs(REVEL.roomNPCs) do
                if REVEL.GetData(otherNPC).Necragmancer == data.PtrHash then
    ...
    if sprite:IsEventTriggered("Shoot") then
        ...
        REVEL.GetData(proj).Necragmancer = data.PtrHash
        proj:Update()
...
local function reviveClose(pro, rags, data)
    for _, rag in ipairs(rags) do
        if rag.Position:Distance(pro.Position) < 30 then
            ...
            if npc then
                REVEL.GetData(npc).Necragmancer = data.Necragmancer
            end
        end
    end
end
        ...
        if pro:IsDead() then
            ...
            if not target then
                for _, rag in ipairs(rags) do
                    ...
                    if (not closestDist or dist < closestDist) and (REVEL.GetData(rag).SpawnID == EntityType.ENTITY_LUST or not data.LustBall) then
                        target = rag
                        closestDist = dist
                        targetIsRag = true
                    end
