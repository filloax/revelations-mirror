...
function REVEL.CharonsOarConvertTear(tear, player, knockbackMulti, numExtraTears, buffCount, tearDamagePercent)
   ...
   REVEL.GetData(tear).oarTear = true
   if not buffCount or buffCount == 0 then
    ...
    for i = 1, numExtraTears + 1 do
        ...
        if knockbackMulti then
            REVEL.GetData(newTear).KnockbackMultiplier = knockbackMulti --/ (numCharonExtraTears - 2)
            REVEL.GetData(newTear).CharonPiercingEnemies = {}
            newTear.TearFlags = BitOr(newTear.TearFlags, TearFlags.TEAR_PIERCING)
        end

        REVEL.GetData(newTear).CharonsOarCheck = true
		...
		REVEL.GetData(newTear).oarTear = true
    end
end
...
function REVEL.Dante.CalculateBuff(player)
    if not revel.data.run.dante.IsCombined then
        if REVEL.GetData(player).NoShotYet then
            REVEL.GetData(player).LastAimDirection = player:GetFireDirection()
        end

        if REVEL.GetData(player).LastAimDirection ~= player:GetFireDirection() then
            REVEL.GetData(player).AimBonus = 0
