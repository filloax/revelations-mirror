        ...
        if not REVEL.room:IsPositionInRoom(npc.Position + npc.Velocity * 2, 32) then
            ...
            if data.IsChampion then
                ...
                for i = 1, math.random(25, 35) do
                    ...
                    local pdata = REVEL.GetData(shooter:FireBossProjectiles(1, npc.Position, 8, monsnowFallingProjectile))
                ...
                for i = 1, math.random(19, 27) do
                    local pdata = REVEL.GetData(npc:FireBossProjectiles(1, Vector.Zero, 6, monsnowProjectile))
                    pdata.Flomp = true
                    pdata.ChangeToSnow = true
                    pdata.RandomizeScale = true
                end
    ...
    if spr:IsEventTriggered("Land") then
        ...
        elseif spr:IsPlaying("JumpDown") then
            ...
            for i = 1, math.random(19, 27) do
                ...
                local pdata = REVEL.GetData(npc:FireBossProjectiles(1, Vector.Zero, 8, monsnowFallingProjectile))
                pdata.SpawnIceCreep = true
                pdata.Flomp = true
                pdata.ChangeToSnow = true
                pdata.RandomizeScale = true
