...
revel:AddCallback(ModCallbacks.MC_NPC_UPDATE, function(_, npc)
    ...
    elseif data.State == "Shoot" then
        ...
        if sprite:IsEventTriggered("Shoot") then
            ...
            REVEL.GetData(ent).ShotByHuffpuff = true
            REVEL.GetData(ent).ShotByHuffpuffVel = vel
            ent:ClearEntityFlags(EntityFlag.FLAG_APPEAR)
...
local function huffpuff_Snowball_NpcUpdate(_, npc)
    ...
    or not REVEL.GetData(npc).ShotByHuffpuff 
    then 
        return 
    end

    if REVEL.GetData(npc).ShotByHuffpuffVel then
        npc.Velocity = REVEL.GetData(npc).ShotByHuffpuffVel
    end
    ...
    if npc:CollidesWithGrid() then
        ...
        REVEL.GetData(npc).ShotByHuffpuff = nil
        ...
        if REVEL.ENT.STRAWBERRY_SNOWBALL:isEnt(npc) then
            for i = 1, 4 do
                ...
                REVEL.GetData(p).NoFrostyProjectile = true
