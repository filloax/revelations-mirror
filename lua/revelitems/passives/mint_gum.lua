...
revel.mintgum = {}
revel.mintgum.defFreezeHits = 5 -- hits required to freeze something
revel.mintgum.defCooldownMult = 1 -- cooldown before unfreezing is multiplied by this
revel.mintgum.freezeAgain = 40 -- cooldown before enemy can be frozen again
revel.mintgum.frozenEnts = {}
...
function REVEL.GumHitEnt(e, maxHits, cooldownMult, noParents, noChildren,
    ...
    maxHits = maxHits or revel.mintgum.defFreezeHits
...
local function tryGlacierSkin(npc)
    if math.random() > 0.5 and npc:GetData().RevelReskin ~= "Glacier" then -- and not npc:IsBoss() then --turn to frost variant
        return REVEL.ForceReplacement(npc, "Glacier")
    end
end

function REVEL.GumFreezeEnt(e, spr, data)
    data = data or e:GetData()
    ...
    if data.mintGumConsiderBoss and EnabledInBosses == 1 then -- just reskin
        ...
        REVEL.sfx:Play(REVEL.SFX.MINT_GUM_FREEZE, 1, 0, false, 1)
        table.insert(revel.mintgum.frozenEnts, e)
    end
...
revel:AddCallback(ModCallbacks.MC_ENTITY_TAKE_DMG,
    ...
    then
        ...
        if player and REVEL.ITEM.MINT_GUM:PlayerHasCollectible(player) then
            local freezeHits
            if e:IsBoss() then
                freezeHits = player:GetData().freezeHits + 5
            else
                freezeHits = player:GetData().freezeHits
            end

            REVEL.GumHitEnt(e, freezeHits, player:GetData().mintCooldownMult)
        end
    end
end)
...
revel:AddCallback(ModCallbacks.MC_POST_PROJECTILE_UPDATE,
                    function(_, projectile)
    local data = projectile:GetData()
    if data.MintGumSlowed then
        -- projectile.Velocity = projectile.Velocity * data.MintGumSlowed
        projectile:SetColor(data.MintGumColor, 7, 50, true, true)
    end
end)
...
revel:AddCallback(ModCallbacks.MC_POST_UPDATE, function()
    if #revel.mintgum.frozenEnts == 0 then return end

    -- Countdown on frozen enemies, as they don't call npcUpdates
    local e, data
    for i = #revel.mintgum.frozenEnts, 1, -1 do -- count downwards to allow for better table removing
        e = revel.mintgum.frozenEnts[i]
        if not toRemove[GetPtrHash(e)] and not e:IsDead() and e:Exists() and
            ...
            elseif data.mintGumTimeOut == 0 or not data.mintGumTimeOut then
                ...
                data.mintGumShot = 0
                if data.mintGumConsiderBoss or e:IsBoss() then
                    data.freezeCooldown = revel.mintgum.freezeAgain * 3
                else
                    data.freezeCooldown = revel.mintgum.freezeAgain
                end

                data.MintgumFrozen = false
                e:ClearEntityFlags(EntityFlag.FLAG_FREEZE)
                REVEL.sfx:NpcPlay(e:ToNPC(), REVEL.SFX.MINT_GUM_BREAK, 1, 0, false, 1)
                table.remove(revel.mintgum.frozenEnts, i)
                tryGlacierSkin(e)
            end
        else
            if e:Exists() then
...
revel:AddCallback(ModCallbacks.MC_EVALUATE_CACHE, function(_, player, flag)
    ...
    elseif flag == CacheFlag.CACHE_WEAPON and
        ...
        data.freezeHits = revel.mintgum.defFreezeHits
        data.mintCooldownMult = revel.mintgum.defCooldownMult
