    ...
    elseif npc.State == NpcState.STATE_ATTACK2 then -- spawn innards or shoot spider at innard
        ...
        if sprite:IsEventTriggered("Attack") then
            if #innards == 0 then
                ...
                if not REVEL.GetData(innards[1]).FramesUntilDeath then
                    REVEL.GetData(innards[1]).FramesUntilDeath = 25
                end
            end
        end
        return true
                ...
                for i=1, 3 do
                    for i2=1, 3 do
                        if sprite:IsPlaying("Attack0"..tostring(i2)..animnames[i]) then
                            ...
                            REVEL.GetData(tear).SpawnBlackCreep = true
                            ...
                            if npc.Variant == 1 then
                                ...
                                REVEL.GetData(tear2).SpawnBlackCreep = true
    ...
    if REVEL.STAGE.Tomb:IsStage() then
        if not data.Ragged then
            if e.HitPoints - dmg - REVEL.GetDamageBuffer(e) <= 0 and not data.Buffed then
                ...
                REVEL.GetData(proj).LustBall = true --Code for ball is in entities2, with the rev shrine/ragmancer stuff
                proj:Update()
...
revel:AddCallback(ModCallbacks.MC_POST_UPDATE, function()
    if REVEL.TombEnvyRotatePos then
        ...
        for i,e in ipairs(REVEL.TombEnvys) do
            REVEL.GetData(e).TombEnvyPos = REVEL.TombEnvyRotatePos+Vector.FromAngle(360/#REVEL.TombEnvys*i+REVEL.TombEnvyRotation):Resized(20+#REVEL.TombEnvys*10)
        end
    end
end)
...
local function tombGreed_PreUpdate(_, npc)
    ...
    if data.GreedSummon then
        ...
        if sprite:GetFrame() == 4 then
            for i=1, 2 do
                ...
                REVEL.GetData(ultra_coin).TombGreedSpinningCoin = true
                if npc.Variant == 1 then
                    REVEL.GetData(ultra_coin).TombGreedBombCoin = true
                end
    ...
    for _,player in ipairs(REVEL.players) do
        if REVEL.GetData(player).TombGreedCurrentCoins and REVEL.GetData(player).TombGreedCurrentCoins ~= player:GetNumCoins() then
            data.GreedNumCoins = data.GreedNumCoins+1
        end
        REVEL.GetData(player).TombGreedCurrentCoins = player:GetNumCoins()
    end
end
