...
do -- Mirror Room
    function REVEL.RenderReflectionsTomb(eff, data)
        for i, ent in ipairs(REVEL.roomEffects) do --render effects before, so they don't overlap stuff
            if (ent.Visible or REVEL.GetData(ent).ForceReflection) and ent.Variant ~= StageAPI.E.Door.V and not REVEL.GetData(ent).NoReflect then
        ...
        for i, ent in ipairs(REVEL.roomEntities) do
            if (ent.Visible or REVEL.GetData(ent).ForceReflection) and ent.Type ~= 1000 and ent.Type ~= 1 and not REVEL.GetData(ent).NoReflect then
                if REVEL.GetData(ent).LateReflection then
    ...
    function REVEL.RenderMirrorOverlaysTomb()
        ...
        for _, spike in ipairs(glassSpikes) do
            if spike.Visible and REVEL.GetData(spike).Animation then
                SpikeCrackSprite.Scale = spike.SpriteScale
                SpikeCrackSprite.Color = spike.Color
                SpikeCrackSprite:SetFrame(REVEL.GetData(spike).Animation, spike:GetSprite():GetFrame())
    ...
    StageAPI.AddCallback("Revelations", StageAPICallbacks.POST_SELECT_STAGE_MUSIC, 1, function(stage, musicID, roomType, rng)
        if StageAPI.GetCurrentRoomType() == RevRoomType.MIRROR and REVEL.STAGE.Tomb:IsStage() then
            if not revel.data.run.NarcissusTombDefeated then
                return REVEL.MUSIC.MIRROR_BOSS_2
            else
    ...
    revel:AddCallback(ModCallbacks.MC_POST_RENDER, function()
        ...
        if musicID == REVEL.MUSIC.MIRROR_BOSS_2 and not WasPlaying then
            MusicPlayStartTime = Isaac.GetTime()
            WasPlaying = true
        elseif musicID ~= REVEL.MUSIC.MIRROR_BOSS_2 and WasPlaying then
...
do -- Narcissus 2
    ...
    local function BreakAllSpikes(instant, phasechange, noshards)
        for i, glassSpike in ipairs(Isaac.FindByType(REVEL.ENT.GLASS_SPIKE.id, REVEL.ENT.GLASS_SPIKE.variant, -1, false, false)) do
            if instant or i == 1 then
                REVEL.GetData(glassSpike).Destroyed = true
                REVEL.GetData(glassSpike).NoMegashards = noshards or REVEL.GetData(glassSpike).NoMegashards
                REVEL.GetData(glassSpike).PhaseDestroyed = phasechange or REVEL.GetData(glassSpike).PhaseDestroyed
            else
        ...
        Individual = {
            TinyPlanet = {
                ...
                OnTrigger = function(npc, data)
                    ...
                    REVEL.GetData(planet).TinyPlanet = true
                    REVEL.GetData(planet).SetSize = 13
                end
            },
            GuppysHairball = {
                Gfx = "guppys_hairball",
                ...
                OnTrigger = function(npc, data)
                    ...
                    REVEL.GetData(hairball).GuppysHairball = true
                    REVEL.GetData(hairball).Narcissus = npc
                    REVEL.GetData(hairball).SetSize = 15
                end
            },
            BobbyBombs = {
                Gfx = "bobby_bomb",
                WhenActive = function(npc, data)
                    ...
                    if npc.FrameCount % 30 == 0 then
                        ...
                        REVEL.GetData(bomb).BobbyBomb = true
                        REVEL.GetData(bomb).Invulnerable = true
                        REVEL.GetData(bomb).SetSize = 13
        ...
        Passive = {
            ...
            SamsonsChains = {
                ...
                OnTrigger = function(npc, data, sprite, target)
                    ...
                    REVEL.GetData(chain).bal = data.bal
                    REVEL.GetData(chain).SamsonsChains = true
                    REVEL.GetData(chain).SetSize = 15
                    REVEL.GetData(chain).Target = target
            ...
            CharmOfTheVampire = {
                ...
                PlayerTakeDamage = function(player, amount, flags, source, frames, npc, data)
                    if not data.VampireUses or data.VampireUses < 3 then
                        ...
                        REVEL.GetData(effect).ForceReflection = true
                    end
                end,
                OnEnd = function(npc, data)
            ...
            Shade = {
                ...
                OnTrigger = function(npc, data)
                    ...
                    REVEL.GetData(shade).Shade = true
                    REVEL.GetData(shade).SetSize = 15
        ...
        Modifier = {
            RubberCement = {
                ...
                ShockwaveSpawn = function(shock)
                    ...
                    REVEL.GetData(shock).Color = Color(1.5,1.5,1.5,1,0.2,0.2,0.2)
                end,
                ShockwaveCollide = function(shock)
                    if not REVEL.GetData(shock).BouncePosition then
                        ...
                        REVEL.GetData(shock).BouncePosition = shock.Position
                        return false
                    elseif REVEL.GetData(shock).BouncePosition:DistanceSquared(shock.Position) < 32 ^ 2 then
                        return false
                    end
                end
            },
            SpoonBender = {
                ...
                ShockwaveUpdate = function(shock, initialDir, corner, npc, data, sprite, target)
                    if shock.FrameCount < 60 and not REVEL.GetData(shock).Collided then
                        ...
                        if not REVEL.GetData(shock).Reset then
                            shock.Velocity = (corner.Position - shock.Position):Resized(10)
                            REVEL.GetData(shock).Reset = true
                        end
                    end
                end
            }
            ...
            RoboBaby2 = {
                ...
                OnTrigger = function(npc, data, sprite, target)
                    for i = 1, 2 do
                        ...
                        REVEL.GetData(robo).RoboBaby = true
                        REVEL.GetData(robo).SetSize = 13
                    end
                end
            },
            DeadBird = {
                ...
                WhenActive = function(npc, data, sprite, target)
                    ...
                    if npc.FrameCount % 30 == 0 then
                        ...
                        REVEL.GetData(bird).DeadBird = true
                        ...
                        REVEL.GetData(bird).SetSize = 13
                        REVEL.GetData(bird).FlyOffset = math.random(1, 30)
    ...
    revel:AddCallback(ModCallbacks.MC_NPC_UPDATE, function(_, npc)
        ...
        local data, target = REVEL.GetData(npc), npc:GetPlayerTarget()
        ...
        if data.TinyPlanet then
            ...
            if #data.Projectiles < 5 then
                for i = 1, 5 - #data.Projectiles do
                    ...
                    REVEL.GetData(pro).Angle = math.random(1, 360)
                    REVEL.GetData(pro).OrbitSpeed = math.random(1, 3)
                    REVEL.GetData(pro).TinyPlanet = true
                    pro.Scale = 1.5
            ...
            for i, pro in ripairs(data.Projectiles) do
                if not pro:Exists() then
                    ...
                    pro.Height = -30
                    REVEL.GetData(pro).Angle = REVEL.GetData(pro).Angle + REVEL.GetData(pro).OrbitSpeed
                    pro.Velocity = ((npc.Position + REVEL.GetOrbitOffset(REVEL.GetData(pro).Angle * (math.pi / 180), i * 30)) - pro.Position) / 2
        ...
        elseif data.GuppysHairball then
            ...
            for _, hairball in ipairs(otherHairballs) do
                if GetPtrHash(hairball) ~= GetPtrHash(npc) and REVEL.GetData(hairball).GuppysHairball and npc.Position:DistanceSquared(hairball.Position) < (npc.Size + hairball.Size) ^ 2 then
            ...
            if npc.HitPoints < npc.MaxHitPoints * 0.5 and npc:GetSprite():IsPlaying("Float5") then
                for i = 1, 2 do
                    ...
                    REVEL.GetData(hairball).GuppysHairball = true
                    REVEL.GetData(hairball).Narcissus = data.Narcissus
                    REVEL.GetData(hairball).SetSize = 13
                end
        ...
        elseif data.RoboBaby then
            ...
            for _, baby in ipairs(npcs) do
                if (GetPtrHash(baby) ~= GetPtrHash(npc)) and REVEL.GetData(baby).RoboBaby and REVEL.GetData(baby).Direction and directions[REVEL.GetData(baby).Direction] then
                    directions[REVEL.GetData(baby).Direction] = nil
                end

                if REVEL.GetData(baby).RoboBaby then
    ...
    revel:AddCallback(ModCallbacks.MC_POST_PROJECTILE_UPDATE, function(_, pro)
        if REVEL.GetData(pro).TinyPlanet then
    ...
    revel:AddCallback(ModCallbacks.MC_NPC_UPDATE, function(_, npc)
        ...
        if npc:HasMortalDamage() and not REVEL.GetData(npc).Death then
            REVEL.GetData(npc).Death = true
        end
    end, REVEL.ENT.NARCISSUS_2.id)

    REVEL.AddDeathEventsCallback {
        Type = REVEL.ENT.NARCISSUS_2.id,
        Variant = REVEL.ENT.NARCISSUS_2.variant,
    ...
    revel:AddCallback(ModCallbacks.MC_NPC_UPDATE, function(_, npc)
        ...
        if not data.Init then
            ...
            REVEL.GetData(tutorialSpike).TutorialSpike = true
            tutorialSpike.EntityCollisionClass = EntityCollisionClass.ENTCOLL_NONE
            tutorialSpike.Visible = false
            REVEL.GetData(tutorialSpike).ForceReflection = true
            tutorialSpike:GetSprite():Play("appear", true)
            REVEL.GetData(tutorialSpike).Animation = "appear"
            REVEL.GetData(tutorialSpike).bal = data.bal
                ...
                for cornerNum, corner in ipairs(REVEL.GetCornerPositions()) do
                    ...
                    local shockSpawned = REVEL.SpawnCustomShockwave(npc.Position, diff:Resized(7), "gfx/effects/revel1/mirror_shockwave.png", nil, EntityGridCollisionClass.GRIDCOLL_NONE, nil, nil, nil, SoundEffect.SOUND_ROCK_CRUMBLE, function(shock)
                        ...
                        if not noSpike then
                            ...
                            REVEL.GetData(spike).bal = data.bal
                            REVEL.GetData(spike).Corner = corner.Position
                            REVEL.GetData(spike).MegashardDirection = REVEL.room:GetClampedPosition(corner.Position, 64) - corner.Position
                            REVEL.GetData(spike).HealDelay = cornerNum * (data.bal.GlassSpikeHealFrequency / 4)
                            spike:ClearEntityFlags(EntityFlag.FLAG_APPEAR)
                            spike:GetSprite():Play("appear", true)
                            REVEL.GetData(spike).Animation = "appear"
        ...
        elseif data.State == "DiveIn" then
            ...
            if sprite:IsEventTriggered("Roar") then
                ...
                for i, megashard in ipairs(Isaac.FindByType(REVEL.ENT.MEGASHARD.id, REVEL.ENT.MEGASHARD.variant, -1, false, false)) do
                    REVEL.GetData(megashard).Destroyed = true
                end
            end
        ...
        elseif data.State == "ActAsReflection" then -- intro phase, different anm2. acts as isaac's reflection, then jumps away, and jumps in as narc.
            ...
            if time > actualStartTime + 9000 then
                if data.TutorialSpike and not REVEL.GetData(data.TutorialSpike).Destroyed then
            ...
            elseif time > actualStartTime + 7500 then
                ...
                if time > actualStartTime + 4000 then
                    if time > actualStartTime + 6000 and not data.TutorialBombShot then
                        ...
                        REVEL.GetData(bomb).ForceReflection = true
                        REVEL.GetData(bomb).Narcissus = npc
                        REVEL.GetData(bomb).Height = 25
                        REVEL.GetData(bomb).TutorialBomb = true
        ...
        elseif data.State == "MirrorIdle" then
            ...
            for _, entity in ipairs(Isaac.FindByType(REVEL.ENT.NARCISSUS_2_NPC.id, REVEL.ENT.NARCISSUS_2_NPC.variant, -1, false, false)) do
                if REVEL.GetData(entity).StopDiveOut then
                    diveOut = false
                end
            end
        ...
        elseif data.State == "Idle" then
            ...
            local glassSpikes = Isaac.FindByType(REVEL.ENT.GLASS_SPIKE.id, REVEL.ENT.GLASS_SPIKE.variant, -1, false, false)
            for _, spike in ipairs(glassSpikes) do
                if REVEL.GetData(spike).Destroyed then
                    isSpikeExploding = true
                elseif REVEL.GetData(spike).MaxChainDelay then
            ...
            for _, shard in ipairs(megashards) do
                if REVEL.GetData(shard).MaxChainDelay then
                    isLaserChainActive = true
                    break
                end
            end
            ...
            for _, entity in ipairs(Isaac.FindByType(REVEL.ENT.NARCISSUS_2_NPC.id, REVEL.ENT.NARCISSUS_2_NPC.variant, -1, false, false)) do
                if REVEL.GetData(entity).ConcurrentActive then
            ...
            if not cancel then
                ...
                if not data.PunchPrep and not data.ChangingPhase then
                    ...
                    if changingPhase then
                        ...
                        chest.Visible = false
                        REVEL.GetData(chest).ForceReflection = true
                        REVEL.GetData(chest).IsNarcissusChest = true
                        data.Chest = chest
                    end
        ...
        elseif data.State == "GroundPound" then
            ...
            if sprite:IsEventTriggered("LandGPound") and not data.FirstLand then
                for _, player in ipairs(REVEL.players) do
                    ...
                    REVEL.GetData(player).PoundedByNarcissus = -data.bal.GroundPoundPlayerStun
                end
        ...
        elseif data.State == "LaserChain" then
            ...
            if sprite:IsEventTriggered("Roar") then
                local glassSpikes = Isaac.FindByType(REVEL.ENT.GLASS_SPIKE.id, REVEL.ENT.GLASS_SPIKE.variant, -1, false, false)
                for _, spike in ipairs(glassSpikes) do
                    REVEL.GetData(spike).MaxChainDelay = data.bal.LaserChainIntervals[#glassSpikes]
                end
            end
    ...
    local function isMegashardOpposite(npc, megashard)
        local data, mdata = REVEL.GetData(npc), REVEL.GetData(megashard)
    ...
    local function laserChainUpdate(npc, data, isShard)
        ...
        if not data.SelectedShard then
            ...
            for _, megashard in ipairs(Isaac.FindByType(REVEL.ENT.MEGASHARD.id, REVEL.ENT.MEGASHARD.variant, -1, false, false)) do
                if isMegashardOpposite(npc, megashard) and not REVEL.GetData(megashard).IsLaserTarget then
            ...
            if #validShards > 0 then
                ...
                REVEL.GetData(data.SelectedShard).bal = data.bal
                REVEL.GetData(data.SelectedShard).IsLaserTarget = npc
            else
                if data.Glow then
                    data.Glow.FadeOut = data.bal.DangerousGlowFadeOut
                end
            ...
            if not data.ChainDelay then
                ...
                REVEL.GetData(data.SelectedShard).Glow = {Color = data.bal.DangerousGlowColor, FadeIn = data.bal.DangerousGlowFadeIn, FadeToVisible = true}
            end
            ...
            if data.ChainDelay <= 0 then
                ...
                if data.LaserChainLength > 0 then
                    REVEL.GetData(data.SelectedShard).MaxChainDelay = data.MaxChainDelay
                    REVEL.GetData(data.SelectedShard).LaserChainLength = data.LaserChainLength
                else
                    REVEL.GetData(data.SelectedShard).Glow.FadeOut = data.bal.DangerousGlowFadeOut
                    REVEL.GetData(data.SelectedShard).IsLaserTarget = nil
                    REVEL.GetData(data.SelectedShard).LaserChainLength = nil
                end
    ...
    revel:AddCallback(ModCallbacks.MC_NPC_UPDATE, function(_, npc)
        ...
        if not data.LockPosition and (clamped.X ~= npc.Position.X or clamped.Y ~= npc.Position.Y) then
            ...
            if math.abs(REVEL.GetAngleDifference(diffangle, velangle)) < 46 or data.NoBounce then
                ...
                for _, megashard in ipairs(Isaac.FindByType(REVEL.ENT.MEGASHARD.id, REVEL.ENT.MEGASHARD.variant, -1, false, false)) do
                    if GetPtrHash(megashard) ~= GetPtrHash(npc) and REVEL.GetData(megashard).LockPosition and REVEL.GetData(megashard).LockPosition:DistanceSquared(data.LockPosition) < megashardMinimumDistance ^ 2 then
                        REVEL.GetData(megashard).Destroyed = true
                    end
                end
    ...
    revel:AddCallback(ModCallbacks.MC_POST_ENTITY_REMOVE, function(_, npc)
        if npc.Variant == REVEL.ENT.NARCISSUS_2.variant and REVEL.room:GetFrameCount() > 5 and not REVEL.game:IsPaused() then
            ...
            REVEL.PlayJingleForRoom(REVEL.MUSIC.MIRROR_BOSS_2_OUTRO)
    ...
    revel:AddCallback(RevCallbacks.POST_BASE_PEFFECT_UPDATE, function(_, player)
        if REVEL.GetData(player).PoundedByNarcissus then
    ...
    revel:AddCallback(ModCallbacks.MC_NPC_UPDATE, function(_, npc)
        ...
        elseif data.MaxChainDelay then
            ...
            for _, player in ipairs(REVEL.players) do
                if REVEL.GetData(player).PoundedByNarcissus or player.Velocity:LengthSquared() > 9 ^ 2 then
                    if player.Position:DistanceSquared(npc.Position) < (player.Size + npc.Size) ^ 2 then
                        data.Destroyed = true
                    end
                end
            ...
            if data.HealDelay then
                ...
                if data.Healing then
                    ...
                    if data.Healing >= 0 then
                        ...
                        for _, narcissus in ipairs(Isaac.FindByType(REVEL.ENT.NARCISSUS_2.id, REVEL.ENT.NARCISSUS_2.variant, -1, false, false)) do
                            if narcissus.HitPoints < narcissus.MaxHitPoints and not REVEL.GetData(narcissus).Death and REVEL.GetData(narcissus).PhaseStartingHP then
                                local heal
                                if narcissus.HitPoints < REVEL.GetData(narcissus).PhaseStartingHP then
    ...
    revel:AddCallback(ModCallbacks.MC_ENTITY_TAKE_DMG, function(_, e, amount, flags)
        if e.Variant == REVEL.ENT.GLASS_SPIKE.variant then
            if HasBit(flags, DamageFlag.DAMAGE_EXPLOSION) then
                REVEL.GetData(e).Destroyed = true
                return false
            end
            ...
            if REVEL.GetData(e).Destroyed then
                REVEL.DamageFlash(e)
                return false
            end
        end
    ...
    revel:AddCallback(ModCallbacks.MC_ENTITY_TAKE_DMG, function(_, e, amount, flags)
        if e.Variant == REVEL.ENT.NARCISSUS_2_NPC.variant then
            if REVEL.GetData(e).Invulnerable then
                return false
            end
        end
    end, REVEL.ENT.NARCISSUS_2_NPC.id)
    ...
    if narc2Balance.GlassSpikeScale ~= 1 then
        revel:AddCallback(ModCallbacks.MC_POST_UPDATE, function()
            ...
            for _, spike in ipairs(spikes) do
                spike.SpriteScale = Vector.One * REVEL.GetData(spike).bal.GlassSpikeScale
            end
        end)
    end
    ...
    function REVEL.GetNarc2Chest()
        ...
        for _, eff in ipairs(narcEffects) do
            if REVEL.GetData(eff).IsNarcissusChest then
                return eff
            end
        end
    end
    ...
    function REVEL.RuinNarc2Items()
        for _, narcissus in ipairs(Isaac.FindByType(REVEL.ENT.NARCISSUS_2.id, REVEL.ENT.NARCISSUS_2.variant, -1, false, false)) do
            if REVEL.GetData(narcissus).UsingItemIndices then
                for _, index in ipairs(REVEL.GetData(narcissus).UsingItemIndices) do
                ...
                REVEL.GetData(narcissus).UsingItems = {}
            end
        end
    end
