...
revel:AddCallback(ModCallbacks.MC_POST_PLAYER_RENDER, function(_, player)
    if not REVEL.game:IsPaused() and REVEL.GetData(player).AegisDir and REVEL.IsRenderPassNormal() then
...
revel:AddCallback(RevCallbacks.POST_BASE_PEFFECT_UPDATE, function(_, player)
    ...
    if REVEL.ITEM.AEGIS:PlayerHasCollectible(player) then
        if not data.AegisShield or not data.AegisShield:Exists() then
            ...
            REVEL.GetData(data.AegisShield).IsAegisShield = true
...
revel:AddCallback(ModCallbacks.MC_POST_EFFECT_UPDATE, function(_, eff)
    if REVEL.GetData(eff).IsAegisShield then
