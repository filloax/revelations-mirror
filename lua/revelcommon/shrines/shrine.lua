...
local function SetupShrine(shrine, useShrine, useShrineName, shrineSet, setName, noAddToSeenShrines)
    local shrineData, shrineSprite = shrine:GetData(), shrine:GetSprite()
    ...
    if useShrine then
        ...
        if useShrine.EID_Description then
            ...
            shrineData.EID_Description = eid_Description
        end
...
function REVEL.SpawnShrine(pos, shutdoors, setName, shrineName, noAddToSeenShrines)
    ...
    shrine:GetData().StartShutDoors = shutdoors
    shrine:GetData().ShutDoors = shutdoors
...
local function TriggerMistake()
    ...
    for _, shrine in ipairs(REVEL.ENT.CURSED_SHRINE:getInRoom()) do
        if not shrine:GetData().Activated then
            roomShrines[#roomShrines+1] = shrine:GetData().ShrineType
            REVEL.ActivateShrine(shrine:GetData().ShrineType, false, shrine:ToNPC())
        end
    end
...
function REVEL.ActivateShrine(shrineType, showMessage, npc, doTriggerEvent, customVanity)
    ...
    local rewardValue = (npc and npc:GetData().Value) or shrineData.Value or REVEL.ShrineBalance.DefaultPactVanity
    local oneChapterValue = (npc and npc:GetData().ValueOneChapter) or shrineData.ValueOneChapter or REVEL.ShrineBalance.DefaultPactVanityOneChapter
