...
Virgil.GameStart = function()
    Virgil.rockEnemyBlacklist[REVEL.COMP_ENTS.POTATO_FOR_SCALE.id] = {
        REVEL.COMP_ENTS.POTATO_FOR_SCALE.variant
    }
end

revel:AddCallback(ModCallbacks.MC_POST_GAME_STARTED, Virgil.GameStart)

if Isaac.GetPlayer(0) then Virgil.GameStart() end
...
revel:AddCallback(ModCallbacks.MC_FAMILIAR_INIT, function(_, e)
    REVEL.GetData(e).player = e.Player -- used when it was not a proper familiar, kept in case it stops being one again
    e.EntityCollisionClass = EntityCollisionClass.ENTCOLL_NONE
    e.GridCollisionClass = EntityGridCollisionClass.GRIDCOLL_GROUND
    REVEL.GetData(e.Player).virgil = e
    e:GetSprite():Play("Idle", true)
    --  e:ClearEntityFlags(EntityFlag.FLAG_APPEAR)

    if REVEL.GetData(e).startPos then -- manual spawn bug workaround
        e.Position = REVEL.GetData(e).startPos
    end
        ...
        if data.randomMoveTimer == 0 then
            ...
            local pos = Isaac.GetFreeNearPosition(
                            data.player.Position + RandomVector() *
                                (math.random() *
                                    Virgil.idlePlayerDistanceMax), 20)
            local speedMult = 1
            ...
            if data.player.Position:DistanceSquared(ent.Position) >
                (Virgil.idlePlayerDistanceMax * 1.1) ^ 2 then
...
local function animWalkFrame(vel, spr, data)
    REVEL.AnimateWalkFrameSpeed(spr, vel, Virgil.walkAnims, false,
                                false, "Idle")
end
...
StageAPI.AddCallback("Revelations", RevCallbacks.POST_STAGEAPI_NEW_ROOM_WRAPPER, 1, function()
    Virgil.trocks = getGoodSecretRocks()
    Virgil.thrownRocks = {}

    for i, player in ipairs(REVEL.players) do
        local virgil = REVEL.GetData(player).virgil
        if virgil then
            if REVEL.GetData(virgil).State == "Respawn" then
                stopGoToPos(virgil, REVEL.GetData(virgil))
...
revel:AddCallback(ModCallbacks.MC_FAMILIAR_UPDATE, function(_, ent)
    ...
    elseif data.State == "Throw_Rock" then
        if ent.Position:Distance(data.rockTarget.Position) < 350 then
            ...
            elseif spr:IsFinished("ThrowRockSecret") then
                ...
                data.State = "Move_Default"
            elseif spr:IsEventTriggered("Throw") then
                Virgil.throwRock(ent, data.rockTarget.Position, false,
                                        true)
            end
        end
                    ...
                    elseif spr:IsEventTriggered("Slap") then
                        data.swatBomb.Velocity =
                            (data.swatBomb.Position - data.player.Position):Resized(
                                15)
                        REVEL.GetData(data.swatBomb).swatted = true
                    end
                else
                    data.swatBomb.Velocity =
                        (data.swatBomb.Position - data.player.Position):Resized(
    ...
    elseif data.State == "AngryPointing" then
        ...
        if data.pointingAt.Position:Distance(ent.Position) < 80 and
            ...
            stopGoToEnt(data)
            data.pointingAt:AddConfusion(EntityRef(ent), Virgil
    ...
    elseif data.State == "Throw_Rock_Enemy" then
        ...
        if spr:IsFinished("ThrowRock") then
            ...
            stopGoToEnt(data)
        elseif spr:IsEventTriggered("Throw") then
            data.thrownRockEnemy = Virgil.throwRock(ent,
                                                            data.evadeEnemy
                                                                .Position,
                                                            true, false, 10)
        end
    ...
    if data.State == "Run_Away" then
        ...
        if data.evadeEnemy:Exists() and not data.evadeEnemy:IsDead() then
            ...
            if REVEL.GetData(data.evadeEnemy).pissedOff then
                path = data.Path
                speed = data.speed
            ...
            if data.FrameCount > 250 then
                ...
                REVEL.GetData(data.evadeEnemy).pissedOff = false
                data.evadeEnemy = nil
                data.FrameCount = 0
                if data.dummy then
                    data.dummy:Remove()
                ...
                data.rockCooldown = Virgil.enemyRockCooldown.Min +
                                        math.random(
                                            Virgil.enemyRockCooldown
                                                .Max -
                                                Virgil
    ...
    if not REVEL.includes(Virgil.busyStates, data.State) then -- action select
        Virgil.chooseState(ent, spr, data)
    end
...
function Virgil.chooseState(ent, spr, data)
    ...
    if #REVEL.roomBombdrops ~= 0 then
        ...
        for i, v in ipairs(REVEL.roomBombdrops) do
            if (v.Variant == BombVariant.BOMB_TROLL or v.Variant ==
                ...
                not REVEL.GetData(v).swatted then
    ...
    if (REVEL.room:IsClear() or data.player:GetHearts() +
        data.player:GetSoulHearts() < 3) and #Virgil.trocks ~= 0 and
        data.tRockCooldown == 0 then
        Virgil.trocks = getGoodSecretRocks() -- update rock in case some got destroyed
        if #Virgil.trocks ~= 0 then -- if there are still some
            local i = math.random(#Virgil.trocks)
            data.rockTarget = Virgil.trocks[i]
    ...
    if data.State ~= "Point_Secret" and Virgil.stageToRoomType[stage] and
        REVEL.room:IsClear() and
        not StageAPI.InNewStage() then
        local rng = REVEL.RNG()
        rng:SetSeed(math.random(10000), 0)
        local targetId = REVEL.level:QueryRoomTypeIndex(Virgil
    ...
    elseif data.State ~= "Point_Secret" and
        (StageAPI.InNewStage() or not Virgil.stageToRoomType[stage]) and
        ...
        if bossId ~= REVEL.level:GetCurrentRoomIndex() then
            ...
            if door then doorPos = door.Position end
        elseif Virgil.revelStageToDoorName[StageAPI.GetCurrentStageDisplayName()] and
            #customDoors ~= 0 then
            local door

            for i, v in ipairs(customDoors) do
                if v.PersistentData.DoorDataName ==
                    Virgil.revelStageToDoorName[StageAPI.GetCurrentStageDisplayName()] then
                    door = v
                    break
                end
            end
...
revel:AddCallback(ModCallbacks.MC_POST_NEW_LEVEL, function()
    ...
    if math.random() < Virgil.companionChance and REVEL.IsRevelStage() and
...
function Virgil.throwRock(ent, pos, hit, tinted, speedMult)
    ...
    REVEL.GetData(rock).rockSpr = rockSpr
    ...
    if not hit then
        ...
        REVEL.GetData(rock).targetPos = pos
        REVEL.GetData(rock).g = g
        REVEL.GetData(rock).speed0 = rock.FallingSpeed
    else
        rock.FallingSpeed = -5
        rock.CollisionDamage = 1
    end
    table.insert(Virgil.thrownRocks, rock)

    REVEL.GetData(rock).virgil = ent
    REVEL.sfx:Stop(SoundEffect.SOUND_TEARS_FIRE)
    return rock
end

revel:AddCallback(ModCallbacks.MC_POST_UPDATE, function()
    for i, rock in ripairs(Virgil.thrownRocks) do
...
StageAPI.AddCallback("Revelations", RevCallbacks.POST_ENTITY_TAKE_DMG, 1, function(e, dmg, flag, src)
    local virgil = REVEL.GetData(e).virgil
    if virgil then
        if e:GetSprite():IsPlaying("Appear") then return false end

        local data = REVEL.GetData(virgil)

        local hp = e:ToPlayer():GetHearts() + e:ToPlayer():GetSoulHearts()

        if src.Entity and
            not REVEL.includes(Virgil.busyStates, data.State) and
...
revel:AddCallback(ModCallbacks.MC_ENTITY_TAKE_DMG, function(_, e, dmg, flag, src)
    if src.Entity and src.Entity.SpawnerType == REVEL.ENT.VIRGIL.id and
        ...
        local virgil = REVEL.GetData(ent).virgil
        if virgil then
            REVEL.GetData(e).pissedOff = true
            REVEL.GetData(virgil).evadeEnemy = e -- in case it hit a different enemy
            return false
        end
    end
end)
