...
-- Assume Repentogon is on for this file, as it has a norpgn alternative
...
revel:AddCallback(ModCallbacks.MC_PRE_ENTITY_SPAWN, function(_, t, v, s, pos, vel, spawn, seed)
	if t == EntityType.ENTITY_PICKUP then
        if v == PickupVariant.PICKUP_HEART and ReplacedHeartSubTypes[s] then
...
revel:AddCallback(ModCallbacks.MC_POST_PLAYER_INIT, function(_, player)
    if REVEL.IsSarah(player) then
        for _, item in ipairs(SarahItemBlacklist) do
...
StageAPI.AddCallback("Revelations", RevCallbacks.POST_STAGEAPI_NEW_ROOM_WRAPPER, 1, function()
    ...
    if REVEL.room:GetType() == RoomType.ROOM_ANGEL
    and REVEL.level:GetCurrentRoomIndex() ~= GridRooms.ROOM_ANGEL_SHOP_IDX
    and hasPenanceEffect
    then
    ...
    if hasPenanceEffect then
        REVEL.level:AddAngelRoomChance(1)
    end
end)

revel:AddCallback(ModCallbacks.MC_POST_DEVIL_CALCULATE, function(_, chance)
    for _, player in ipairs(REVEL.players) do
        if REVEL.HasPenanceEffect(player)  then
            if HasPenanceBlackHearts(player) then
                return 1
            else
                return 0
            end
        ...
        if REVEL.room:IsClear() then
            for i = 0, 7 do
                ...
                if door and (door.TargetRoomType == RoomType.ROOM_ANGEL or door.TargetRoomType == RoomType.ROOM_DEVIL) then
                    -- Not necessary, handled by POST_CALCULATE_DEIVL_CHANCE
                    -- local blackHearts = REVEL.GetBlackHearts(player) + revel.data.run.penanceBlackHeartsOut[REVEL.GetPlayerID(player)]
...
revel:AddCallback(ModCallbacks.MC_ENTITY_TAKE_DMG, function(_, player, amount, flags, source, iframes)
    player = player:ToPlayer()

    if REVEL.HasPenanceEffect(player) 
    and not HasBit(flags, DamageFlag.DAMAGE_FAKE) and not REVEL.room:IsClear() 
