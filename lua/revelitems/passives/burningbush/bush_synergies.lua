        ...
        if not data.__LaserBushInit then
            ...
            elseif e:IsCircleLaser() then --Tech x without brimstone
                ...
                for i=1,math.floor(4+e.Radius/6) do
                    ...
                    eff:GetData().Angle = angle
                    eff:GetData().Distance = e.Radius * 0.6

                    eff.SpriteOffset = REVEL.GetOrbitOffset(angle, eff:GetData().Distance) + e.SpriteOffset - e.Velocity
                end
            end
...
StageAPI.AddCallback("Revelations", "REV_ON_TEAR", 0, function(e, data, spr, player, split)
    if player:HasWeaponType(WeaponType.WEAPON_MONSTROS_LUNGS) and not split then
        if player:HasCollectible(149) or not REVEL.ITEM.BURNBUSH:PlayerHasCollectible(player) or data.BurningBush or player:GetData().FiringFireTear then return end
...
StageAPI.AddCallback("Revelations", "REV_ON_TEAR", 1, function(e, data, spr, player, split)
    if not split and not player:GetData().FiringFireTear and HasBit(e.TearFlags, TearFlags.TEAR_ABSORB) and REVEL.ITEM.BURNBUSH:PlayerHasCollectible(player) then --lacryphage tear
...
do
    local function burnbushGelloUpdate(_, familiar)
        ...
        and familiar.FrameCount < 15 then --temp, only when launched
            ...
            -- BurningbushShooterUpdate(familiar, data, familiar.Player:GetData().BushStats, familiar.Player, false, false)
