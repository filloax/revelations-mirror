...
function REVEL.SpawnSpotlight(color, pos, fadein, darkAround, invisibleSpot, ignoreDarkness)
    ...
    local data, sprite = spot:GetData(), spot:GetSprite()
    ...
    if darkAround then
        ...
        if not (DarkAroundRenderer and DarkAroundRenderer.Ref) then
            ...
            renderer:GetData().DarkAroundSpotlightRenderer = true
    ...
    beam:GetData().Spot = EntityPtr(spot)
...
function REVEL.SpawnEntSpotlight(color, ent, effect, fadein, darkAround, invisibleSpot, noOverrideLast)
    ...
    if effect then
        spot:GetData()[effect] = true
    end

    local data = spot:GetData()

    data.Entity = ent
    if not noOverrideLast and ent:GetData().LastSpotlight then
        ent:GetData().LastSpotlight:GetData().StopBeam = true
    end
    ent:GetData().LastSpotlight = spot

    return spot
end

function REVEL.FadeoutSpotlight(spot)
    spot:GetData().StopBeam = true
end
...
function REVEL.SetSpotlightColor(spot, color)
    local data, spr = spot:GetData(), spot:GetSprite()
    ...
    if not IsAnimOn(spr, "Spot Fadeout") then
        ...
        elseif data.Entity then
            ...
            or (not data.Entity.Visible and not data.ForceWhenEntInvisible) then
                ...
                local edata = data.Entity:GetData()
...
local function spotlightsPostRender()
    ...
    for _, beam in ipairs(beams) do
        local bdata, bsprite = beam:GetData(), beam:GetSprite()
...
local function darkAroundRendererPostRender(_, effect)
    if effect:GetData().DarkAroundSpotlightRenderer then
