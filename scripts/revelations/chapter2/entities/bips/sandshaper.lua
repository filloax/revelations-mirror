...
local NpcPathMap = REVEL.NewPathMapFromTable("Sandshaper", {
    GetTargetIndices = function()
        ...
        for _, npc in ipairs(sandshapers) do
            if REVEL.GetData(npc).TargetIndex then
                if not addedTargets[REVEL.GetData(npc).TargetIndex] then
                    targetIndices[#targetIndices + 1] = REVEL.GetData(npc).TargetIndex
                    initialTargetIndices[#initialTargetIndices + 1] = REVEL.GetData(npc).TargetIndex
                    addedTargets[REVEL.GetData(npc).TargetIndex] = true
                end
            end
        end
...
revel:AddCallback(ModCallbacks.MC_NPC_UPDATE, function(_, npc)
    ...
    local pathfinder, target, data, sprite = npc.Pathfinder, npc:GetPlayerTarget(), REVEL.GetData(npc), npc:GetSprite()
    ...
    elseif data.State == "ProjectileRaise" then
        ...
        if sprite:IsPlaying("ProjectileRaiseLoop") then
            ...
            if #data.Projectiles < 8 then
                ...
                REVEL.GetData(newProjectile).Sandshaper = npc
...
revel:AddCallback(ModCallbacks.MC_POST_PROJECTILE_UPDATE, function(_, pro)
    if REVEL.GetData(pro).SandProjectile and not pro:HasProjectileFlags(ProjectileFlags.NO_WALL_COLLIDE) then
        pro.GridCollisionClass = EntityGridCollisionClass.GRIDCOLL_WALLS

        if REVEL.GetData(pro).Sandshaper and not REVEL.GetData(pro).Sandshaper:Exists() then
...
revel:AddCallback(ModCallbacks.MC_ENTITY_TAKE_DMG, function(_, ent, amount)
    if ent.Variant == REVEL.ENT.SANDSHAPER.variant then
        ...
        if REVEL.GetData(ent).CounterCooldown <= 0 then
            REVEL.GetData(ent).BeenHit = true
            REVEL.GetData(ent).CounterCooldown = 120
        end
