    ...
    if not HasBit(useFlags, UseFlag.USE_CARBATTERY) and player.Variant == PlayerVariant.PLAYER then
        for _,t in ipairs(REVEL.roomProjectiles) do
            if REVEL.room:IsPositionInRoom(t.Position, 0) then
                ...
                REVEL.GetData(fruit).FruitID = fdata.ID
                REVEL.GetData(fruit).Flag = fdata.Cache
                t:Remove()
            end
        end
        REVEL.GetData(player).WakaWakaTime = WakaWakaFruitDecay * 2
    ...
    for _,player in ipairs(REVEL.players) do
        ...
        for i,fruit in ipairs(Isaac.FindByType(REVEL.ENT.WAKAWAKA_FRUIT.id, REVEL.ENT.WAKAWAKA_FRUIT.variant, -1, false, false)) do
            if not fruit:GetSprite():IsPlaying("Collect") and fruit.Position:DistanceSquared(player.Position) < (25 * 25) then
                ...
                local id = REVEL.GetData(fruit).FruitID
                ...
                player:AddCacheFlags(REVEL.GetData(fruit).Flag)
...
revel:AddCallback(ModCallbacks.MC_POST_UPDATE, UpdateFruits)

revel:AddCallback(ModCallbacks.MC_PRE_FAMILIAR_COLLISION, function(_, e, c)
    if e.SubType == REVEL.ITEM.WAKA_WAKA.id then
        if REVEL.GetData(e.Player).WakaWakaTime and REVEL.GetData(e.Player).WakaWakaTime > 0 then
