...
function ShootFireTear(player, pos, vel, strengthPct, fromPlayer, lifeMult, dmgMult, scaleMult)
    ...
    local data, sprite = REVEL.GetData(fire), fire:GetSprite()
...
local function FireTearUpdate(fire)
    ...
    if REVEL.STAGE.Glacier:IsStage() then
        ...
        for _, hazard in ipairs(iceHazards) do
            ...
            if not REVEL.GetData(hazard).LockedInPlace and hazard.Position:DistanceSquared(fire.Position) < minDist*minDist then
...
function spawnPermaFire(pos, vel, spawnerFire, sizeMult, lifeMult, noHeight, randomReplace, dmgMultReplace)
    local player
    local sdata = REVEL.GetData(spawnerFire)
    ...
    local data, sprite = REVEL.GetData(permaFire), permaFire:GetSprite()
...
revel:AddCallback(ModCallbacks.MC_POST_UPDATE, function()
    ...
    local fires = REVEL.filter(REVEL.roomTears, function(tear) return REVEL.GetData(tear).BurningBush end)
    for _, fire in ipairs(fires) do
        FireTearUpdate(fire)

        if REVEL.GetData(fire).SetScale then
            fire.SpriteScale = Vector.One * REVEL.GetData(fire).SetScale
        end
