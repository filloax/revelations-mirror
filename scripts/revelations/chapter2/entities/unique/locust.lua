...
StageAPI.AddCallback("Revelations", "POST_SPAWN_ENTITY", 1, function(entity)
    if entity.Type == REVEL.ENT.LOCUST.id and entity.Variant == REVEL.ENT.LOCUST.variant then
        ...
        for i = 1, locustCount do
            ...
            REVEL.GetData(locust).Leader = entity
...
revel:AddCallback(ModCallbacks.MC_POST_RENDER, function(_, npc)
    ...
    for _, locust in ipairs(locusts) do
        if REVEL.GetData(locust).IsEatingEnemy then
            eatingEnemy = true
        end
        if REVEL.GetData(locust).Dashing then
            dashinglocust = true
        end
    end
    ...
    if data.LocustsLeading then
        ...
        for i, locust in ripairs(data.LocustsLeading) do
            if not locust:Exists() or locust:IsDead() or locust:HasEntityFlags(EntityFlag.FLAG_ICE_FROZEN) then
                ...
                if not REVEL.GetData(locust).Dashing then
        ...
        for _, ent in ipairs(REVEL.roomEnemies) do
            ...
            if not REVEL.includes(locustImmune, ent.Type) and (ent:IsVulnerableEnemy() or ent.Type == EntityType.ENTITY_RAG_MEGA) then
                local nearLocusts = {}
                for _, locust in ipairs(locusts) do
                    if REVEL.GetData(locust).IsDashing and locust.Position:DistanceSquared(ent.Position) < locustRadiusSquared then
                ...
                if #nearLocusts >= 3 then
                    ...
                    if ent.Type == REVEL.ENT.ANTLION.id and ent.Variant == REVEL.ENT.ANTLION.variant and REVEL.GetData(ent).Burrowed and not ent:IsDead() then
        ...
        for _, locust in ipairs(locusts) do
            if REVEL.GetData(locust).IsDashing then
        ...
        if not data.Dashing then
            ...
            if data.DashTimer <= 0 then
                ...
                for i, locust in ipairs(data.LocustsLeading) do
                    local ldata = REVEL.GetData(locust)
        ...
        if not data.Leader then
            ...
            if #validLeaders > 0 then
                ...
                data.Leader = closest
                local ldata = REVEL.GetData(closest)
            ...
            REVEL.StopUsingPathMap(REVEL.GenericFlyingChaserPathMap, npc)
            if not data.Leader:Exists() or data.Leader:IsDead() or data.Leader:HasEntityFlags(EntityFlag.FLAG_ICE_FROZEN) or not REVEL.GetData(data.Leader).LocustsLeading or (npc:HasEntityFlags(EntityFlag.FLAG_FRIENDLY) and not data.Leader:HasEntityFlags(EntityFlag.FLAG_FRIENDLY)) then
                data.Leader = nil
            end
        end
    end
