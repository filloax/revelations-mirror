...
local function InitCats(npc, yarn)
	...
	if not yarn then
		...
		REVEL.GetData(yarn).CatsInitialized = true
    ...
    REVEL.GetData(yarn).IsChampion = isChampion

    if REVEL.GetData(yarn).IsChampion then
        REVEL.GetData(yarn).bal = REVEL.GetBossBalance(catBalance, "RIPGuppy")
    else
        REVEL.GetData(yarn).bal = REVEL.GetBossBalance(catBalance, "Default")
    end
    ...
    for i, cat in ipairs(cats) do
        ...
        local cdata = REVEL.GetData(cat)
        cdata.Leader = npc
        cdata.Yarn = yarn
        cdata.bal = REVEL.GetData(yarn).bal
    ...
    REVEL.GetData(npc).IsLeader = true
end

local function SetBMinCooldown(cats, cooldown)
    for _, cat in ipairs(cats) do
        if not REVEL.GetData(cat).AttackBCooldown or REVEL.GetData(cat).AttackBCooldown < cooldown then
            REVEL.GetData(cat).AttackBCooldown = cooldown
        end
    end
end
...
local function SetMinAttackCooldown(cats, cooldown)
    for _, cat in ipairs(cats) do
        if not REVEL.GetData(cat).AttackCooldown or REVEL.GetData(cat).AttackCooldown < cooldown then
            REVEL.GetData(cat).AttackCooldown = cooldown
        end
    end
end
...
local function CatastropheYarnUpdate(npc)
    REVEL.GetData(npc).Invulnerable = true
    ...
    if not (npc.Velocity:LengthSquared() > 0) then
        if not REVEL.GetData(npc).Touched and not npc:GetSprite():IsPlaying("Untouched") then
            npc:GetSprite():Play("Untouched", true)
        end
    else
        REVEL.GetData(npc).Touched = true
    end

    npc.Velocity = npc.Velocity * 0.975

    if REVEL.GetData(npc).Touched then
    ...
    if npc.Velocity:LengthSquared() > 10 then
        ...
        REVEL.GetData(npc).LastStruckByCat = nil
    end
    ...
    if REVEL.GetData(npc).LastStruckByCat then
        npc.CollisionDamage = 1
    else
        npc.CollisionDamage = 0
    end

    if not REVEL.GetData(npc).CatsInitialized then
        InitCats(nil, npc)
        REVEL.GetData(npc).CatsInitialized = true
    end
    ...
    if REVEL.GetData(npc).IsChampion then
        if not REVEL.GetData(npc).HPCalculated then
            local hp = 0
            ...
            npc.MaxHitPoints = hp
            npc.HitPoints = hp
            REVEL.GetData(npc).HPCalculated = true
        end
...
local function catastrophe_NpcUpdate(_, npc)
        ...
        for i, cat in ripairs(data.Cats) do
            if not cat:Exists() or cat.HitPoints <= 100 then
                ...
                if not cat:GetSprite():IsPlaying("Appear") then
                    local cdata, csprite = REVEL.GetData(cat), cat:GetSprite()
    ...
    if not amDeadCat and catsInPhaseTwo == #data.Cats and not data.AllCatsPhaseTwo then
        for _, cat in ipairs(data.Cats) do
            REVEL.GetData(cat).AllCatsPhaseTwo = true
        end
    end
    ...
    if not amDeadCat and data.IsLeader then
        if not data.FightStarted and data.Yarn then
            if data.Yarn.Velocity:LengthSquared() > 0 then
                for _, cat in ipairs(data.Cats) do
                    REVEL.GetData(cat).FightStarted = true
                end
            end
        else
            local shouldBeOut = 2
            ...
            if #activeCats < shouldBeOut and #validToWake > 0 and allInitialized then
                ...
                for i = 1, math.min(#validToWake, numNeeded) do
                    ...
                    REVEL.GetData(cat).Wrapped = false
                    REVEL.GetData(cat).State = "Idle"
                end

                for _, cat in ipairs(data.Cats) do
                    REVEL.GetData(cat).LastWrapped = nil
                end
            end
        end
    end
        ...
        if sprite:WasEventTriggered("Swipe") then
            if npc.Velocity:LengthSquared() > 7 ^ 2 then
                if data.Yarn.Position:DistanceSquared(npc.Position) < (npc.Size + data.Yarn.Size) ^ 2 then
                    REVEL.GetData(data.Yarn).LastStruckByCat = true
        ...
        if sprite:IsEventTriggered("Shoot") then
            ...
            if variant == REVEL.ENT.CATASTROPHE_GUPPY.variant then
                ...
                for i = -1, 1 do
                    ...
                    REVEL.GetData(fly).CatastropheSpawned = true
...
local function catastrophe_EntTakeDamage(_, ent, dmg, flag, source)
    if REVEL.GetData(ent).Invulnerable then
        if ent.Variant == REVEL.ENT.CATASTROPHE_YARN.variant then
            REVEL.PlaySound(ent, REVEL.GetData(ent).bal.Sounds.HitYarn)
        elseif not REVEL.GetData(ent).ChampionGuppy then
    ...
    elseif IsCat(ent.Variant) then
        if ent.HitPoints - dmg - REVEL.GetDamageBuffer(ent) <= 100 then
    		...
    		return false
        elseif REVEL.GetData(ent).IsChampion then
            ent.HitPoints = ent.HitPoints + dmg
            REVEL.GetData(ent).Yarn.HitPoints = REVEL.GetData(ent).Yarn.HitPoints - dmg
        end
    end
end
