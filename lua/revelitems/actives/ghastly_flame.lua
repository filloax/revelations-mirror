...
function revel.gflame.fireFlame(player)
    local flame = REVEL.ENT.DECORATION:spawn(
        player.Position,
        REVEL.GetCorrectedFiringInput(player) * revel.gflame.flameSpeed,
        player
    )
    ...
    flame.Color = revel.gflame.flameColor
    flame:GetSprite():LoadGraphics()
    --  flame:GetSprite():Play("FireStage00", true)
    flame:GetData().timeMax = revel.gflame.flameLife
    flame:GetData().time = flame:GetData().timeMax
    flame:GetData().homingSpeed = 0
    flame:GetData().homingLerp = 1
    flame.CollisionDamage = player.Damage * 2.5
    flame.GridCollisionClass = EntityGridCollisionClass.GRIDCOLL_NONE

    local i = #revel.gflame.flames + 1

    revel.gflame.flames[i] = flame
    revel.gflame.flameSeeds[flame.InitSeed] = player
end

revel:AddCallback(ModCallbacks.MC_POST_UPDATE, function()
    for i, v in ripairs(revel.gflame.flames) do
        local data = v:GetData()

        local speed = REVEL.Lerp(revel.gflame.flameSpeedEnd,
                                    revel.gflame.flameSpeed,
        ...
        elseif data.time > 0 then
            ...
            v.SpriteScale = Vector(m, m)
            v.SizeMulti = Vector(m, m)
            v.Color = Color.Lerp(REVEL.NO_COLOR, revel.gflame.flameColor,
                                    data.time / 25)
            data.time = data.time - 1
        else
            revel.gflame.flameSeeds[v.InitSeed] = nil
            v:Remove()
            table.remove(revel.gflame.flames, i)
        end
    end

    for i, v in ripairs(revel.gflame.ghostFlames) do
        if v:IsDead() or not v:Exists() then
            table.remove(revel.gflame.ghostFlames, i)
        end
...
revel:AddCallback(ModCallbacks.MC_FAMILIAR_INIT, function(_, e)
    local data, spr = e:GetData(), e:GetSprite()
    ...
    if REVEL.IsThereCurse(LevelCurse.CURSE_OF_DARKNESS) then
        data.light = REVEL.SpawnLightAtEnt(e, revel.gflame.lightColor, 1.3)
    end
...
revel:AddCallback(ModCallbacks.MC_FAMILIAR_UPDATE, function(_, e)
    ...
    if e.FrameCount == revel.gflame.ghostLife then
    ...
    if (not data.light or not data.light:Exists()) and
        ...
        data.light = REVEL.SpawnLightAtEnt(e, revel.gflame.lightColor, 1.5)
    end
    ...
    if spr:IsPlaying("idle" .. data.type) then
        ...
        if e.FrameCount % 20 == 0 then
            local flame = Isaac.Spawn(1000, EffectVariant.RED_CANDLE_FLAME,
                                        0, e.Position +
                                            REVEL.GetXVector(
                                                revel.gflame.ghostCandleOffset[data.type]),
                                        RandomVector() * 2, e)
            ...
            flame:GetData().height =
                revel.gflame.ghostCandleOffset[data.type].Y
            flame:GetData().fallingSpeed = -5
            flame.SpriteOffset = REVEL.GetYVector(revel.gflame
            ...
            table.insert(revel.gflame.ghostFlames, flame)
        end
...
revel:AddCallback(ModCallbacks.MC_ENTITY_TAKE_DMG,
    ...
    if e:ToNPC() and e.HitPoints - dmg - REVEL.GetDamageBuffer(e) <= 0 and src.Entity and
        revel.gflame.flameSeeds[src.Entity.InitSeed] then
        local player = revel.gflame.flameSeeds[src.Entity.InitSeed]
