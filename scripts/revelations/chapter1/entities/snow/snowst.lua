        ...
        if not data.IsShrieking then
            ...
            if sprite:IsEventTriggered("Shoot") then
                ...
                for i=1, 4 do
                    ...
                    if spawndip[i] then
                        REVEL.GetData(proj).SpawnDip = true
                    end
                    REVEL.GetData(proj).SnowstProjectile = true
                    REVEL.GetData(proj).ProjectileSize = math.random(7,9)
                    proj.Height = -20
...
revel:AddCallback(ModCallbacks.MC_POST_PROJECTILE_UPDATE, function(_, proj)
    ...
    if data.SnowstProjectile then
        proj:GetSprite():SetFrame("RegularTear"..tostring(REVEL.GetData(proj).ProjectileSize), 0)
...
revel:AddCallback(ModCallbacks.MC_ENTITY_TAKE_DMG, function(_, ent, dmg)
    if ent.Variant == REVEL.ENT.SNOWST.variant then
        if REVEL.GetData(ent).IsShrieking then
            return false
        end
    end
end, REVEL.ENT.SNOWST.id)
