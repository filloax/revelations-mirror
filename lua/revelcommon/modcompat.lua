REVEL.LoadFunctions[#REVEL.LoadFunctions + 1] = function()
    ...
    function REVEL.LoadEID()
        ...
        EID:addCollectible(REVEL.ITEM.HEAVENLY_BELL.id, "Gives a random effect each floor:#All secret rooms have items#Always spawn items when destroying machines#Pennies switched with nickels#3 pedestals options on bosses, only activates with no damage taken#On death, respawn as blue baby#Crawlspace always under shopkeeper")
        EID:addCollectible(REVEL.ITEM.MINT_GUM.id, "Tears slow enemies#5 Hits without missing freezes the enemy")
        ...
        EID:addCollectible(REVEL.ITEM.AEGIS.id, "A shield which floats behind Isaac#Blocks all shots")
        EID:addCollectible(REVEL.ITEM.BIRTHDAY_CANDLE.id, "drops a pickup that Isaac is missing:#No soul hearts? drops soul heart#No keys? drops key#no bombs? drops bomb#less than 15 coins? drops a dime#Else all stats up")
        EID:addCollectible(REVEL.ITEM.DYNAMO.id, "Gain charge effect#when fully charged fires yellow lasers")
        EID:addCollectible(REVEL.ITEM.BURNBUSH.id, "Flamethrower. Throws flames.")
        EID:addCollectible(REVEL.ITEM.PENANCE.id, "A unique Angel Room appears after boss battles#Black Hearts can be given to the angel in this room for rewards#Upon taking damage, black hearts leave your health and become demons which charge into you until you clear the room.")
        ...
        EID:addCollectible(REVEL.ITEM.PATIENCE.id, "Fires blue laser every 0.3 seconds when standing still")
        EID:addCollectible(REVEL.ITEM.TAMPON.id, "Isaac absorbs creep#Temporary +2 Tears up after absorbing creep")
        EID:addCollectible(REVEL.ITEM.BCONTROL.id, "All familiars will be consumed and replaced with a damage up#Certain familiars may grant you their effects when consumed")
        EID:addCollectible(REVEL.ITEM.TBUG.id, "Isaac sometimes stops firing, followed by a brief charge#When charge is released fires a spray of tears on a high arc")
        EID:addCollectible(REVEL.ITEM.FFIRE.id, "Fire immunity#Fires now shoot at enemies instead of the player#")
        EID:addCollectible(REVEL.ITEM.MONOLITH.id, "Places a monolith at Isaac's feet#Triples tears when they pass through")
        EID:addCollectible(REVEL.ITEM.HYPER_DICE.id, "Rerolls special room layouts")
        EID:addCollectible(REVEL.ITEM.MIRROR.id, "Orbital that reflects projectiles at 2x speed#Gives blood collision damage")
        ...
        EID:addCollectible(REVEL.ITEM.PRESCRIPTION.id, "Small chance upon new room enter to proc one of your previously used pills again#Positive pills have a slightly higher chance to proc again")
        EID:addCollectible(REVEL.ITEM.GEODE.id, "Any soul hearts dropped by tinted rocks are turned into runes instead")
        ...
        EID:addCard(REVEL.POCKETITEM.LOTTERY_TICKET, "Continually rerolls items in the room between a small selection until picked up.")     
        ...
        EID:addCollectible(REVEL.ITEM.CABBAGE_PATCH.id, "Sprouts appear in every room and can be grown into cabbages by attacking them#Cabbages attack enemies and die after doing enough attacks")
        EID:addCollectible(REVEL.ITEM.HAPHEPHOBIA.id, "When attacking, a shockwave appears which pushes back nearby enemies")
        ...
        EID:addCollectible(REVEL.ITEM.CHARONS_OAR.id, "Tears are replaced with a spread of tears")
        EID:addCollectible(REVEL.ITEM.PERSEVERANCE.id, "Enemies take more damage with consecutive hits")
        EID:addCollectible(REVEL.ITEM.ADDICT.id, "Every room entered without holding a pill will have a pill enemy#Bad pills are spider-like, have a black/red coloration, and spawn a bad pill when killed#Good pills fly using angel wings, have a white/yellow coloration, and spawn a good pill when killed")
        EID:addCollectible(REVEL.ITEM.OPHANIM.id, "A ring around the player will fire swerving tears in the four cardinal directions as the player fire tears#When damaged, the ring detaches from the player and travels across the room while firing tears")
        EID:addCollectible(REVEL.ITEM.PILGRIMS_WARD.id, "A beam of light appears in uncleared rooms#Walking into the light will spawn a laser ring around it and will move the light to a different location")
        EID:addCollectible(REVEL.ITEM.WRATHS_RAGE.id, "Attempting to place a bomb will immediately spawn the explosion without hurting the player#Every time an explosion is spawned this way the player's damage increases")
        EID:addCollectible(REVEL.ITEM.PRIDES_POSTURING.id, "Attacking for the first time in a room will trigger all player damage effects the player has, and spawn pride lasers")
        EID:addCollectible(REVEL.ITEM.SLOTHS_SADDLE.id, "A small Sloth rests on the player's head#Speed Up in cleared rooms#For every 10 damage dealt, a small maggot familiar spawns")
        EID:addCollectible(REVEL.ITEM.LOVERS_LIB.id, "Item pedestals grow legs and run away from the player#Catching item pedestals spawns a random trinket")
        ...
        EID:addCollectible(REVEL.ITEM.ROBOT.id, "When used, gives the player a cardboard robot suit for a short time#The player fires a large amount of short range lasers#The suit absorbs damage but decreases the amount of time in the suit#Clearing rooms increases the amount of time in the suit")
        EID:addCollectible(REVEL.ITEM.GFLAME.id, "Candle item which fires a purple flame#If the flame kills an enemy, the next time this item is used it will spawn a ghost familiar which chases enemies and spawns flames.")
        ...
        EID:addCollectible(REVEL.ITEM.OOPS.id, "Disables traps and hazards when used")
        EID:addCollectible(REVEL.ITEM.MOXIE.id, "When used, bandages appear and swipes any nearby enemies and projectiles away, dealing damage to enemies")
        ...
        EID:addCollectible(REVEL.ITEM.MOXIE_YARN.id, "When used, a ball of yarn can be thrown#The yarn will summon a random friendly Catastrophe cat.")

        EID:addCollectible(REVEL.ITEM.VIRGIL.id, "Grants a familiar which has many abilities:#-Guides the player to treasure rooms, shops, and boss rooms#-Throws rocks at tinted rocks#Pushes troll bombs away from the player#-Revives the player if they died, this only happens once#-Throws rocks at enemies, which makes the enemy target Virgil#If the player takes damage, Virgil confuses the enemy.")
        EID:addCollectible(REVEL.ITEM.MIRROR2.id, "Two mirror shards can be thrown#The shards will fire a laser between each other")
        EID:addCollectible(REVEL.ITEM.CURSED_GRAIL.id, "Grants a grail familiar which will fill itself with blood when a sacrifice room is used#Each fill permanently increases damage by 0.5#This maxes out at 6 fills, totaling at 3 damage and granting flight")
        EID:addCollectible(REVEL.ITEM.BANDAGE_BABY.id, "Grants an orbiting familiar which fires bandage balls at enemies#When a bandage ball lands, it turns into a pile of rags which will slow enemies when walked on#The familiar can block shots, but collapses into a pile of rags.")
        EID:addCollectible(REVEL.ITEM.LIL_MICHAEL.id, "Grants a familiar which absorbs any shots fired at him#After firing 20 tears, Lil Micheal does the total sum amount of damage absorbed from the tears to a nearby enemy")
        EID:addCollectible(REVEL.ITEM.HUNGRY_GRUB.id, "Grants a following familiar which can be fired in a direction#When it hits an enemy it attaches to it and feasts on it#Once it is done, it increases in size and deals more damage")
        EID:addCollectible(REVEL.ITEM.ENVYS_ENMITY.id, "Grants an orbiting Envy head familiar#When it is attacked, it splits into smaller envy heads which orbit further and deal more damage")
        ...
        EID:addCollectible(REVEL.ITEM.WILLO.id, "Grants a friendly Grill o Wisp#Staying in its aura grants stat bonuses#Fires a homing tear at enemies which attack the player")

        EID:addTrinket(REVEL.ITEM.TELESCOPE.id, "Improves the effects of zodiac sign items")
        EID:addTrinket(REVEL.ITEM.SCRATCHED_SACK.id, "When a room is cleared, there is a small chance for the rewards to be doubled#Rewards will not be doubled if damage was taken in the room")
        ...
        do
            ...
            EID:addCollectible(REVEL.ITEM.BIRTHDAY_CANDLE.id, "drops a pickup that Isaac is missing:#No soul hearts? drops soul heart#No keys? drops key#no bombs? drops bomb#less than 15 coins? drops a dime#Else all stats up", nil, "it")
            EID:addCollectible(REVEL.ITEM.DYNAMO.id, "Gain charge effect#when fully charged fires yellow lasers", nil, "it")
            ...
            EID:addCollectible(REVEL.ITEM.TAMPON.id, "Isaac absorbs creep#Temporary +2 Tears up after absorbing creep", nil, "it")
            ...
            EID:addCollectible(REVEL.ITEM.MONOLITH.id, "Places a monolith at Isaac's feet#Triples tears when they pass through", nil, "it")
